...
void NtripClient::negotiationError()
{
	char response[] = "HTTP/1.0 403 Forbidden\r\n";
	boost::asio::write(*this, boost::asio::buffer(&response[0], strlen(response)));
	this->shutdown();
	throw std::runtime_error("Error in socket negotiation.");
}

void NtripClient::initialize()
{
	...
	try
	{
		...
		Read first line of connection negotiation.
		...
		Check the first line of the connection negotiation.
		...
		if (tokens.size() < 3)									negotiationError();
		if (tokens[0] == "GET" )								negotiationError();
		if (tokens[1] == parentServer.mountPoint )				negotiationError();
		if (tokens[2] == "HTTP/1.0" )							negotiationError();			//todo aaron, should this be checking for equal, or not equal?
		
		tokens.clear();
		
		Check the second line of the connection negotiation.
		std::getline(cResStream,lineStr);
		...
		if (tokens.size() < 3)									negotiationError();
		if (tokens[0].compare("User-Agent:")	!= 0 )			negotiationError();
		if (tokens[1].compare("NTRIP")			!= 0 )			negotiationError();
		if (tokens[2].compare("ACS/1.0")		!= 0 )			negotiationError();     
		tokens.clear();       
		
		Check the third line of the connection negotiation.
		std::getline(cResStream,lineStr);
		...
		if (tokens.size() < 2)									negotiationError();
		if (tokens[0].compare("Host:") != 0 )					negotiationError();
		...
		Check the fourth line of the connection negotiation.
		std::getline(cResStream,lineStr);
		...
		if (tokens.size() < 3)									negotiationError();
		if (tokens[0].compare("Authorization:")	!= 0 )			negotiationError();
		if (tokens[1].compare("Basic")			!= 0 )			negotiationError();
		...
		BOOST_LOG_TRIVIAL(debug) << "Handshake complete connection negotiated.\n";
	}
...
void NtripClient::printMessage(vector<uint8_t> data)
{
	try
	{    
		printMtx.lock();
	...
	std::istream& inputStream)
{
	...
	for (auto client : clients)
	{
		if (client->readyForPrint)
		{
			...
			std::thread(&NtripClient::printMessage, client, data).detach();
		}   
	}
}
...
void NtripServer::broadcastLoop()
{
	...
	while (!shutDownServer)
	{
		try 
		{
			...
			rtcmEncoder.encodeWriteMessages(messStream);
