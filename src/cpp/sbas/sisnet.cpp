...
void DS2DCParser::parse(std::istream& inputStream)
{
    ...
    while (inputStream)
    {
        ...
        if (sbas.freq == 1)
            sbas.type = std::stoi(mess.substr(2, 2), nullptr, 16) >> 2;
        else
            sbas.type = std::stoi(mess.substr(1, 2), nullptr, 16) >> 2;
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            if (i < 28)
            {
                frame[i] += ((byte >> 6) & 0xFFFF);
                frame[i + 1] += ((byte << 2) & 0x00FFFF);
            }
            else if (i == 28)
            {
                frame[i] += ((byte >> 6) & 0xFFFF);
                frameCRC += ((byte << 18) & 0xFC0000);
            }
            else if (i == 29)
            {
                frameCRC += ((byte << 10) & 0x03FC00);
            }
            else if (i == 30)
            {
                frameCRC += ((byte << 2) & 0x0003FC);
            }
            else if (i == 31)
            {
                frameCRC += ((byte >> 6) & 0x000003);
            }
        }
...
void SisnetStream::sisnetResponseHandler(const boost::system::error_code& err)
{
    ...
    if (pos == string::npos)
    {
        BOOST_LOG_TRIVIAL(error) << "Error in starting string : Invalid Server Response:  "
                                 << private_string;
        delayedReconnect();
        return;
    }
...
void SisnetStream::sisnetStartHandler(const boost::system::error_code& err)
{
    ...
    timer.async_wait(
        boost::bind(&SisnetStream::timeoutHandler, this, boost::asio::placeholders::error)
    );
...
void SisnetStream::requestResponseHandler(const boost::system::error_code& err)
{
    ...
    if (pos == string::npos)
    {
        BOOST_LOG_TRIVIAL(error) << "Error in Authentication : Invalid Server Response:  "
                                 << privateString;
        delayedReconnect();
        return;
    }
    ...
    boost::asio::async_write(
        *_socket,
        request,
        boost::bind(&SisnetStream::sisnetStartHandler, this, boost::asio::placeholders::error)
    );
}
