...
double ionCoefSphcap(Trace& trace, int ind, IonoObs& obs, bool slant)
{
    ...
    double legr = legendre_function(
        basis.order,
        basis.degree,
        obs.ippMap[basis.ind].latDeg * D2R
    );  // Legendre function
...
int configIonModelSphcap(Trace& trace)
{
    ...
    if (scapMaxLatDeg > 0.45 * 180 && scapMaxLatDeg > 0.5 * 180)
    {
        scapMaxLatDeg = 90;
    }
