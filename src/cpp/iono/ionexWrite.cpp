...
double
ionVtec(Trace& trace, GTime time, VectorPos& ionPP, int layer, double& vtecstd, KFState& kfState)
{
    switch (acsConfig.ionModelOpts.model)
    {
        case E_IonoModel::SPHERICAL_HARMONICS:
            return ionVtecSphhar(trace, time, ionPP, layer, vtecstd, kfState);
        case E_IonoModel::SPHERICAL_CAPS:
            return ionVtecSphcap(trace, time, ionPP, layer, vtecstd, kfState);
        case E_IonoModel::BSPLINE:
            return ionVtecBsplin(trace, time, ionPP, layer, vtecstd, kfState);
        default:
            return 0;
    }
}

void writeIonexHead(Trace& ionex)
{
    ionexMapIndex = 1;
    ...
    tracepdeex(
        0,
        ionex,
        "  %6.1f%6.1f%6.1f%40sLAT1 / LAT2 / DLAT\n",
        ionexLatmin + ionexLatinc * (ionexLatres - 1),
        ionexLatmin,
        -ionexLatinc,
        " "
    );
    tracepdeex(
        0,
        ionex,
        "  %6.1f%6.1f%6.1f%40sLON1 / LON2 / DLON\n",
        ionexLonmin,
        ionexLonmin + ionexLoninc * (ionexLonres - 1),
        ionexLoninc,
        " "
    );
...
void writeIonexEpoch(Trace& trace, Trace& ionex, GTime time, KFState& kfState)
{
    GTow tow     = time;
    int  timeseg = floor(tow / acsConfig.ionexGrid.time_res);

    if (lastIonex >= 0 && lastIonex == timeseg)
    {
        return;
    }
    ...
    tracepdeex(
        4,
        trace,
        "  ..Writing IONEX epoch:%6.0f%6.0f%6.0f%6.0f%6.0f%6.0f \n",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
    ...
    tracepdeex(
        0,
        ionex,
        "%6.0f%6.0f%6.0f%6.0f%6.0f%6.0f%24sEPOCH OF CURRENT MAP\n",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
    ...
    for (int ihgt = 0; ihgt < acsConfig.ionModelOpts.layer_heights.size(); ihgt++)
        for (int ilat = 0; ilat < ionexLatres; ilat++)
        {
            ...
            tracepdeex(
                0,
                ionex,
                "  %6.1f%6.1f%6.1f%6.1f%6.1f%28sLAT/LON1/LON2/DLON/H",
                ipp[0],
                ionexLonmin,
            ...
            for (int ilon = 0; ilon < ionexLonres; ilon++)
            {
                ...
                tracepdeex(
                    5,
                    std::cout,
                    "IPP: %8.4f,%9.4f; layr: %1d; delay: %12.6f; var: %.4e\n",
                    ipp[0] * R2D,
                    ipp[1] * R2D,
                    ihgt,
                    iono,
                ...
                if (tecrms > 9999 || tecrms <= 0)
                {
                    tecrms = 9999;
                }

                if (iono > +9999 || iono < -9999)
                {
                    iono   = 9999;
                    tecrms = 9999;
                }
...
void ionexFileWrite(Trace& trace, string filename, GTime time, KFState& kfState)
{
    ...
    if (acsConfig.ionModelOpts.layer_heights.size() < 1)
        return;
    if (acsConfig.ionModelOpts.function_order < 1)
        return;
    ...
    std0Key.num  = 0;
    if (kfState.getKFSigma(std0Key, sigma0) == false)
        return;
    if (sigma0 > 1)
        return;
