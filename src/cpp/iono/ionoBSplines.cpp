...
{
    bspBasisMap.clear();
    if (acsConfig.ionexGrid.lat_width > 180 || acsConfig.ionexGrid.lat_width < 0 ||
        acsConfig.ionexGrid.lon_width > 360 || acsConfig.ionexGrid.lon_width < 0)
    {
    ...
    if ((latmin < -90) || ((BSPLINE_LAT_CENTRE + BSPLINE_LAT_WIDTH) > 90))
    {
...
*/
double
ionVtecBsplin(Trace& trace, GTime time, VectorPos& ionPP, int layer, double& var, KFState& kfState)
{
    var = 0;
