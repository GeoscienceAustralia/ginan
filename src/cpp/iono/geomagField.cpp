/** References
 * 1. P.Alken, E.Thébault, C.D.Beggan, H.Amit, J.Aubert, J.Baerenzung, T.N.Bondar, W.J.Brown,
 * S.Califf, A.Chambodut & A.Chulliat, International geomagnetic reference field: the thirteenth
 * generation. Earth, Planets and Space, 2021.
 * 2. E.Thébault, C.C.Finlay, C.D.Beggan, P.Alken, J.Aubert, O.Barrois, F.Bertrand, T.Bondar,
 * A.Boness, L.Brocco & E.Canet. International geomagnetic reference field: the 12th generation.
 * Earth, Planets and Space, 2015.
 * 3. D.E.Winch, D.J.Ivers, J.P.R.Turner & R.J.Stening. Geomagnetism and Schmidt
 * quasi-normalization. Geophysical Journal International, 2005.
 ...
 */
void readIGRF(string filename)  ///< IGRF file to read
{
    if (filename.empty())
    {
        return;
    ...
    while (std::getline(infile, line))
    {
        ...
        else if (split[0] == "g" && split.size() > 3)
        {
            int n = stoi(split[1]);
            int m = stoi(split[2]);
        ...
        else if (split[0] == "h" && split.size() > 3)
        {
            int n = stoi(split[1]);
            int m = stoi(split[2]);
 ...
 */
double decimalYear(GTime time)  ///< time
{
    GEpoch ep  = time;
 ...
 * For years starting in 1900 and ending in 2020, the change rates should be calculated using the
 * two nearest data points For the final 5 years of model validity (2020 to 2025 for IGRF-13), the
 * Secular Variation coefficients are explicitly provided See ref [1]
 */
bool getSHCoef(
...
/** Compute the geomagnetic field components in geocentric local reference system at given time and
 * position See ref [1] [2] Note that the normalisation of associated Legendre functions for
 * Geomagnetic field is different from that for Geogravitational field, i.e. Pnm_gra_norm(x) =
 * sqrt((2-dm0)*(2*n+1) *	(n-m)/(n+m)) * Pnm(x) Pnm_mag_norm(x) = sqrt((2-dm0)*
 * (n-m)/(n+m)) * Pnm(x) where dm0 is the Kronecker delta which is equal to 1 for m==0 and 0 for
 * m!=0 Thus we have Pnm_gra_norm(x) = sqrt(2*n+1) * Pnm_mag_norm(x) See ref [3] [4]
 */
...
)
{
    ...
    for (int n = 1; n <= igrfMF.maxDegree; n++)
    {
        ...
        double factor =
            1 / sqrt(2 * n + 1);  // factor converting Pnm_gra_norm(x) to Pnm_mag_norm(x)
        for (int m = 0; m <= n; m++)
        {
            dVtheta += leg.dPnm(n, m) *
                       (igrfMF.gnm(n, m) * cosmPhi(m) + igrfMF.hnm(n, m) * sinmPhi(m));  // lat
            dVphi += m * leg.Pnm(n, m) *
                     (igrfMF.hnm(n, m) * cosmPhi(m) - igrfMF.gnm(n, m) * sinmPhi(m));    // lon
            dVr += leg.Pnm(n, m) *
                   (igrfMF.gnm(n, m) * cosmPhi(m) + igrfMF.hnm(n, m) * sinmPhi(m));      // r
        }
