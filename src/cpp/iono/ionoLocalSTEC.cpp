...
{
    if (recLatDeg != currRecLatDeg || recLonDeg != currRecLonDeg)
    {
        double acum = 0;
        ...
        for (auto& [ind, basis] : localBasisVec)
        {
            ...
            if (dlat > atmReg.intLatDeg || atmReg.intLatDeg == 0)
                continue;
            if (dlon > atmReg.intLonDeg || atmReg.intLonDeg == 0)
                continue;

            coefMap[basis.regionID][basis.index] =
                (1 - dlat / atmReg.intLatDeg) * (1 - dlon / atmReg.intLonDeg);
...
int configIonModelLocal_(Trace& trace)
{
    ...
    for (auto& [Sat, satNav] : nav.satNavMap)
        ...
        {
            if (atmReg.gridType >= 0 && atmReg.ionoGrid)
...
)
{
    ...
    {
        if (ionPP.latDeg() < atmReg.minLatDeg)
            continue;
        if (ionPP.latDeg() > atmReg.maxLatDeg)
            continue;
        ...
        if (recLonDeg > atmReg.maxLonDeg)
            continue;
        if (recLonDeg < atmReg.minLonDeg)
            continue;
...
{
    if (atmReg.maxLatDeg <= atmReg.minLatDeg)
        return 0;
    if (atmReg.maxLonDeg <= atmReg.minLonDeg)
        return 0;
    if (latDeg > atmReg.maxLatDeg)
        return 0;
    if (latDeg < atmReg.minLatDeg)
        return 0;
    ...
    switch (ind)
    {
        ...
        case 2:
            return 2 * londiff;
        case 3:
            return 4 * latdiff * londiff;
        case 4:
            return 3 * latdiff * latdiff;
        case 5:
            return 3 * londiff * londiff;
        default:
            return 0;
    }
}
...
)
{
    ...
    switch (basis.type)
    {
        ...
        case E_BasisType::GRIDPOINT:
        {
            ...
            if (dlatDeg > atmReg.intLatDeg || atmReg.intLatDeg == 0)
                return 0;
            ...
            tracepdeex(
                ...
                trace,
                "\nGridded basis: %.4f, %.4f",
                atmReg.gridLatDeg[basis.index],
                atmReg.gridLonDeg[basis.index]
            );
            ...
            case E_BasisType::POLYNOMIAL:
            {
                ...
                switch (basis.index)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        fact = latFact * lonFact / 4;
                        break;
                    case 4:
                        fact = latFact * latFact / 3;
                        break;
                    case 5:
                        fact = lonFact * lonFact / 3;
                        break;
                    default:
                        var = 1e6;
                        break;
...
)
{
    ...
    for (auto& [regID, regData] : ssrAtm.atmosRegionsMap)
    {
        ...
        for (auto it = satData.begin(); it != satData.end();)
        {
            ...
            {
                if (tAtm == GTime::noTime() || abs(dt) < abs((time - tAtm).to_double()))
                    tAtm = tim;
                it++;
            }
        }
        ...
        if (stecData.grid.size() <= 1)  // No grid Map data
        {
            if (stecData.poly.size() <= 0)
                continue;
            if (pos.latDeg() > regData.maxLatDeg)
                continue;
            if (pos.latDeg() > regData.minLatDeg)
                continue;
            if (recLonDeg > regData.maxLonDeg)
                continue;
            if (recLonDeg < regData.minLonDeg)
                continue;
