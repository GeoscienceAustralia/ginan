...
void ionosphereSsrUpdate(Trace& trace, KFState& kfState)
{
    ...
    {
        case E_IonoModel::SPHERICAL_HARMONICS:
            return ionOutputSphcal(trace, kfState);
        case E_IonoModel::LOCAL:
            return ionOutputLocal(trace, kfState);
        case E_IonoModel::MEAS_OUT:
            return;
        case E_IonoModel::NONE:
            return;
    }
...
bool configIonModel(Trace& trace)
{
    switch (acsConfig.ionModelOpts.model)
    {
        case E_IonoModel::MEAS_OUT:
            return true;
        case E_IonoModel::SPHERICAL_HARMONICS:
            return configIonModelSphhar(trace);
        case E_IonoModel::SPHERICAL_CAPS:
            return configIonModelSphcap(trace);
        case E_IonoModel::BSPLINE:
            return configIonModelBsplin(trace);
        case E_IonoModel::LOCAL:
            return configIonModelLocal_(trace);
        default:
            return false;
    }
}
...
double ionModelCoef(Trace& trace, int ind, IonoObs& obs, bool slant)
{
    ...
    {
        case E_IonoModel::SPHERICAL_HARMONICS:
            return ionCoefSphhar(trace, ind, obs, slant);
        case E_IonoModel::SPHERICAL_CAPS:
            return ionCoefSphcap(trace, ind, obs, slant);
        case E_IonoModel::BSPLINE:
            return ionCoefBsplin(trace, ind, obs, slant);
        case E_IonoModel::LOCAL:
            return ionCoefLocal(trace, ind, obs);
        default:
            return 0;
    }
}
...
)
{
    ...
    for (auto& [id, rec] : receiverMap)
    {
        ...
        for (auto& [sys, nsat] : satcnt)
        {
            ...
            if (rec.id == acsConfig.pivot_receiver)
                nsat = 999;
    ...
    if (acsConfig.ionModelOpts.model == E_IonoModel::LOCAL)
    {
        ...
        {
            if (regData.ionoGrid)
                nStateTot += nSatTot * (regData.gridLatDeg.size() - 1);
            else if (regData.ionoPolySize > 0)
                nStateTot += nSatTot * (regData.ionoPolySize - 1);
        }
    }
    ...
    if (nMeasTot < nStateTot)
    {
        tracepdeex(
            2,
            trace,
            "#IONO_MOD Not enough Measurements %5d < %4d;  %3d, %3d\n",
            nMeasTot,
            nStateTot,
            nSatTot,
            nRecTot
    ...
    for (auto& [id, rec] : receiverMap)
        for (auto& obs : only<GObs>(rec.obsList))
        {
            ...
            if (stationList[rec.id][sys] < MIN_NSAT_REC)
            {
                continue;
            }
            ...
            /************ receiver DCB ************/ /* We may need to change this for multi-code
                                                        solutions */
            SatSys sat0;
            sat0.sys = sys;
            sat0.prn = 0;
            ...
            for (int i = 0; i < acsConfig.ionModelOpts.numBasis; i++)
            {
                ...
                tracepdeex(
                    4,
                    trace,
                    "#IONO_MOD %s %4d %9.5f %10.5f %8.5f %8.5f %12.5e %9.5f %12.5e\n",
    ...
    // combine the measurement list into a single design matrix, measurement vector, and measurement
    // noise vector
...
)
{
    ...
    if (getCmpSSRIono(trace, time, nav.ssrAtm, rRec, ionoDelay, variance, Sat))
        return ionoDelay;
    if (getIGSSSRIono(trace, time, nav.ssrAtm, rRec, azel, ionoDelay, variance))
        return ionoDelay;
