...
shar_valid				time validity of a rotation matrix (the rotation matrix will chase the sun
position) Sph_Basis_list			List of ionosphere basis time:  		I 		time of
observations (to update the rotation matrix) IPP: 			I 		Ionospheric piercing point to be
updated Since the spherical harmonic model has global validity, the check always return 1
-----------------------------------------------------
Author: Ken Harima @ RMIT 29 July 2020
-----------------------------------------------------*/
int configIonModelSphhar(Trace& trace)
{
    if (acsConfig.ionModelOpts.function_degree == 0)
        acsConfig.ionModelOpts.function_degree = acsConfig.ionModelOpts.function_order;
    else if (acsConfig.ionModelOpts.function_order > acsConfig.ionModelOpts.function_degree)
        acsConfig.ionModelOpts.function_order = acsConfig.ionModelOpts.function_degree;
...
bool ippCheckSphhar(GTime time, VectorPos& ionPP)
{
    ...
    if (acsConfig.ionModelOpts.use_rotation_mtx)
    {
        ...
        if (sphTime == GTime::noTime() || fabs((time - sphTime).to_double()) > sphValid)
        {
            VectorEcef rSun;
            ...
            sphRotMatrix = Eigen::AngleAxisd(lon, Vector3d::UnitZ()) *
                           Eigen::AngleAxisd(-lat, Vector3d::UnitY());
...
----------------------------------------------------------------------------*/
double
ionVtecSphhar(Trace& trace, GTime time, VectorPos& ionPP, int layer, double& var, KFState& kfState)
{
...
)
{
    ...
    if (sphBasisIndexMaps.size() < atmGlob.layers.size())
    {
        ...
        {
            if (maxdeg < atmLay.maxDegree)
                maxdeg = atmLay.maxDegree;
            if (maxord < atmLay.maxOrder)
                maxord = atmLay.maxOrder;
    ...
    for (auto& [layer, atmLay] : atmGlob.layers)
    {
        ...
        {
            int reindex = sphBasisIndexMaps[harmonic.layer][harmonic.degree][harmonic.order]
                                           [harmonic.trigType];
