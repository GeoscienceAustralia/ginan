...
bool ippInRange(GTime time, VectorPos& ionPP)
{
    switch (acsConfig.ionModelOpts.model)
    {
        ...
        case E_IonoModel::SPHERICAL_HARMONICS:
            return ippCheckSphhar(time, ionPP);
        case E_IonoModel::SPHERICAL_CAPS:
            return ippCheckSphcap(time, ionPP);
        case E_IonoModel::BSPLINE:
            return ippCheckBsplin(time, ionPP);
        case E_IonoModel::LOCAL:
            return ippCheckLocal(time, ionPP);
...
void obsIonoData(Trace& trace, Receiver& rec)
{
    ...
    tracepdeex(
        4,
        trace,
        "ION_MEAS sat    tow     RawGF meas  RawGF std.  GF_code_mea  GF_phas_mea  GF to TECu\n"
    );
    ...
    for (auto& obs : only<GObs>(rec.obsList))
        if (obs.satNav_ptr)
            if (obs.satStat_ptr)
            {
                ...
                if (satStat.el < recOpts.elevation_mask_deg * D2R || obs.exclude)
                {
                ...
                obs.stecToDelay =
                    STEC2DELAY * (SQR(satNav.lamMap[frq2]) - SQR(satNav.lamMap[frq1]));
                ...
                {
                    obs.ippMap[j].slantFactor = ionppp(
                        pos,
                        satStat,
                        RE_WGS84 / 1000,
                    ...
                    tracepdeex(
                        5,
                        trace,
                        "IPP_verif  %s   %8.3f %8.3f   %8.3f %8.3f ",
                        obs.Sat.id().c_str(),
                        pos.latDeg(),
                        pos.lonDeg(),
                        posp.latDeg(),
                ...
                if (fabs(lc.GF_Phas_m - lc_pre.GF_Phas_m) > 0.05 /* Basic cycle slip detection */
                    || fabs(lc.GF_Phas_m - oldSTEC) > 0.05 || satStat.ambvar <= 0)
                {
                    satStat.gf_amb = amb;
                ...
                obs.stecCodeCombo = static_cast<int>(obs.sigs[frq1].code) * 100 +
                                    static_cast<int>(obs.sigs[frq2].code);
                ...
                tracepdeex(
                    4,
                    trace,
                    "ION_MEAS %s %8.0f  %10.4f  %10.3e  %10.4f  %10.4f  %10.4f\n",
                    obs.Sat.id().c_str(),
                    tow,
                    obs.stecVal,
                    obs.stecVar,
...
void writeIonStec(string filename, KFState& kfState)
{
    ...
    tracepdeex(
        0,
        stecfile,
        "#TYP_MEA,%4s,%10s,%4s,%3s,%10s,%10s,%1s,%1s,%13s,%13s,%13s,%4s,%13s,%13s,%13s",
        "week",
        "tow",
        "site",
        "sat",
    ...
    {
        tracepdeex(0, stecfile, ",%8s,%8s,%8s,%8s", "height", "ipplat", "ipplon", "slant_f");
    }
    ...
    for (auto& [key, index] : kfState.kfIndexMap)
    {
        ...
        tracepdeex(
            0,
            stecfile,
            "IONO_MEA,%4d,%10.3f,%4s,%3s,%10.4f,%10.4e",
            week,
            tow,
            key.str.c_str(),
            key.Sat.id().c_str(),
