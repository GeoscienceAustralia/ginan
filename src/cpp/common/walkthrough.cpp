 ...
 * The basic structure for kalman filtering in ginan is the KFState struct, it is required for all
 * filtering. The KFKey struct is used to define a state element within the filter. These are used
 * rather than row/column indexing to eliminate manual bookkeeping State elements are generally
 * initialised using an InitialState object, which defines the initial value, sigma, and process
 * noise to be applied between epochs. Measurements are defined individually as KFMeasEntry objects
 * and added to a list, before being combined to create a KFMeas object that contains the aggregated
 * measurement list in matrix form. The measurement's design matrix is set by defining the
 * sensitivity of the measurement to state elements, and its innovation (OMC) and noise are set with
 * helper functions. State elements are automatically added to the filter when a stateTransition
...
KFState walkthrough1()
{
    ...
    // prepare the filter
    KFState kfState;
    kfState.output_residuals =
        true;  // turn on nice outputs for pre/postfit residuals when filtering
    ...
    for (int epoch = 1; epoch <= 2; epoch++)
    {
        ...
        if (epoch == 1)
            printWait(noteAddMeas);
        {
            // get the computed (usually just current) state
            double stateVal =
                posInit.x;  // inital computed state is whatever we are going to initialise it as
            kfState.getKFValue(
                posKey,
                stateVal
            );  // override that with whatever is in the kalman filter, if there is something there,
                // that is.

            double computed = stateVal;
            double observed =
                actualPosition +
                rando(randoDev);  // the measurement is the actual position +- some error noise
            ...
            measEntry.addDsgnEntry(
                ...
                1,
                posInit
            );  // this measEntry is a direct measurement of the state with key posKey ->
                // sensitivity is 1, create and initialise the state with posInit if required.
            measEntry.setInnov(omc);
            measEntry.setNoise(measVar);

            // this is optional, but makes the output easier to read. Usually this would be passed
            // in through KFMeasEntry's constructor
            if (1)
            {
                KFKey obsKey;
                obsKey.str       = "Meas";
...

        {
            ...
            double computed = stateVal / 2.54;  // this measurement is in inches, this time the
                                                // 'computation' is needed before use.
            double observed =
                actualPosition /
                2.54;  // the measurement is the actual position if it were measured in inches.
            ...
            measEntry.addDsgnEntry(
                ...
                1 / 2.54,
                posInit
            );  // this measEntry is a scaled measurement of the state with key posKey ->
                // sensitivity of measuremet to change in state is 1/2.54, create and initialise the
                // state with posInit if required.
            measEntry.setInnov(omc);  // error between the current state value, and what we think it
                                      // should be (observed minus computed)
            measEntry.setNoise(measVar
            );  // how much variance we expect in the measurement - our rulers are not perfect
        ...
        kfState.stateTransition(
            std::cout,
            time
        );  // the state transition to t = time initialises newly created states, and adds process
            // noise to existing ones as per their initialisation

        kfState.outputStates(std::cout, " - POST STATE TRANSITION");
        if (epoch == 1)
            printWait(noteInit);
        if (epoch == 2)
            printWait(noteProcNoise);

        KFMeas kfMeas(
            kfState,
            kfMeasEntryList,
            time
        );  // filterKalman requires a consolidated KFMeas input, rather
            // than a list of individial KFMeasEntrys, create it here
        ...
        kfState.filterKalman(
            std::cout,
            kfMeas,
            "",
            true
        );  // perform the kalman filter operation (the true indicates this is an extended kalman
            // filter using precalculated OMCs, this should generally be true in ginan)
        if (epoch == 1)
            printWait(noteResiduals);
        if (epoch == 2)
            printWait(noteReject);

        kfState.outputStates(std::cout, " - POST FILTER");
        if (epoch == 1)
            printWait(notePostFilter1);
    }
...
void walkthrough2(KFState& kfState)
{
    // unrequired preparation
    double                           measVar = 1;
    std::random_device               randoDev;
    ...
    posInit.P = 10000;     // large P values indicate large uncertainty in initial position
                        // initial x value defaults to 0 if undefined, substantially different from
                        // actual initial values defined above
    ...
    for (double t = 0; t <= 2; t += dt)
    {
        double actualPosition =
            x0  // compute positions and velocities of a simulated ball in gravity
            + v0 * t + a * t * t / 2;

        double actualVelocity = v0 + a * t;

        std::cout << "\n"
                  << "\n\n\n\nEpoch " << "\n t = " << t << "\n x = " << actualPosition
                  << "\n v = " << actualVelocity << "\n";
        time += dt;  // generally we wouldnt be iterating over t, dt like this, we would be given a
                     // time from some external observation source and we'd just use it
        ...
        kfState.stateTransition(
            std::cout,
            time
        );  // since we're using rates we need to compute the current value of the states since they
            // have changed over the period since the stateTransition at the end of this loop was
            // last called, (at t-dt) call stateTransition to do this computation
        ...
        if (t == 0)
            printWait(
                noteAddMeas +
                "\nAlso note that the current true value of the x,v,t at each epoch "
                "will be shown as above\n\t"
            );

        double observed = actualPosition;  // the measurement is the actual position

        // create three types of measurements for the same object, with more or less kinematics
        // involved
        for (int i = 1; i <= 3; i++)
        {
            posKey.num = i;
            ...
            else if (i == 2)
            {
                posInit.Q = 0;
                velInit.Q = SQR(10);
            }  // The Q value (process noise) should be 0 for elements that have rates applied - eg,
               // for num==2 a vel term is the derivative of pos, so posInit.Q = 0
            else if (i == 3)
            {
                posInit.Q = 0;
                velInit.Q = 0;
            }  // only the highest derivative should get process noise.
            ...
            KFKey obsKey;  // give the measEntry a KFKey just so the residuals/measurements outputs
                           // are easier to read (optional)
            obsKey.str = "MEAS";
            obsKey.num = i;
            ...
            if (i >= 2)
                kfState.setKFTransRate(
                    posKey,
                    velKey,
                    1,
                    velInit
                );  // here we define that velKey is the rate derivative of posKey, with scaling 1,
                    // and some initial velocity state

            if (i == 3)
                kfState.setAccelerator(
                    posKey,
                    velKey,
                    accKey,
                    1,
                    accInit
                );  // here we define that accKey is an accelerator of posKey, with velKey being an
                    // intermediate derivative between that and this.
        ...
        kfState.stateTransition(
            std::cout,
            time
        );  // since we called stateTransition with t = time above, this will only initialise new
            // states - no time has passed to change state values or process noises
        ...
        kfState.outputStates(std::cout, " - POST FILTER");
        if (t == 0)
            printWait(noteFirstMeas);
        if (t == dt)
            printWait(noteSecondMeas);
        if (t == dt * 2)
            printWait(noteThirdMeas);
        if (t > 1)
            printWait(note1Second);
    }
