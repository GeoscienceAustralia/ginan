...
void NtripUploader::serverResponse(unsigned int status_code, string http_version)
{
...
void NtripUploader::messageTimeoutHandler(const boost::system::error_code& err)
{
    ...
    // fire this callback again in the future
    {
        ...
        );  // check uploader twice a second to account for aliasing
    ...
    if (streamConfig.itrf_datum)
        ssrMeta.referenceDatum = 0;  // Orbit corrections, 0 - ITRF, 1 - Regional.
    else
        ssrMeta.referenceDatum = 1;
    ...
    for (auto [messCode, msgOpts] : streamConfig.rtcmMsgOptsMap)
    {
        ...
        if (updateInterval == 0 || ((long int)targetTime) % updateInterval != 0)
        {
            continue;
        }
        ...
        switch (messCode)
        {
            ...
            case RtcmMessageType::COMPACT_SSR:
            {
                ...
                if (lastSubType == CompactSSRSubtype::GRD ||
                    lastSubType == CompactSSRSubtype::TEC || lastSubType == CompactSSRSubtype::ATM)
    ...
    if (length != 0)
    {
        ...
        if (url.protocol == "https")
        {
            boost::asio::async_write(
                *_sslsocket,
                outMessages,
                boost::bind(&NtripUploader::writeHandler, this, bp::error)
            );
        }
        else
        {
...
void NtripUploader::connected()
{
    ...
    // Although there should be no downloading attempting to download monitors the socket
    // connection.
    startRead(true);
