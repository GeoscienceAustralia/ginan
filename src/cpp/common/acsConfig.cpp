 ...
 * The Pea uses one or more yaml files as its source of configuration. YAML files are simple
 heirarchical documents which are straightforward to edit in basic text editors.
 *
 *
 * The configuration parser performs many functions simultaneously, which may lead to issues if
 modifications are made without due care.
 * In most cases, finding a similar configuration and duplicating all references to it will be the
 most straightforward method of creating new parameters.
 *
 * The lines in this file can become very long, because they do perform so many functions at once,
 however they should be aligned such that columns of similar parameters form naturally on the
 screen.
 * This is preferable to more standard formatting because of the vast amount of duplication that
 would be required otherwise. The block-select tool in the IDE will come in handy.
 * There are cases where artificial scoping is used to allow dummy variables to be used such that
 rather than copying one variable name into multiple function calls, which is always a source of
 bugs,
 * a single common parameter name may be used across many unrelated lines. `thing` is one such
 variable name.
 *
...
/** Some configuration structures are possible to be configured specific for eg a receiver or signal
 * or combination of both.
 * *
 * Rather than creating multitudes of 'initialised' variables to correspond with config parameters,
 * a map and pointer algebra is used to keep track of such things. Typically, the base container,
 * the member variable, and a boolean to signify initialisation are included in a parsing line.
 *
 * Inheritance is performed by using the overloaded += operator on an object, with any
 * configurations that have been initialised in the secondary object overwriting those values in the
 * first.
 */
...
/**
 *
 * Each configuration parameter is defined with a default initialisation value which is most
 * sensible for the majority of use-cases. Parameters which are not assigned using the config will
 * default to those values, allowing the size of yaml files to be minimised.
 *
...
void getUserAliases(vector<string>& aliasPairs)
{
    userAliases.clear();
...
struct Indentor
{
    ...
    Indentor(char c = ' ', int width = 4) : width{width}, indent{c} {}
    ...
    operator string() const { return string(indentation, indent); }
...
/** Helper object to temporarily and automatically disable and reenable a stream when the object
 * goes out of scope
 */
struct TempStreamDisabler
{
    ...
    TempStreamDisabler(std::ostream& stream) : stream{stream} {}
...
{
    // keep going until the end of the file, or this function returns due to the next iterator not
    // being a sibling
    while (it != acsConfig.yamlDefaults.end())
    {
        ...
        // do this one, and bump the iterator
        {
            ...
            while ((pos_end = stack.find(":", pos_start)) != string::npos)
            {
                ...
                if (cutstr.find('!') != string::npos)
                    optionLevel = 1;
                else if (cutstr.find('@') != string::npos)
                    optionLevel = 2;
                else if (cutstr.find('#') != string::npos)
                    optionLevel = 3;
            ...
            // output the boilerplate of the name, and comment up to the point where the children
            // are nested
            tracepdeex(
                0,
                std::cout,
                "\n%s%s\t%-30s",
                ((string)indentor).c_str(),
                token.c_str(),
                (defaultVal).c_str()
            );
            ...
            html << "\n"
                 ...
                 << (nextIsChild ? "<b>" : "") << token << (nextIsChild ? " ⯆</b>" : "");
            ...
            if (defaults.enumName.empty() == false)
            {
                ...
                if (defaultVal.find('[') == string::npos)
                    link += "[`" + linkName + "`]";
                else
                    link += "[`[" + linkName + "]`]";
            ...
            {
                // this has no children, output the default value of this parameter instead -
                // according to its commented parameter type
                ...
                for (auto once : {1})
                {
                    ...
                    if (type == typeid(bool).name())
                    {
                        ...
                        html << "\n"
                             << htmlIndentor << "<option value='true' "
                             << (defaultVal == "true" ? " selected" : "") << ">true</option>";
                        html << "\n"
                             << htmlIndentor << "<option value='false' "
                             << (defaultVal == "false" ? " selected" : "") << ">false</option>";
                    ...
                    {
                        html << "\n"
                             << htmlIndentor << "<input type='text' class='value' value='"
                             << defaultVal << "'>";
                    }
                }
            }
 ...
 * A configurator is generated that can be used to edit the default configuration via interactive
 * html scripts
 */
void ACSConfig::outputDefaultConfiguration(int level)
{
    ...
    md << "\n"
       << "This document outlines the major configuration options available in ginan that are most "
          "applicable to "
          "general users. "
       << "For more advanced configuration options and their defaults, use the `-Y <level>` option "
          "at the command line "
          "to view increasing levels of advanced configurations.";
    ...
    std::cout << "An interactive configuration inspector has been generated and saved to "
                 "GinanYamlInspector.html"
              << "\n";
 ...
 */
void ACSConfig::info(Trace& s)  ///< Trace file to output to
{
    std::stringstream ss;
    ...
    {
        ss << "\tigrf_files:                      ";
        for (auto& a : igrf_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!ocean_tide_loading_blq_files.empty())
    {
        ss << "\tocean_tide_loading_blq_files:    ";
        for (auto& a : ocean_tide_loading_blq_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!atmos_tide_loading_blq_files.empty())
    {
        ss << "\tatmos_tide_loading_blq_files:    ";
        for (auto& a : atmos_tide_loading_blq_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!ocean_pole_tide_loading_files.empty())
    {
        ss << "\tocean_pole_tide_loading_files:   ";
        for (auto& a : ocean_pole_tide_loading_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!pseudo_filter_files.empty())
    {
        ss << "\tpseudo_filter_files:             ";
        for (auto& a : pseudo_filter_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!erp_files.empty())
    ...
    {
        ss << "\tegm_files:                       ";
        for (auto& a : egm_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!planetary_ephemeris_files.empty())
    {
        ss << "\tplanetary_ephemeris_files:       ";
        for (auto& a : planetary_ephemeris_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!ocean_tide_potential_files.empty())
    {
        ss << "\tocean_tide_potential_files:      ";
        for (auto& a : ocean_tide_potential_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!atmos_tide_potential_files.empty())
    {
        ss << "\tatmos_tide_potential_files:      ";
        for (auto& a : atmos_tide_potential_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!cmc_files.empty())
    {
        ss << "\tcmc_files:                       ";
        for (auto& a : cmc_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!hfeop_files.empty())
    {
        ss << "\thfeop_files:                     ";
        for (auto& a : hfeop_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!atmos_ocean_dealiasing_files.empty())
    {
        ss << "\tatmos_oceean_dealiasing_files:   ";
        for (auto& a : atmos_ocean_dealiasing_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!ocean_pole_tide_potential_files.empty())
    {
        ss << "\tocean_pole_tide_potential_files: ";
        for (auto& a : ocean_pole_tide_potential_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!space_weather_files.empty())
    {
        ss << "\tspace_weather_files:			  ";
        for (auto& a : space_weather_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!sid_files.empty())
    ...
    {
        ss << "\tcom_files:                       ";
        for (auto& a : com_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!orography_files.empty())
    {
        ss << "\torography_files:                 ";
        for (auto& a : orography_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!gpt2grid_files.empty())
    {
        ss << "\tgpt2grid_files:                  ";
        for (auto& a : gpt2grid_files)
            ss << a << " ";
        ss << "\n";
    }
    if (!nav_rtcm_inputs.empty())
    {
        ss << "\trtcm_inputs:                     ";
        for (auto& a : nav_rtcm_inputs)
            ss << a << " ";
        ss << "\n";
    }
    if (!qzs_rtcm_inputs.empty())
    {
        ss << "\tqzl6_inputs:                     ";
        for (auto& a : qzs_rtcm_inputs)
            ss << a << " ";
        ss << "\n";
    }
    if (!sisnet_inputs.empty())
    {
        ss << "\tsisnet_inputs:                   ";
        for (auto& a : sisnet_inputs)
            ss << a << " ";
        ss << "\n";
    }
    if (!rnx_inputs.empty())
    {
        ss << "\trnx_inputs:                      ";
        for (auto& [z, A] : rnx_inputs)
            for (auto& a : A)
                ss << a << " ";
        ss << "\n";
    }
    if (!pseudo_sp3_inputs.empty())
    {
        ss << "\tsp3_inputs:                      ";
        for (auto& [z, A] : pseudo_sp3_inputs)
            for (auto& a : A)
                ss << a << " ";
        ss << "\n";
    }
    if (!pseudo_snx_inputs.empty())
    {
        ss << "\tsnx_inputs:                      ";
        for (auto& [z, A] : pseudo_snx_inputs)
            for (auto& a : A)
                ss << a << " ";
        ss << "\n";
    }
    if (!obs_rtcm_inputs.empty())
    {
        ss << "\trtcm_inputs:                     ";
        for (auto& [z, A] : obs_rtcm_inputs)
            for (auto& a : A)
                ss << a << " ";
        ss << "\n";
    }
    ...
    if (1)
    {
        ss << "\ttrace level:                   " << trace_level << "\n";
    }
    if (output_satellite_trace)
    {
        ss << "\tsatellite trace filename:      " << satellite_trace_filename << "\n";
    }
    if (output_receiver_trace)
    {
        ss << "\treceiver trace filename:       " << receiver_trace_filename << "\n";
    }
    if (output_json_trace)
    {
        ss << "\tjson trace filename:           " << receiver_json_filename << "\n";
    }
    if (output_network_trace)
    {
        ss << "\tnetwork trace filename:        " << network_trace_filename << "\n";
    }
    if (output_ionosphere_trace)
    {
        ss << "\tionosphere trace filename:     " << ionosphere_trace_filename << "\n";
    }
    if (output_clocks)
    {
        ss << "\tclocks filename:               " << clocks_filename << "\n";
    }
    if (output_ionex)
    {
        ss << "\tionex filename:                " << ionex_filename << "\n";
    }
    if (output_sinex)
    {
        ss << "\tsinex filename:                " << sinex_filename << "\n";
    }
    if (output_ionstec)
    {
    ...
    if (output_cost)
    {
        ss << "\tcost filename:                 " << cost_filename << "\n";
    }
    if (output_trop_sinex)
    {
        ss << "\ttrop sinex filename:           " << trop_sinex_filename << "\n";
    }
    if (output_gpx)
    {
        ss << "\tgpx filename:                  " << gpx_filename << "\n";
    }
    if (output_pos)
    {
    ...
    if (output_sp3)
    {
        ss << "\tsp3 filename:                  " << sp3_filename << "\n";
    }
    if (output_decoded_rtcm_json)
    {
        ss << "\tdecoded rtcm json filename:    " << decoded_rtcm_json_filename << "\n";
    }
    if (output_encoded_rtcm_json)
    {
        ss << "\tencoded rtcm json filename:    " << encoded_rtcm_json_filename << "\n";
    }
    if (output_sbas_ems)
    {
        ss << "\tSBAS EMS filename:             " << ems_filename << "\n";
    }
    ...
    if (epoch_interval > 0)
    {
        ss << "\tepoch_interval: " << epoch_interval << "\n";
    }
    if (max_epochs > 0)
    {
        ss << "\tmax_epochs:     " << max_epochs << "\n";
    }
    if (!start_epoch.is_not_a_date_time())
    {
        ss << "\tepoch start:    " << start_epoch << "\n";
    }
    if (!end_epoch.is_not_a_date_time())
    {
        ss << "\tepoch end:      " << end_epoch << "\n";
    }
...
)
{
    ...
    // this function is fiddly - re-ordering or simplifying will likely lead to default
    // configuration output issues
    ...
    for (int i = 0; i < yamlNodeDescriptor.size(); i++)
    {
        ...
        if (acsConfig.yamlDefaults.find(stack) == acsConfig.yamlDefaults.end())
        {
            if (i == yamlNodeDescriptor.size() - 1)
                acsConfig.yamlDefaults[stack] = {defaultValue, comment, type};
            else
                acsConfig.yamlDefaults[stack] = {"", ""};
            acsConfig.yamlDefaults[stack].found = test;
        }

        // override comment if it hasnt been set yet
        if (i == yamlNodeDescriptor.size() - 1 && acsConfig.yamlDefaults[stack].comment.empty() &&
            comment.empty() == false)
        {
...
{
    auto [optNode, stack] = stringsToYamlObject(
        yamlBase,
        yamlNodeDescriptor,
        comment,
        stringify(output),
    ...
    try
    {
        ...
        {
            BOOST_LOG_TRIVIAL(debug) << "Yaml entry " << nonNumericStack(stack, dummy)
                                     << " is configured with its default value, deleting it "
                                        "entirely would simplify the configuration file.";
        }
...
)
{
    ...
    auto [optNode, stack] = stringsToYamlObject(
        ...
        yamlNodeDescriptor,
        comment + enumOptions,
        stringify(enumVector)
    );  // do this twice to populate the defaults before using strings
...
)
{
...

    {
        ...
        setInited(
            output,
            thing,
            tryGetFromYaml(thing, newYaml, {"0! estimated"}, "Estimate state in kalman filter")
        );
    }
    {
        auto& thing = output.use_remote_sigma;
        setInited(
            output,
            thing,
            tryGetFromYaml(
                ...
                newYaml,
                {"4@ use_remote_sigma"},
                "Use remote filter sigma for initial sigma"
            )
        );
    }
    {
        auto& thing = output.sigma;
        setInited(
            output,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = output.apriori_value;
        setInited(
            output,
            thing,
            tryGetFromYaml(thing, newYaml, {"3! apriori_value"}, "Apriori state values")
        );
    }
    {
        auto& thing = output.process_noise;
        setInited(
            output,
            thing,
            tryGetFromYaml(thing, newYaml, {"2! process_noise"}, "Process noise sigmas")
        );
    }
    {
        auto& thing = output.sigma_limit;
        setInited(
            output,
            thing,
            tryGetFromYaml(
                ...
                newYaml,
                {"@ sigma_limit"},
                "Maximum sigma before the state is removed"
            )
        );
    }
    {
        auto& thing = output.outage_limit;
        setInited(
            output,
            thing,
            tryGetFromYaml(
                ...
                newYaml,
                {"@ outage_limit"},
                "Maximum unestimated time before the state is removed"
            )
        );
    }
    {
        auto& thing = output.tau;
        setInited(
            output,
            thing,
            tryGetFromYaml(
                ...
                newYaml,
                {"@ tau"},
                "Correlation times for gauss markov noise, defaults to -1 -> inf (Random Walk)"
            )
        );
    }
    {
        auto& thing = output.mu;
        setInited(
            output,
            thing,
            tryGetFromYaml(thing, newYaml, {"@ mu"}, "Desired mean value for gauss markov states")
        );
    }
    {
        auto& thing = output.comment;
        setInited(
            output,
            thing,
            tryGetFromYaml(thing, newYaml, {"@ comment"}, "Comment to apply to the state")
        );
    }
    {
...
)
{
    ...
    for (auto& value : optsList)
    {
        ...
        if (std::find(mappedList["<AUTO>"].begin(), mappedList["<AUTO>"].end(), value) !=
            mappedList["<AUTO>"].end())
        {
            continue;
        }
...
)
{
    ...
    tryGetKalmanFromYaml(
        empOpts.emp_d_0,
        empNode,
        "6@ emp_d_0",
        "Empirical accleration direct bias "
    );
    tryGetKalmanFromYaml(
        empOpts.emp_d_1,
        empNode,
        "6@ emp_d_1",
        "Empirical accleration direct 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_d_2,
        empNode,
        "6@ emp_d_2",
        "Empirical accleration direct 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_d_3,
        empNode,
        "6@ emp_d_3",
        "Empirical accleration direct 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_d_4,
        empNode,
        "6@ emp_d_4",
        "Empirical accleration direct 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_y_1",
        "Empirical accleration Y 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_y_2,
        empNode,
        "6@ emp_y_2",
        "Empirical accleration Y 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_y_3,
        empNode,
        "6@ emp_y_3",
        "Empirical accleration Y 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_y_4,
        empNode,
        "6@ emp_y_4",
        "Empirical accleration Y 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_b_1",
        "Empirical accleration B 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_b_2,
        empNode,
        "6@ emp_b_2",
        "Empirical accleration B 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_b_3,
        empNode,
        "6@ emp_b_3",
        "Empirical accleration B 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_b_4,
        empNode,
        "6@ emp_b_4",
        "Empirical accleration B 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_r_0",
        "Empirical accleration radial bias "
    );
    tryGetKalmanFromYaml(
        empOpts.emp_r_1,
        empNode,
        "6@ emp_r_1",
        "Empirical accleration radial 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_r_2,
        empNode,
        "6@ emp_r_2",
        "Empirical accleration radial 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_r_3,
        empNode,
        "6@ emp_r_3",
        "Empirical accleration radial 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_r_4,
        empNode,
        "6@ emp_r_4",
        "Empirical accleration radial 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_t_0",
        "Empirical accleration tangential bias "
    );
    tryGetKalmanFromYaml(
        empOpts.emp_t_1,
        empNode,
        "6@ emp_t_1",
        "Empirical accleration tangential 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_t_2,
        empNode,
        "6@ emp_t_2",
        "Empirical accleration tangential 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_t_3,
        empNode,
        "6@ emp_t_3",
        "Empirical accleration tangential 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_t_4,
        empNode,
        "6@ emp_t_4",
        "Empirical accleration tangential 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_n_0",
        "Empirical accleration normal bias "
    );
    tryGetKalmanFromYaml(
        empOpts.emp_n_1,
        empNode,
        "6@ emp_n_1",
        "Empirical accleration normal 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_n_2,
        empNode,
        "6@ emp_n_2",
        "Empirical accleration normal 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_n_3,
        empNode,
        "6@ emp_n_3",
        "Empirical accleration normal 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_n_4,
        empNode,
        "6@ emp_n_4",
        "Empirical accleration normal 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_p_1",
        "Empirical accleration P 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_p_2,
        empNode,
        "6@ emp_p_2",
        "Empirical accleration P 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_p_3,
        empNode,
        "6@ emp_p_3",
        "Empirical accleration P 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_p_4,
        empNode,
        "6@ emp_p_4",
        "Empirical accleration P 4 per rev"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        empNode,
        "6@ emp_q_1",
        "Empirical accleration Q 1 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_q_2,
        empNode,
        "6@ emp_q_2",
        "Empirical accleration Q 2 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_q_3,
        empNode,
        "6@ emp_q_3",
        "Empirical accleration Q 3 per rev"
    );
    tryGetKalmanFromYaml(
        empOpts.emp_q_4,
        empNode,
        "6@ emp_q_4",
        "Empirical accleration Q 4 per rev"
    );
}
...
)
{
    ...
    tryGetKalmanFromYaml(comOpts.cr, comNode, "2@ cr", "SRP coefficient");
    tryGetKalmanFromYaml(comOpts.cd, comNode, "2@ cd", "Drag coefficient");
#endif
}
...
)
{
    ...
    tryGetKalmanFromYaml(
        ...
        recNode,
        "8@ strain_rate",
        "Velocity (large gain, for geodetic timescales)"
    );
    ...
    tryGetKalmanFromYaml(
        ...
        recNode,
        "3# pcv",
        "Antenna phase center variations (experimental)"
    );
    ...
    tryGetKalmanFromYaml(
        recOpts.slr_range_bias,
        recNode,
        "9@ slr_range_bias",
        "Satellite Laser Ranging range bias"
    );
    tryGetKalmanFromYaml(
        recOpts.slr_time_bias,
        recNode,
        "9@ slr_time_bias",
        "Satellite Laser Ranging time bias"
    );
...
)
{
    ...
    auto orbitsNode = stringsToYamlObject(
        comNode,
        {"@ orbit_propagation"},
        "Enable specific orbit propagation models"
    );
    auto pseudo_pulses = stringsToYamlObject(
        orbitsNode,
        {"@ pseudo_pulses"},
        "Apply process noise to simulate pseudo-stochastic pulses commonly applied in least "
        "squares solutions"
    );
    ...
    }
    {
        ...
        setInited(
            ...
            tryGetFromYaml(
                ...
                orbitsNode,
                {"0! mass"},
                "Satellite mass for use if not specified in the SINEX metadata file"
            )
        );
    }
    {
        auto& thing = orbOpts.area;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"0! area"},
                "Satellite area for use in solar radiation and albedo calculations"
            )
        );
    }
    {
        auto& thing = orbOpts.power;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"0@ power"},
                "Transmission power use if not specified in the SINEX metadata file"
            )
        );
    }
    {
        auto& thing = orbOpts.srp_cr;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"0@ srp_cr"},
                "Coefficient of reflection of the satellite"
            )
        );
    }
    {
        auto& thing = orbOpts.drag_cd;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"0@ drag_cd"},
                "Coefficient of drag of the satellite"
            )
        );
    }
    {
        auto& thing = orbOpts.planetary_perturbations;
        setInited(
            orbOpts,
            thing,
            tryGetEnumVec(
                ...
                "Acceleration due to third celestial bodies"
            )
        );
    }
    {
        auto& thing = orbOpts.solar_radiation_pressure;
        setInited(
            orbOpts,
            thing,
            tryGetEnumOpt(
                ...
                "Model accelerations due to solar radiation pressure"
            )
        );
    }
    {
        auto& thing = orbOpts.drag;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(thing, orbitsNode, {"@ drag"}, "Model accelerations due to drag")
        );
    }
    {
        auto& thing = orbOpts.empirical;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"@ empirical"},
                "Model accelerations due to empirical accelerations"
            )
        );
    }
    {
        auto& thing = orbOpts.antenna_thrust;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                orbitsNode,
                {"@ antenna_thrust"},
                "Model accelerations due to the emitted signal from the antenna"
            )
        );
    }
    {
        auto& thing = orbOpts.albedo;
        setInited(
            orbOpts,
            thing,
            tryGetEnumOpt(
                ...
                orbitsNode,
                {"@ albedo"},
                "Model accelerations due to the albedo effect from Earth (Visible and Infra-red)"
            )
        );
    }
    {
        auto& thing = orbOpts.empirical_dyb_eclipse;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                "Turn on/off the eclipse on each axis (D, Y, B)"
            )
        );
    }
    {
        auto& thing = orbOpts.empirical_rtn_eclipse;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                "Turn on/off the eclipse on each axis (R, T, N)"
            )
        );
    }
    {
        auto& thing = orbOpts.pseudoPulses.enable;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                pseudo_pulses,
                {"@ enable"},
                "Enable applying process noise impulses to orbits upon state errors"
            )
        );
    }
    {
        auto& thing = orbOpts.pseudoPulses.interval;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                pseudo_pulses,
                {"@ interval"},
                "Interval between applying pseudo pulses"
            )
        );
    }
    {
    ...
    {
        auto& thing = orbOpts.pseudoPulses.epochs;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                pseudo_pulses,
                {"@ epochs"},
                "epochs where pseudo pulses are applied [second of day since midnight GPST]"
            )
        );
    }
    {
        auto& thing = orbOpts.pseudoPulses.pos_proc_noise;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                pseudo_pulses,
                {"@ pos_process_noise"},
                "Sigma to add to orbital position states"
            )
        );
    }
    {
        auto& thing = orbOpts.pseudoPulses.vel_proc_noise;
        setInited(
            orbOpts,
            thing,
            tryGetFromYaml(
                ...
                pseudo_pulses,
                {"@ vel_process_noise"},
                "Sigma to add to orbital velocity states"
            )
        );
    }
    ...
    auto [surfacesNode, surfacesString] = stringsToYamlObject(
        comNode,
        {"6@ surface_details"},
        "List of details for srp and drag surfaces"
    );
...
)
{
    ...
    {
        auto& thing = comOpts.exclude;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(thing, comNode, {"0! exclude"}, "Exclude receiver from processing")
        );
    }
    {
        auto& thing = comOpts.pseudo_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                comNode,
                {"0@ pseudo_sigma"},
                "Standard deviation of pseudo measurmeents"
            )
        );
    }
    {
        auto& thing = comOpts.laser_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                comNode,
                {"0@ laser_sigma"},
                "Standard deviation of SLR laser measurements"
            )
        );
    }
    {
        auto& thing = comOpts.clock_codes;
        setInited(
            comOpts,
            thing,
            tryGetEnumVec(
                ...
                comNode,
                {"3@ clock_codes"},
                "Codes for IF combination based clocks"
            )
        );
    }
    {
        auto& thing = comOpts.apriori_sigma_enu;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                comNode,
                {"4@ apriori_sigma_enu"},
                "Sigma applied for weighting in mincon transformation estimation. (Lower is "
                "stronger weighting, "
                "Negative is unweighted, ENU separation unsupported for satellites)"
            )
        );
    }
    {
        auto& thing = comOpts.mincon_scale_apriori_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                thing,
                comNode,
                {"4@ mincon_scale_apriori_sigma"},
                "Scale applied to apriori sigmas while weighting in mincon transformation "
                "estimation"
            )
        );
    }
    {
        auto& thing = comOpts.mincon_scale_filter_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                thing,
                comNode,
                {"4@ mincon_scale_filter_sigma"},
                "Scale applied to filter sigmas while weighting in mincon transformation estimation"
            )
        );
    }
    {
        auto& thing = antenna_boresight;
        tryGetFromYaml(
            thing,
            comNode,
            {"@ antenna_boresight"},
            "Antenna boresight (Up) in satellite body-fixed frame"
        );
    }
    {
        auto& thing = antenna_azimuth;
        tryGetFromYaml(
            thing,
            comNode,
            {"@ antenna_azimuth"},
            "Antenna azimuth (North) in satellite body-fixed frame"
        );
    }
    {
        auto& thing = comOpts.ellipse_propagation_time_tolerance;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                "Time gap tolerance under which the ellipse propagator can be used for orbit "
                "prediction"
            )
        );
    }
    {
        auto& thing = comOpts.posModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ pos", "@ enable"}, "Enable modelling of position")
        );
    }
    {
        auto& thing = comOpts.posModel.sources;
        setInited(
            comOpts,
            thing,
            tryGetEnumVec(thing, modelsNode, {"@ pos", "@ sources"}, "Enable modelling of position")
        );
    }
    {
        auto& thing = comOpts.clockModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ clock", "@ enable"}, "Enable modelling of clocks")
        );
    }
    {
        auto& thing = comOpts.clockModel.sources;
        setInited(
            comOpts,
            thing,
            tryGetEnumVec(
    ...
    {
        auto& thing = comOpts.attitudeModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ attitude", "@ enable"},
                "Enables non-nominal attitude types"
            )
        );
    }
    {
        auto& thing = comOpts.attitudeModel.sources;
        setInited(
            comOpts,
            thing,
            tryGetEnumVec(
                ...
                modelsNode,
                {"@ attitude", "@ sources"},
                "List of sourecs to use for attitudes"
            )
        );
    }
    {
        auto& thing = comOpts.attitudeModel.model_dt;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ attitude", "@ model_dt"},
                "Timestep used in modelling attitude"
            )
        );
    }
    {
        auto& thing = comOpts.codeBiasModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ code_bias", "@ enable"},
                "Enable modelling of code biases"
            )
        );
    }
    {
        ...
        setInited(
            ...
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ code_bias", "@ default_bias"},
                "Bias to use when no code bias is found"
            )
        );
    }
    {
        auto& thing = comOpts.codeBiasModel.undefined_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ code_bias", "@ undefined_sigma"},
                "Uncertainty sigma to apply to default code biases"
            )
        );
    }
    {
        auto& thing = comOpts.phaseBiasModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = comOpts.phaseBiasModel.default_bias;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ phase_bias", "@ default_bias"},
                "Bias to use when no phase bias is found"
            )
        );
    }
    {
        auto& thing = comOpts.phaseBiasModel.undefined_sigma;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ phase_bias", "@ undefined_sigma"},
                "Uncertainty sigma to apply to default phase biases"
            )
        );
    }
    {
        auto& thing = comOpts.pcoModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ pco", "@ enable"},
                "Enable modelling of phase center offsets"
            )
        );
    }
    {
        auto& thing = comOpts.pcvModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ pcv", "@ enable"},
                "Enable modelling of phase center variations"
            )
        );
    }
    {
        auto& thing = comOpts.phaseWindupModel.enable;
        setInited(
            comOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ phase_windup", "@ enable"},
                "Model phase windup due to relative rotation of circularly polarised antennas"
            )
        );
    }

    if (antenna_boresight.size() == 3)
    {
        comOpts.antenna_boresight = Vector3d(antenna_boresight.data());
        setInited(comOpts, comOpts.antenna_boresight);
    }
    if (antenna_azimuth.size() == 3)
    {
        comOpts.antenna_azimuth = Vector3d(antenna_azimuth.data());
        setInited(comOpts, comOpts.antenna_azimuth);
    }
}
...
)
{
    ...
    {
        auto& thing = satOpts.error_model;
        setInited(satOpts, thing, tryGetEnumOpt(thing, satNode, {"1@ error_model"}));
    }
    {
        auto& thing = satOpts.code_sigma;
        setInited(
            satOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = satOpts.phase_sigma;
        setInited(
            satOpts,
            thing,
            tryGetFromYaml(
...
)
{
    ...
    {
        auto ionospheric_component = stringsToYamlObject(
            modelsNode,
            {"@ ionospheric_components"},
            "Ionospheric models produce frequency-dependent effects"
        );
        auto ionospheric_model = stringsToYamlObject(
            modelsNode,
            {"@ ionospheric_model"},
    ...
    {
        auto& thing = recOpts.kill;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, recNode, {"0@ kill"}, "Remove receiver from future processing")
        );
    }
    {
        auto& thing = recOpts.zero_dcb_codes;
        setInited(recOpts, thing, tryGetEnumVec(thing, recNode, {"3@ zero_dcb_codes"}));
    }
    {
        auto& thing = apriori_pos;
        tryGetFromYaml(
            thing,
            recNode,
            {"4@ apriori_position"},
            "Apriori position in XYZ ECEF frame"
        );
    }
    {
        auto& thing = recOpts.antenna_type;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                recNode,
                {"4@ antenna_type"},
                "Antenna type and radome in 20 character string as per sinex"
            )
        );
    }
    {
        auto& thing = recOpts.receiver_type;
        setInited(
            recOpts,
            thing,
    ...
    }
    {
        ...
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, recNode, {"4@ domes_number", ""}, "Domes number for the receiver")
        );
    }
    {
        auto& thing = recOpts.site_description;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = recOpts.elevation_mask_deg;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = recOpts.receiver_reference_system;
        setInited(
            recOpts,
            thing,
            tryGetEnumOpt(
                ...
                "Receiver will use this system as reference clock"
            )
        );
    }
    {
        auto& thing = recOpts.error_model;
        setInited(recOpts, thing, tryGetEnumOpt(thing, recNode, {"1! error_model"}));
    }
    {
        auto& thing = recOpts.code_sigma;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = recOpts.phase_sigma;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = recOpts.eccentricityModel.enable;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ eccentricity", "enable"},
                "Enable antenna eccentrities"
            )
        );
    }
    {
        auto& thing = eccentricity;
        tryGetFromYaml(
            thing,
            modelsNode,
            {"@ eccentricity", "offset"},
            "Antenna offset in ENU frame"
        );
    }

    {
        auto& thing = recOpts.tropModel.enable;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
    ...
    {
        auto& thing = recOpts.tropModel.models;
        setInited(
            recOpts,
            thing,
            tryGetEnumVec(
    ...
    {
        auto& thing = recOpts.tideModels.enable;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ tides", "@ enable"},
                "Enable modelling of tidal displacements"
            )
        );
    }
    {
        auto& thing = recOpts.tideModels.solid;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ tides", "@ solid"}, "Enable solid Earth tides")
        );
    }
    {
        auto& thing = recOpts.tideModels.otl;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ tides", "@ otl"}, "Enable ocean tide loading")
        );
    }
    {
        auto& thing = recOpts.tideModels.atl;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ tides", "@ atl"},
                "Enable atmospheric tide loading"
            )
        );
    }
    {
        auto& thing = recOpts.tideModels.spole;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ tides", "@ spole"},
                "Enable solid Earth pole tides"
            )
        );
    }
    {
        auto& thing = recOpts.tideModels.opole;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ tides", "@ opole"}, "Enable ocean pole tides")
        );
    }
    {
        auto& thing = recOpts.range;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ range", "@ enable"},
                "Enable modelling of signal time of flight time due to range"
            )
        );
    }
    {
        auto& thing = recOpts.relativity;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ relativity", "@ enable"},
                "Enable modelling of relativistic effects"
            )
        );
    }
    {
        auto& thing = recOpts.relativity2;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ relativity2", "@ enable"},
                "Enable modelling of secondary relativistic effects"
            )
        );
    }
    {
        auto& thing = recOpts.sagnac;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                ...
                modelsNode,
                {"@ sagnac", "@ enable"},
                "Enable modelling of sagnac effect"
            )
        );
    }
    {
        auto& thing = recOpts.integer_ambiguity;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ integer_ambiguity", "@ enable"},
                "Model ambiguities due to unknown integer number of cycles in phase measurements"
            )
        );
    }
    {
        auto& thing = recOpts.ionospheric_component;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ ionospheric_components", "enable"},
                "Enable ionospheric modelling"
            )
        );
    }
    {
        auto& thing = recOpts.ionospheric_component2;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ ionospheric_components", "use_2nd_order"})
        );
    }
    {
        auto& thing = recOpts.ionospheric_component3;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ ionospheric_components", "use_3rd_order"})
        );
    }
    {
        auto& thing = recOpts.mapping_function;
        setInited(
            recOpts,
            thing,
            tryGetEnumOpt(
                thing,
                modelsNode,
                {"@ ionospheric_components", "@ mapping_function"},
                "Mapping function if not specified in the data or model"
            )
        );
    }
    {
        auto& thing = recOpts.geomagnetic_field_height;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ ionospheric_components", "@ geomagnetic_field_height"},
                "ionospheric pierce point layer height if not specified in the data or model (km)"
            )
        );
    }
    {
        auto& thing = recOpts.mapping_function_layer_height;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ ionospheric_components", "@ mapping_function_layer_height"},
                "mapping function layer height if not specified in the data or model (km)"
            )
        );
    }
    {
        auto& thing = recOpts.ionospheric_model;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ ionospheric_model", "enable"},
                "Compute ionosphere maps from a network of receivers"
            )
        );
    }
    {
        auto& thing = recOpts.tropospheric_map;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(
                thing,
                modelsNode,
                {"@ tropospheric_map", "enable"},
                "Compute tropospheric maps from a network of receivers"
            )
        );
    }
    {
        auto& thing = recOpts.eop;
        setInited(
            recOpts,
            thing,
            tryGetFromYaml(thing, modelsNode, {"@ eop", "enable"}, "Enable modelling of eops")
        );
    }
    {
        auto& thing = recOpts.eccentricityModel.eccentricity;
        if (eccentricity.size() == 3)
        {
            thing = Vector3d(eccentricity.data());
            setInited(recOpts, thing);
        }
    }
    {
        auto& thing = recOpts.apriori_pos;
        if (apriori_pos.size() == 3)
        {
            thing = Vector3d(apriori_pos.data());
            setInited(recOpts, thing);
        }
    }
                ...
                if (phaseNode.IsSequence())
                {
                    ...
                    for (auto item : phaseNode)
                    {
                        try
                        {
                            ...
                            E_ObsCode obsCode =
                                string_to_enum_nocase_throw<E_ObsCode>(codeStr.c_str());
...
)
{
    ...
    for (int i = 0; i < yamls.size(); i++)
    {
        ...
        tryGetFromYaml(
            yamlAliases,
            {yaml, ""},
            {"3! satellite_options", Sat.id(), "@ aliases"},
            "Aliases for this satellite"
        );
    ...
    if (Sat.blockType().empty() == false)
        aliases.push_back(Sat.blockType());
    aliases.push_back(Sat.id());
    if (Sat.svn().empty() == false)
        aliases.push_back("SVN_" + Sat.svn());
    ...
    for (auto& alias : aliases)
        for (int S = 0; S <= suffixes.size(); S++)
        {
            ...
            vector<string> estimationDescriptorVec = {
...
)
{
    ...
    for (int i = 0; i < yamls.size(); i++)
    {
        ...
        tryGetFromYaml(
            yamlAliases,
            {yaml, ""},
            {"3! receiver_options", id, "@ aliases"},
            "Aliases for this receiver"
        );
        ...
        for (int S = 0; S <= suffixes.size(); S++)
        {
            ...
            for (auto& yaml : yamls)
            {
                stringsToYamlObject(
                    {yaml, ""},
                    {"3! receiver_options"},
                    "Options to configure individual receivers or global configs"
                );
...
)
{
    for (YAML::const_iterator it = node.begin(); it != node.end(); it++)
    {
        ...
        {
            // this yaml stack not found in the available options, check to see if it could have
            // worked if it were an alias
        ...
        {
            // is a final value, this must pass on its own - ie, dont let final leafs with dumb
            // names get aliased away, only pass those that should
...
)
{
    ...
    if (vm.count("very-verbose"))
    {
        ...
        acsSeverity = boost::log::trivial::trace;
    }
    if (vm.count("verbose"))
    {
        boost::log::core::get()->set_filter(
        ...
        acsSeverity = boost::log::trivial::debug;
    }
    if (vm.count("quiet"))
    {
        boost::log::core::get()->set_filter(
    ...
    if (acsConfig.compare_clocks || vm.count("compare_clocks"))
    {
        std::cout << "\n"
                  << "----- Clock Comparator -----" << "\n";
    ...
    if (acsConfig.compare_orbits || vm.count("compare_orbits"))
    {
        std::cout << "\n"
                  << "----- Orbit Comparator -----" << "\n";
    ...
    if (acsConfig.compare_attitudes || vm.count("compare_attitudes"))
    {
        std::cout << "\n"
                  << "----- Attitude Comparator -----" << "\n";
    ...
    valid &= checkValidFiles(
        acsConfig.ocean_tide_loading_blq_files,
        "ocean loading information (Blq file)"
    );
    valid &= checkValidFiles(
        acsConfig.atmos_tide_loading_blq_files,
        "atmospheric loading information (Blq file)"
    );
    ...
    valid &=
        checkValidFiles(acsConfig.planetary_ephemeris_files, "Planetary and lunar ephemerides");
    valid &= checkValidFiles(acsConfig.ocean_tide_potential_files, "Ocean tide file (tide file)");
    valid &=
        checkValidFiles(acsConfig.atmos_tide_potential_files, "Atmospheric tide file  (tide file)");
    ...
    valid &= checkValidFiles(acsConfig.space_weather_files, "Space weather");
    valid &= checkValidFiles(
        acsConfig.atmos_ocean_dealiasing_files,
        "Atmosphere and Ocean De-aliasing (AOD1B file)"
    );
 ...
 * Command line options will override any values set in config files, which will themselves override
 * any program default values.
 */
bool ACSConfig::parse(
    const vector<string>& filenames,  ///< Path to yaml based config file
    boost::program_options::variables_map&
        newCommandOpts                ///< Variable map object of command line options
)
{
    ...
    for (int i = 0; i < filenames.size(); i++)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Checking configuration file " << filename;
    ...
    for (int i = 0; i < yamlList.size(); i++)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading configuration from file " << filename;
        ...
        // 		outputs
        {
            ...
            {
                auto metadata = stringsToYamlObject(
                    outputs,
                    {"2! metadata"},
                ...
                tryGetFromYaml(
                    ...
                    metadata,
                    {"@ analysis_agency"},
                    "Agency for output files headers"
                );
                tryGetFromYaml(
                    config_details,
                    metadata,
                    {"@ config_details"},
                    "Comments and details specific to the config"
                );
                tryGetFromYaml(
                    analysis_centre,
                    metadata,
                    {"@ analysis_centre"},
                    "Analysis centre for output files headers"
                );
                tryGetFromYaml(
                    ac_contact,
                    metadata,
                    {"@ ac_contact"},
                    "Contact person for output files headers"
                );
                tryGetFromYaml(
                    analysis_software,
                    metadata,
                    {"@ analysis_software"},
                    "Program for output files headers"
                );
                tryGetFromYaml(
                    analysis_software_version,
                    metadata,
                    {"@ analysis_software_version"},
                    "Version for output files headers"
                );
                tryGetFromYaml(
                    rinex_comment,
                    metadata,
                    {"@ rinex_comment"},
                    "Comment for output files headers"
                );
                tryGetFromYaml(
                    reference_system,
                    metadata,
                    {"@ reference_system"},
                    "Terrestrial Reference System Code"
                );
                tryGetFromYaml(
                    time_system,
                    metadata,
                    {"@ time_system"},
                    "Time system - e.g. \"G\", \"UTC\""
                );
                tryGetFromYaml(
                ...
                tryGetFromYaml(
                    ...
                    metadata,
                    {"@ geoid_model"},
                    "Geoid model name for undulation values"
                );
                tryGetFromYaml(
                    gradient_mapping_function,
                    metadata,
                    {"@ gradient_mapping_function"},
                    "Name of mapping function used for mapping horizontal troposphere gradients"
                );
            }
...

            {
                ...
                tryGetFromYaml(
                    ...
                    trace,
                    {"0! output_network"},
                    "Output trace files for complete network of receivers, inclucing kalman filter "
                    "results and "
                    "statistics"
                );
                tryGetFromYaml(
                    ...
                    trace,
                    {"0@ output_ionosphere"},
                    "Output trace files for ionosphere processing, inclucing kalman filter results "
                    "and statistics"
                );
                tryGetFromYaml(
                ...
                tryGetFromYaml(
                    ...
                    "Output detailed observation data including CN0, elevation, azimuth, and "
                    "signal availability"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        satellite_trace_filename,
                        trace,
                        {"1@ satellite_filename"},
                        "Template filename for satellite trace files"
                    )
                );
                conditionalPrefix(
                    "<TRACE_DIRECTORY>",
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ionosphere_trace_filename,
                        trace,
                        {"1@ ionosphere_filename"},
                        "Template filename for ionosphere trace files"
                    )
                );
                conditionalPrefix(
                    "<TRACE_DIRECTORY>",
                ...
                tryGetFromAny(
                    ...
                    trace,
                    {"! level"},
                    "Threshold level for printing messages (0-6). Increasing this increases the "
                ...
                tryGetFromYaml(
                    ...
                    trace,
                    {"! output_predicted_states"},
                    "Output states after state transition 1"
                );
                tryGetFromYaml(
                    output_initialised_states,
                    trace,
                    {"! output_initialised_states"},
                    "Output states after state transition 2"
                );
                tryGetFromYaml(
                    output_residuals,
                    trace,
                ...
                tryGetFromYaml(
                    ...
                    trace,
                    {"! output_statistics"},
                    "Output statistics accumulated each epoch"
                );
                tryGetFromYaml(
                    output_summaries,
                    trace,
                    {"! output_summaries"},
                    "Output summaries accumulated each epoch"
                );
                tryGetFromYaml(
                    output_json_trace,
                    trace,
                    {"@ output_json"},
                    "Output json formatted trace files"
                );
            }

            {
                auto output_rotation = stringsToYamlObject(
                    outputs,
                    {"2@ output_rotation"},
                    "Trace files can be rotated periodically by epoch interval. These options "
                    "specify the period that "
            ...
            {
                auto bias_sinex = stringsToYamlObject(
                    outputs,
                    {"3@ bias_sinex"},
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ...
                        bias_sinex,
                        {"@ directory"},
                        "Directory to output bias sinex files to"
                    )
                );
                conditionalPrefix(
                    "<BIAS_SINEX_DIRECTORY>",
                    ...
                    tryGetFromYaml(
                        ...
                        bias_sinex,
                        {"@ filename"},
                        "Template filename for bias sinex files"
                    )
                );
                tryGetFromYaml(
                    bias_time_system,
                    bias_sinex,
                    {"@ bias_time_system"},
                    "Time system for bias SINEX \"G\", \"C\", \"R\", \"UTC\", \"TAI\""
                );
                tryGetFromYaml(
                    ambrOpts.code_output_interval,
                    bias_sinex,
                    {"@ code_output_interval"},
                    "Update interval for code  biases"
                );
                tryGetFromYaml(
                    ambrOpts.phase_output_interval,
                    bias_sinex,
                    {"@ phase_output_interval"},
                    "Update interval for phase biases"
                );
                tryGetFromYaml(
                    ambrOpts.output_rec_bias,
                    bias_sinex,
                    {"@ output_rec_bias"},
                    "output receiver biases"
                );
            }

            {
                auto clocks =
                    stringsToYamlObject(outputs, {"3! clocks"}, "Rinex formatted clock files");
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ...
                        clocks,
                        {"@ directory"},
                        "Directory to output clock files to"
                    )
                );
                conditionalPrefix(
                    "<CLOCKS_DIRECTORY>",
                    ...
                    tryGetFromYaml(
                        ...
                        clocks,
                        {"@ filename"},
                        "Template filename for clock files"
                    )
                );
                ...
                tryGetFromYaml(
                    ...
                    clocks,
                    {"@ output_interval"},
                    "Update interval for clock records"
                );
            }

            {
                auto decoded_rtcm = stringsToYamlObject(
                    outputs,
                    {"6@ decoded_rtcm"},
                    "RTCM messages that are received may be recorded to human-readable json files"
                );
                ...
                tryGetFromYaml(
                    ...
                    decoded_rtcm,
                    {"0@ output"},
                    "Enable exporting decoded RTCM data to file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ...
                    tryGetFromYaml(
                        ...
                        decoded_rtcm,
                        {"@ directory"},
                        "Directory to export decoded RTCM data"
                    )
                );
                conditionalPrefix(
                    "<DECODED_RTCM_DIRECTORY>",
                    decoded_rtcm_json_filename,
                    tryGetFromYaml(
                        decoded_rtcm_json_filename,
                        decoded_rtcm,
                        {"@ filename"},
                        "Decoded RTCM data filename"
                    )
                );
            }

            {
                auto encoded_rtcm = stringsToYamlObject(
                    outputs,
                    {"6@ encoded_rtcm"},
                    "RTCM messages that are encoded and transmitted may be recorded to "
                ...
                tryGetFromYaml(
                    ...
                    encoded_rtcm,
                    {"0@ output"},
                    "Enable exporting encoded RTCM data to file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ...
                    tryGetFromYaml(
                        ...
                        encoded_rtcm,
                        {"@ directory"},
                        "Directory to export encoded RTCM data"
                    )
                );
                conditionalPrefix(
                    "<ENCODED_RTCM_DIRECTORY>",
                    encoded_rtcm_json_filename,
                    tryGetFromYaml(
                        encoded_rtcm_json_filename,
                        encoded_rtcm,
                        {"@ filename"},
                        "Encoded RTCM data filename"
                    )
                );
            }

            {
                auto erp = stringsToYamlObject(
                    outputs,
                    {"3@ erp"},
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ...
                        erp,
                        {"@ directory"},
                        "Directory to export erp data files"
                    )
                );
                conditionalPrefix(
                    "<ERP_DIRECTORY>",
                    erp_filename,
                    tryGetFromYaml(erp_filename, erp, {"@ filename"}, "ERP data output filename")
                );
            }

            {
                auto ionex = stringsToYamlObject(
                    outputs,
                    {"4@ ionex"},
                ...
                tryGetFromYaml(
                    ...
                    ionex,
                    {"0@ output"},
                    "Enable exporting ionospheric model data"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ionex_directory,
                    tryGetFromYaml(
                        ...
                        ionex,
                        {"@ directory"},
                        "Directory to export ionex data"
                    )
                );
                conditionalPrefix(
                    "<IONEX_DIRECTORY>",
                    ionex_filename,
                    tryGetFromYaml(ionex_filename, ionex, {"@ filename"}, "Ionex data filename")
                );
                tryGetFromYaml(
                    ionexGrid.lat_centre,
                    ionex,
                    {"@ grid", "@ lat_centre"},
                    "Center lattitude for models"
                );
                tryGetFromYaml(
                    ionexGrid.lon_centre,
                    ionex,
                    {"@ grid", "@ lon_centre"},
                    "Center longitude for models"
                );
                tryGetFromYaml(
                    ionexGrid.lat_width,
                    ionex,
                    {"@ grid", "@ lat_width"},
                    "Total lattitudinal width of model"
                );
                tryGetFromYaml(
                    ionexGrid.lon_width,
                    ionex,
                    {"@ grid", "@ lon_width"},
                    "Total longitudinal width of model"
                );
                tryGetFromYaml(
                    ionexGrid.lat_res,
                    ionex,
                    {"@ grid", "@ lat_resolution"},
                    "Interval between lattitude outputs"
                );
                tryGetFromYaml(
                    ionexGrid.lon_res,
                    ionex,
                    {"@ grid", "@ lon_resolution"},
                    "Interval between longitude outputs"
                );
                tryGetFromYaml(
                    ionexGrid.time_res,
                    ionex,
                    {"@ grid", "@ time_resolution"},
                    "Interval between output epochs"
                );
            }

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ionstec_directory,
                    tryGetFromYaml(ionstec_directory, ionstec, {"@ directory"})
                );
                conditionalPrefix(
                    "<IONSTEC_DIRECTORY>",
                    ionstec_filename,
...

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ems_directory,
                    tryGetFromYaml(ems_directory, sbas_ems, {"@ directory"})
                );
                conditionalPrefix(
                    "<EMS_DIRECTORY>",
                    ems_filename,
                    tryGetFromYaml(ems_filename, sbas_ems, {"@ filename"})
                );
            }

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    sinex_directory,
                    tryGetFromYaml(sinex_directory, sinex, {"@ directory"})
                );
                conditionalPrefix(
                    "<SINEX_DIRECTORY>",
                    sinex_filename,
                    tryGetFromYaml(sinex_filename, sinex, {"@ filename"})
                );
            }

            {
                auto log = stringsToYamlObject(
                    outputs,
                    {"3! log"},
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    log_directory,
                    tryGetFromYaml(log_directory, log, {"@ directory"}, "Log output directory")
                );
                conditionalPrefix(
                    "<LOG_DIRECTORY>",
                    log_filename,
                    tryGetFromYaml(log_filename, log, {"@ filename"}, "Log output filename")
                );
            }

            {
                auto gpx = stringsToYamlObject(
                    outputs,
                    {"3! gpx"},
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    gpx_directory,
                    tryGetFromYaml(gpx_directory, gpx, {"@ directory"})
                );
                conditionalPrefix(
                    "<GPX_DIRECTORY>",
                    gpx_filename,
                    tryGetFromYaml(gpx_filename, gpx, {"@ filename"})
                );
            }
            {
                auto pos = stringsToYamlObject(
                    outputs,
                    {"3! pos"},
                    "POS files contain point data that may be easily viewed in GIS mapping software"
                );
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    pos_directory,
                    tryGetFromYaml(pos_directory, pos, {"@ directory"})
                );
                conditionalPrefix(
                    "<POS_DIRECTORY>",
                    pos_filename,
                    tryGetFromYaml(pos_filename, pos, {"@ filename"})
                );
            }
...

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ntrip_log_directory,
                    tryGetFromYaml(ntrip_log_directory, ntrip_log, {"@ directory"})
                );
                conditionalPrefix(
                    "<NTRIP_LOG_DIRECTORY>",
                    ntrip_log_filename,
                    tryGetFromYaml(ntrip_log_filename, ntrip_log, {"@ filename"})
                );
            }

            {
                ...
                tryGetFromYaml(
                    ...
                    network_statistics,
                    {"0@ output"},
                    "Enable exporting network statistics data to file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    ...
                    tryGetFromYaml(
                        ...
                        network_statistics,
                        {"@ directory"},
                        "Directory to export network statistics data"
                    )
                );
                conditionalPrefix(
                    "<NETWORK_STATISTICS_DIRECTORY>",
                    ...
                    tryGetFromYaml(
                        ...
                        network_statistics,
                        {"@ filename"},
                        "Network statistics data filename"
                    )
                );
            }

            {
                auto sp3 = stringsToYamlObject(
                    outputs,
                    {"3@ sp3"},
                ...
                tryGetFromYaml(
                    ...
                    sp3,
                    {"@ output_inertial"},
                    "Output the entries using inertial positions and velocities"
                );
                tryGetFromYaml(
                    output_sp3_velocities,
                    sp3,
                    {"@ output_velocities"},
                    "Output velocity data to SP3 file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    sp3_directory,
                    tryGetFromYaml(
                        ...
                        sp3,
                        {"@ directory"},
                        "Directory to store SP3 outputs"
                    )
                );
                conditionalPrefix(
                    "<SP3_DIRECTORY>",
                    sp3_filename,
                    tryGetFromYaml(sp3_filename, sp3, {"@ filename"}, "SP3 output filename")
                );
                conditionalPrefix(
                    "<SP3_DIRECTORY>",
                    ...
                    tryGetFromYaml(
                        ...
                        sp3,
                        {"@ predicted_filename"},
                        "Filename for predicted SP3 outputs"
                    )
                );
                tryGetEnumVec(
                    sp3_clock_sources,
                    sp3,
                    {"@ clock_sources"},
                    "List of sources for clock data for SP3 outputs"
                );
                tryGetEnumVec(
                    sp3_orbit_sources,
                    sp3,
                    {"@ orbit_sources"},
                    "List of sources for orbit data for SP3 outputs"
                );
                tryGetFromYaml(
                    sp3_output_interval,
                    sp3,
            ...
            {
                auto orbit_ics = stringsToYamlObject(
                    outputs,
                    {"4@ orbit_ics"},
                    "Orbital parameters can be output in a yaml that Ginan can later use as an "
                ...
                tryGetFromYaml(
                    ...
                    orbit_ics,
                    {"@ output"},
                    "Output orbital initial condition file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    orbit_ics_directory,
                    tryGetFromYaml(
                        ...
                        orbit_ics,
                        {"@ directory"},
                        "Output orbital initial condition directory"
                    )
                );
                conditionalPrefix(
                    "<ORBIT_ICS_DIRECTORY>",
                    ...
                    tryGetFromYaml(
                        ...
                        orbit_ics,
                        {"@ filename"},
                        "Output orbital initial condition filename"
                    )
                );
            }

            {
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ...
                        orbex,
                        {"@ directory"},
                        "Output orbex directory"
                    )
                );
                conditionalPrefix(
                    "<ORBEX_DIRECTORY>",
                    orbex_filename,
                    tryGetFromYaml(orbex_filename, orbex, {"@ filename"}, "Output orbex filename")
                );
                tryGetEnumVec(
                    orbex_orbit_sources,
                    orbex,
                    {"@ orbit_sources"},
                    "Sources for orbex orbits"
                );
                tryGetEnumVec(
                    orbex_clock_sources,
                    orbex,
                    {"@ clock_sources"},
                    "Sources for orbex clocks"
                );
                tryGetEnumVec(
                    orbex_attitude_sources,
                    orbex,
                    {"@ attitude_sources"},
                    "Sources for orbex attitudes"
                );
                tryGetEnumVec(
                    orbex_record_types,
                    orbex,
                    {"@ record_types"},
                    "List of record types to output to orbex file"
                );
                tryGetFromYaml(
                    orbex_output_interval,
                    orbex,
                    {"@ output_interval"},
                    "Update interval for orbex records (irregular epoch interval is currently NOT "
                    "supported)"
                );
            }

            {
                ...
                tryGetFromYaml(
                    ...
                    cost,
                    {"0@ output"},
                    "Enable data exporting to troposphere COST file"
                );
                tryGetEnumVec(
                    cost_data_sources,
                    cost,
                    {"@ sources"},
                    "Source for troposphere delay data - KALMAN, etc."
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    cost_directory,
                    tryGetFromYaml(
                        ...
                        cost,
                        {"@ directory"},
                        "Directory to export troposphere COST file"
                    )
                );
                conditionalPrefix(
                    "<COST_DIRECTORY>",
                    cost_filename,
                    tryGetFromYaml(cost_filename, cost, {"@ filename"}, "Troposphere COST filename")
                );
                tryGetFromYaml(
                    cost_time_interval,
                    cost,
                    {"@ time_interval"},
                    "Time interval between entries in troposphere COST file (sec)"
                );
                tryGetFromYaml(
                    cost_format,
                    cost,
...

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    rinex_nav_directory,
                    tryGetFromYaml(rinex_nav_directory, rinex_nav, {"@ directory"})
                );
                conditionalPrefix(
                    "<RINEX_NAV_DIRECTORY>",
                    rinex_nav_filename,
                    tryGetFromYaml(rinex_nav_filename, rinex_nav, {"@ filename"})
                );
...

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    rinex_obs_directory,
                    tryGetFromYaml(rinex_obs_directory, rinex_obs, {"@ directory"})
                );
                conditionalPrefix(
                    "<RINEX_OBS_DIRECTORY>",
                    rinex_obs_filename,
                    tryGetFromYaml(rinex_obs_filename, rinex_obs, {"@ filename"})
                );
...

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    rtcm_nav_directory,
                    tryGetFromYaml(rtcm_nav_directory, rtcm_nav, {"@ directory"})
                );
                conditionalPrefix(
                    "<RTCM_NAV_DIRECTORY>",
                    rtcm_nav_filename,
                    tryGetFromYaml(rtcm_nav_filename, rtcm_nav, {"@ filename"})
                );
            }

            {
                ...
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    rtcm_obs_directory,
                    tryGetFromYaml(rtcm_obs_directory, rtcm_obs, {"@ directory"})
                );
                conditionalPrefix(
                    "<RTCM_OBS_DIRECTORY>",
                    rtcm_obs_filename,
                    tryGetFromYaml(rtcm_obs_filename, rtcm_obs, {"@ filename"})
                );
            }
...

            {
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(raw_custom_directory, raw_custom, {"directory"})
                );
                conditionalPrefix(
                    "<CUSTOM_DIRECTORY>",
                    raw_custom_filename,
                    tryGetFromYaml(raw_custom_filename, raw_custom, {"filename"})
                );
            }

            {
                ...
                tryGetFromYaml(
                    ...
                    slr_obs,
                    {"0@ output"},
                    "Enable data exporting to tabular SLR obs file"
                );
                conditionalPrefix(
                    "<OUTPUTS_ROOT>",
                    slr_obs_directory,
                    tryGetFromYaml(
                        ...
                        slr_obs,
                        {"@ directory"},
                        "Directory to export tabular SLR obs file"
                    )
                );
                conditionalPrefix(
                    "<SLR_OBS_DIRECTORY>",
            ...
            {
                auto trop_sinex =
                    stringsToYamlObject(outputs, {"3@ trop_sinex"}, docs["trop_sinex"]);

                tryGetFromYaml(
                    output_trop_sinex,
                    trop_sinex,
                    {"0@ output"},
                    "Enable data exporting to troposphere SINEX file"
                );
                tryGetEnumVec(
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        ...
                        trop_sinex,
                        {"@ directory"},
                        "Directory to export troposphere SINEX file"
                    )
                );
                conditionalPrefix(
                    "<TROP_SINEX_DIRECTORY>",
                ...
                tryGetFromYaml(
                    ...
                    trop_sinex,
                    {"@ sol_type"},
                    "Troposphere SINEX solution type"
                );
                tryGetFromYaml(
                    trop_sinex_obs_code,
                    trop_sinex,
                    {"@ obs_code"},
                    "Troposphere SINEX observation code"
                );
                tryGetFromYaml(
                    trop_sinex_const_code,
                    trop_sinex,
                    {"@ const_code"},
                    "Troposphere SINEX const code"
                );
                tryGetFromYaml(
                    trop_sinex_version,
                    trop_sinex,
                    {"@ version"},
                    "Troposphere SINEX version"
                );
            }
            ...
            {
                auto ssr_outputs =
                    stringsToYamlObject(outputs, {"2@ ssr_outputs"}, docs["ssr_outputs"]);

                tryGetEnumVec(
                    ssrOpts.ephemeris_sources,
                    ssr_outputs,
                    {"@ ephemeris_sources"},
                    "Sources for SSR ephemeris"
                );
                tryGetEnumVec(
                    ssrOpts.clock_sources,
                    ssr_outputs,
                    {"@ clock_sources"},
                    "Sources for SSR clocks"
                );
                tryGetEnumVec(
                    ssrOpts.code_bias_sources,
                    ssr_outputs,
                    {"2@ code_bias_sources"},
                    "Sources for SSR code biases"
                );
                tryGetEnumVec(
                    ssrOpts.phase_bias_sources,
                    ssr_outputs,
                    {"2@ phase_bias_sources"},
                ...
                tryGetFromYaml(
                    ssrOpts.extrapolate_corrections,
                    ssr_outputs,
                ...
                {
                    auto atmospheric =
                        stringsToYamlObject(ssr_outputs, {"@ atmospheric"}, docs["atmospheric"]);

                    tryGetEnumVec(
                        ssrOpts.atmosphere_sources,
                        atmospheric,
                        {"@ sources"},
                    ...
                    tryGetFromYaml(
                        ...
                        atmospheric,
                        {"@ region_id"},
                        "Region ID for atmospheric corrections"
                    );
                    tryGetFromYaml(
                        ssrOpts.region_iod,
                        atmospheric,
                        {"@ region_iod"},
                        "Region IOD for atmospheric corrections (default: -1 for undefined)"
                    );
                    ...
                    tryGetFromYaml(
                        ...
                        atmospheric,
                        {"@ grid_type"},
                        "Grid type for gridded atmospheric corrections"
                    );
                    tryGetFromYaml(
                        ssrOpts.use_grid_iono,
                        atmospheric,
                        {"@ use_grid_iono"},
                        "Grid type for gridded atmospheric corrections"
                    );
                    tryGetFromYaml(
                        ssrOpts.use_grid_trop,
                        atmospheric,
                        {"@ use_grid_trop"},
                        "Grid type for gridded atmospheric corrections"
                    );
                    ...
                    tryGetFromYaml(
                        ssrOpts.cmpssr_stec_format,
                        atmospheric,
                        {"@ cmpssr_stec_format"},
                        "Format of STEC gridded corrections: 0:4bit(LSB=0.04) , 1:4bit(LSB=0.12), "
                        "2:5bit, 3:7bit, "
                        "4:16bit"
                    );
                    tryGetFromYaml(
                        ssrOpts.cmpssr_trop_format,
                        atmospheric,
                        {"@ cmpssr_trop_format"},
                        "Format of Trop. ZWD corrections: 0:8bit, 1:6bit"
                    );
                }
            }

            {
                ...
                tryGetFromYaml(
                    ...
                    streams,
                    {"0@ root_url"},
                    "Root url to be prepended to all other streams specified in this section. If "
                    "the streams used have "
                    "individually specified root urls, usernames, or passwords, this should not be "
                    "used."
                );
                ...
                auto [outStreamNode, outStreamString] = stringsToYamlObject(
                    streams,
                    {"1@ labels"},
                    "List of output stream is with further information to be found in its own "
                    "section, as per XMPL "
                    "below"
                );
                ...
                for (auto outLabelYaml : outStreamNode)
                {
                    ...
                    tryGetStreamFromYaml(
                        netOpts.uploadingStreamData[outLabel],
                        streams,
                        {outLabel}
                    );
                    ...
                    conditionalPrefix(
                        "<ROOT_STREAM_URL>",
                        netOpts.uploadingStreamData[outLabel].url
                    );
        ...
        // 		inputs
        {
            ...
            auto tides = stringsToYamlObject(
                inputs,
                {"2@ tides"},
                "Files specifying tidal loading and potential inputs"
            );
            auto ionosphere = stringsToYamlObject(
                inputs,
                {"3@ ionosphere"},
            ...
            getAppendFiles(
                ...
                inputs,
                {"4@ planetary_ephemeris_files"},
                "List of jpl files to use"
            );
            ...
            getAppendFiles(
                space_weather_files,
                inputs,
                {"4@ space_weather_files"},
                "List of space weather files to use"
            );
            getAppendFiles(
                atm_reg_definitions,
                ionosphere,
                {"@ atm_reg_definitions"},
                "List of files to define regions for compact SSR"
            );
            getAppendFiles(
                ion_files,
                ionosphere,
                {"@ ion_files"},
                "List of IONEX files for VTEC input"
            );
            ...
            getAppendFiles(
                ...
                troposphere,
                {"@ gpt2grid_files"},
                "List of gpt2 grid files to use"
            );
            getAppendFiles(
                orography_files,
                troposphere,
                {"@ orography_files"},
                "List of orography files to use"
            );
            getAppendFiles(
                ocean_tide_potential_files,
                tides,
                {"@ ocean_tide_potential_files"},
                "List of tide files to use"
            );
            getAppendFiles(
            ...
            getAppendFiles(
                ...
                tides,
                {"@ ocean_tide_loading_blq_files"},
                "List of otl blq files to use"
            );
            getAppendFiles(
                atmos_tide_loading_blq_files,
                tides,
                {"@ atmos_tide_loading_blq_files"},
                "List of atl blq files to use"
            );
            getAppendFiles(
                ocean_pole_tide_loading_files,
                tides,
                {"@ ocean_pole_tide_loading_files"},
                "List of opole files to use"
            );
            getAppendFiles(
            ...
            getAppendFiles(
                ocean_pole_tide_potential_files,
                tides,
                {"@ ocean_pole_tide_potential_files"},
                "List of tide files to use"
            );
            ...
            tryGetEnumVec(
                ...
                tides,
                {"@ atl_blq_row_order"},
                "Row order for amplitude and phase components in ATL BLQ files"
            );
            tryGetEnumVec(
                otl_blq_row_order,
                tides,
                {"@ otl_blq_row_order"},
                "Row order for amplitude and phase components in OTL BLQ files"
            );
            ...
            tryGetEnumVec(
                ...
                tides,
                {"@ atl_blq_col_order"},
                "Column order for amplitude and phase components in ATL BLQ files"
            );
            tryGetEnumVec(
                otl_blq_col_order,
                tides,
                {"@ otl_blq_col_order"},
                "Column order for amplitude and phase components in OTL BLQ files"
            );

            {
                auto gnss_data = stringsToYamlObject(
                    inputs,
                    {"2! gnss_observations"},
                ...
                conditionalPrefix(
                    ...
                    tryGetFromAny(
                        ...
                        commandOpts,
                        gnss_data,
                        {"0! gnss_observations_root"},
                        "Root path to be added to all other gnss data inputs (unless they are "
                        "absolute)"
                    )
                );
                ...
                tryGetMappedList(
                    ...
                    {"1# custom_inputs"},
                    "<GNSS_OBS_ROOT>",
                    "List of customfiles inputs to use"
                );
                tryGetMappedList(
                    obs_rtcm_inputs,
                    commandOpts,
                    ...
                    {"1! rtcm_inputs"},
                    "<GNSS_OBS_ROOT>",
                    "List of rtcmfiles  inputs to use for observations"
                );
            }

            {
                auto pseudo_observation_data = stringsToYamlObject(
                    inputs,
                    {"2@ pseudo_observations"},
                    "Use data from pre-processed data products as observations. Useful for "
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        pseudo_obs_root,
                        pseudo_observation_data,
                        {"0@ pseudo_observations_root"},
                        "Root path to be added to all other pseudo obs data files (unless they are "
                        "absolute)"
                    )
                );
                ...
                tryGetMappedList(
                    ...
                    {"@@ sp3_inputs"},
                    "<PSEUDO_OBS_ROOT>",
                    "List of sp3 inputs to use for pseudoobservations"
                );
                tryGetMappedList(
                    pseudo_snx_inputs,
                    commandOpts,
                    ...
                    {"1@ snx_inputs"},
                    "<PSEUDO_OBS_ROOT>",
                    "List of snx inputs to use for pseudoobservations"
                );
                conditionalPrefix(
                    "<PSEUDO_OBS_ROOT>",
                    pseudo_filter_files,
                    tryGetFromAny(
                        ...
                        pseudo_observation_data,
                        {"1# filter_files"},
                        "List of inputs to use for custom pseudoobservations"
                    )
                );
                ...
                tryGetFromYaml(
                    ...
                    pseudo_observation_data,
                    {"@ eci_pseudoobs"},
                    "Pseudo observations are provided in eci frame rather than standard ECEF SP3 "
                    "files"
                );
            }

            {
                ...
                conditionalPrefix(
                    ...
                    tryGetFromYaml(
                        sat_data_root,
                        satellite_data,
                        {"0! satellite_data_root"},
                        "Root path to be added to all other satellite data files (unless they are "
                        "absolute)"
                    )
                );
                conditionalPrefix(
                ...
                conditionalPrefix(
                    ...
                    tryGetFromAny(
                        ...
                        satellite_data,
                        {"2@ sid_files"},
                        "List of sat ID     files to use - from "
                ...
                conditionalPrefix(
                    ...
                    tryGetFromAny(
                        ...
                        satellite_data,
                        {"2@ com_files"},
                        "List of com        files to use - retroreflector offsets from "
                        "centre-of-mass for spherical "
                        "sats"
                    )
                );
                conditionalPrefix(
                    ...
                    tryGetFromAny(
                        ...
                        satellite_data,
                        {"2@ crd_files"},
                        "List of crd        files to use - SLR observation data"
                    )
                );
                conditionalPrefix(
                    "<SAT_DATA_ROOT>",
                ...
                {
                    auto rtcm_inputs =
                        stringsToYamlObject(satellite_data, {"! rtcm_inputs"}, docs["rtcm_inputs"]);
                    ...
                    conditionalPrefix(
                        ...
                        tryGetFromYaml(
                            ...
                            rtcm_inputs,
                            {"0! rtcm_inputs_root"},
                            "Root path to be added to all other rtcm inputs (unless they are "
                            "absolute)"
                        )
                    );
                    ...
                    conditionalPrefix(
                        ...
                        tryGetFromAny(
                            ...
                            rtcm_inputs,
                            {"2@ qzl6_inputs"},
                            "List of qzss L6    inputs to use for corrections"
                        )
                    );

                    tryGetFromYaml(
                        ssrInOpts.code_bias_valid_time,
                        rtcm_inputs,
                        {"@ code_bias_validity_time"},
                        "Valid time period of SSR code biases"
                    );
                    tryGetFromYaml(
                        ssrInOpts.phase_bias_valid_time,
                        rtcm_inputs,
                        {"@ phase_bias_validity_time"},
                        "Valid time period of SSR phase biases"
                    );
                    tryGetFromYaml(
                        ssrInOpts.one_freq_phase_bias,
                        rtcm_inputs,
                        {"@ one_freq_phase_bias"},
                        "Used stream have one SSR phase bias per frequency"
                    );
                    tryGetFromYaml(
                        ssrInOpts.global_vtec_valid_time,
                        rtcm_inputs,
                        {"@ global_vtec_valid_time"},
                        "Valid time period of global VTEC maps"
                    );
                    tryGetFromYaml(
                        ssrInOpts.local_stec_valid_time,
                        rtcm_inputs,
                        {"@ local_stec_valid_time"},
                        "Valid time period of local STEC corrections"
                    );
                    tryGetFromYaml(
                        ssrInOpts.local_trop_valid_time,
                        rtcm_inputs,
                        {"@ local_trop_valid_time"},
                        "Valid time period of local Troposphere corrections"
                    );
                    tryGetFromYaml(
                    ...
                    tryGetEnumOpt(
                        ...
                        rtcm_inputs,
                        {"1! ssr_antenna_offset"},
                        "Ephemeris type that is provided in the listed SSR stream, i.e. satellite "
                        "antenna-phase-centre "
                        "(APC) or centre-of-mass (COM). This information is listed in the NTRIP "
                        "Caster's sourcetable"
                    );
                }

                {
                    auto sbas_inputs = stringsToYamlObject(
                        satellite_data,
                        {"! sbas_inputs"},
                        "Configuration for SBAS related input. Including SiSNet streams and EMS "
                        "files"
                    );
                    ...
                    conditionalPrefix(
                        ...
                        tryGetFromYaml(
                            sisnet_inputs_root,
                            sbas_inputs,
                            {"2@ sisnet_inputs_root"},
                            "Root path to be added to all other sisnet inputs (unless they are "
                            "absolute)"
                        )
                    );
                    ...
                    conditionalPrefix(
                        ...
                        tryGetFromAny(
                            ...
                            sbas_inputs,
                            {"2@ sisnet_inputs"},
                            "List of sisnet inputs to use for corrections"
                        )
                    );
        ...
        // 		processing_options
        {
            auto processing_options = stringsToYamlObject(
                {yaml, ""},
            ...
            // 			process_modes
            {
                auto process_modes = stringsToYamlObject(
                    processing_options,
                    {"1! process_modes"},
                    "Aspects of the processing flow may be enabled and disabled according to "
                ...
                tryGetFromYaml(
                    ...
                    process_modes,
                    {"@ ionosphere"},
                    "Compute Ionosphere models based on GNSS measurements"
                );
                tryGetFromYaml(
            ...
            // 			gnss_general
            {
                auto general = stringsToYamlObject(
                    processing_options,
                    {"0! gnss_general"},
                ...
                tryGetFromYaml(
                    ...
                    general,
                    {"@ require_apriori_positions"},
                    "Restrict processing to receivers that have apriori positions available"
                );
                tryGetFromYaml(
                    require_site_eccentricity,
                    general,
                    {"@ require_site_eccentricity"},
                    "Restrict processing to receivers that have site eccentricity information"
                );
                tryGetFromYaml(
                    pppOpts.merge_correlated_states,
                    general,
                    {"@ merge_correlated_states"},
                    "Combine correlated states to eliminate unestimable states"
                );
                tryGetFromYaml(
                    require_sinex_data,
                    general,
                    {"@ require_sinex_data"},
                    "Restrict processing to receivers that have sinex data available"
                );
                tryGetFromYaml(
                    require_antenna_details,
                    general,
                    {"@ require_antenna_details"},
                    "Restrict processing to receivers that have antenna details"
                );
                tryGetFromYaml(
                    require_reflector_com,
                    general,
                    {"@ require_reflector_com"},
                    "Restrict processing to SLR observations that have center of mass to laser "
                ...
                tryGetFromYaml(
                    ...
                    general,
                    {"@ reference_clock"},
                    "ID of sat/rec to use for reference clock in pivot calculations"
                );
                tryGetFromYaml(
                    reference_bias,
                    general,
                    {"@ reference_bias"},
                    "ID of sat/rec to use for reference bias in pivot calculations"
                );
                tryGetFromYaml(
                    pivot_receiver,
                    general,
                    {"@ pivot_receiver"},
                    "Largely deprecated option for iono"
                );
                tryGetFromYaml(
                    interpolate_rec_pco,
                    general,
                    {"@ interpolate_rec_pco"},
                    "Interpolate other known pco values to find pco for unknown frequencies"
                );
                tryGetFromYaml(
                    auto_fill_pco,
                    general,
                    {"@ auto_fill_pco"},
                    "Use similar PCOs when requested values are not found"
                );
                tryGetFromYaml(
                    pppOpts.equate_ionospheres,
                    general,
                    {"@ equate_ionospheres"},
                    "Use same STEC values for different receivers, useful for simulated rtk mode"
                );
                tryGetFromYaml(
                    pppOpts.equate_tropospheres,
                    general,
                    {"@ equate_tropospheres"},
                ...
                tryGetFromYaml(
                    ...
                    general,
                    {"@ use_rtk_combo"},
                    "Combine applicable observations to simulate an rtk solution"
                );
                tryGetFromYaml(
                    pppOpts.use_primary_signals,
                    general,
                    {"@ use_primary_signals"},
                    "Limit processing to first signal of a frequency when multiple are available"
                );
                tryGetFromYaml(
                    pppOpts.add_eop_component,
                    general,
                    {"@ add_eop_component"},
                    "Add eop adjustments as a component in residual chain (for adjusting frames to "
                    "match ecef "
                    "ephemeris)"
                );
                tryGetFromYaml(
                    ...
                    general,
                    {"@ delete_old_ephemerides"},
                    "Remove old ephemerides that have accumulated over time from before far before "
                    "the currently "
                    "processing epoch"
                );
                tryGetFromYaml(
                    ...
                    general,
                    {"@ use_tgd_bias"},
                    "Use TGD/BGD bias from ephemeris, DO NOT turn on unless using "
                    "Klobuchar/NeQuick Ionospheres"
                );
                tryGetFromYaml(
                    common_sat_pco,
                    general,
                    {"@ common_sat_pco"},
                    "Use L1 satellite PCO values for all signals"
                );
                tryGetFromYaml(
                    common_rec_pco,
                    general,
                    {"@ common_rec_pco"},
                    "Use L1 receiver PCO values for all signals"
                );
                tryGetFromYaml(
                    leap_seconds,
                    general,
                    {"@ gpst_utc_leap_seconds"},
                    "Difference between gps time and utc in leap seconds"
                );

                tryGetFromYaml(
                    process_meas[CODE],
                    general,
                    {"1@ code_measurements", "process"},
                    "Process code measurements"
                );
                tryGetFromYaml(
                    process_meas[PHAS],
                    general,
                    {"1@ phase_measurements", "process"},
                ...
                tryGetFromYaml(
                    ...
                    general,
                    {"@ fixed_phase_bias_var"},
                    "Variance of phase bias to be considered fixed/binded"
                );
                tryGetFromYaml(
                    adjust_rec_clocks_by_spp,
                    general,
                    {"@ adjust_rec_clocks_by_spp"},
                    "Adjust receiver clocks by SPP values to minimise prefit residuals"
                );
                tryGetFromYaml(
                    adjust_clocks_for_jumps_only,
                    general,
                    {"@ adjust_clocks_for_jumps_only"},
                    "Round clock adjustments from SPP to half milliseconds"
                );
                // tryGetFromYaml	(minimise_sat_clock_offsets,				general, {"@
                // minimise_sat_clock_offsets"
                // }, "Apply gauss-markov mu values to satellite clocks to minimise offsets with
                // respect to broadcast values");
                tryGetFromYaml(
                    minimise_sat_orbit_offsets,
                    general,
                    {"@ minimise_sat_orbit_offsets"},
                    "Apply gauss-markov mu values to satellite orbits to minimise offsets with "
                ...
                tryGetFromYaml(
                    ...
                    general,
                    {"@ minimise_ionosphere_offsets"},
                    "Apply gauss-markov mu values to stec values to minimise offsets with respect "
                    "to klobuchar values"
                );

                auto clock_offset = stringsToYamlObject(
                    general,
                    {"@ minimise_sat_clock_offsets"},
                    "Apply gauss-markov mu values to satellite clocks to minimise offsets with "
                    "respect to broadcast "
                ...
                tryGetFromYaml(
                    minimise_sat_clock_offsets.enable,
                    clock_offset,
                    {"@ enable"},
                    "Enable gauss-markov mu values to satellite clocks to minimise offsets with "
                    "respect to broadcast "
                    "values"
                );
                tryGetFromYaml(
                    minimise_sat_clock_offsets.max_offset,
                    clock_offset,
                    {"@ max_offset"},
                ...
                for (E_Sys sys : magic_enum::enum_values<E_Sys>())
                {
                    ...
                    tryGetFromYaml(
                        ...
                        sys_options,
                        {"2@ reject_eclipse"},
                        "Exclude satellites that are in eclipsing region"
                    );
                    tryGetFromYaml(
                        receiver_amb_pivot[sys],
                        sys_options,
                        {"2@ receiver_amb_pivot"},
                        "Constrain: set of ambiguities, to eliminate receiver rank deficiencies"
                    );
                    tryGetFromYaml(
                        network_amb_pivot[sys],
                        sys_options,
                        {"2@ network_amb_pivot"},
                        "Constrain: set of ambiguities, to eliminate network  rank deficiencies"
                    );
                    tryGetFromYaml(
                        use_for_iono_model[sys],
                        sys_options,
                        {"2@ use_for_iono_model"},
                        "Use this constellation as part of Ionospheric model"
                    );
                    tryGetFromYaml(
                        use_iono_corrections[sys],
                        sys_options,
                        {"2@ use_iono_corrections"},
                        "Use external ionosphere delay estimation for this constellation"
                    );
                    ...
                    tryGetFromYaml(
                        constrain_best_ambiguity_integer[sys],
                        sys_options,
                        {"@ constrain_best_ambiguity_integer"},
                    ...
                    tryGetFromYaml(
                        ...
                        sys_options,
                        {"@ constrain_clock"},
                        "ID of a sat/rec for constraining its clock"
                    );
                    tryGetFromYaml(
                        constrain_phase_bias[sys],
                        sys_options,
                        {"@ constrain_phase_bias"},
                        "ID of a sat/rec for constraining its phase bias"
                    );
                    tryGetFromYaml(
                        eph_time_delay[sys],
                        sys_options,
            ...
            // 			epoch_control
            {
                auto epoch_control = stringsToYamlObject(
                    processing_options,
                    {"0! epoch_control"},
                ...
                tryGetFromAny(
                    ...
                    epoch_control,
                    {"@ epoch_tolerance"},
                    "Tolerance of times to add to an epoch (usually half of the original data's "
                    "sample rate)"
                );
                tryGetFromAny(
                    max_epochs,
                ...
                tryGetFromAny(
                    ...
                    epoch_control,
                    {"! start_epoch"},
                    "(YYYY-MM-DD hh:mm:ss) The time of the first epoch to process (all "
                    "observations before this will "
                    "be skipped)"
                );
                tryGetFromAny(
                    stopStr,
                    ...
                    epoch_control,
                    {"! end_epoch"},
                    "(YYYY-MM-DD hh:mm:ss) The time of the last epoch to process (all observations "
                    "after this will be "
                    "skipped)"
                );

                if (!startStr.empty())
                    start_epoch = boost::posix_time::time_from_string(startStr);
                if (!stopStr.empty())
                    end_epoch = boost::posix_time::time_from_string(stopStr);
                ...
                tryGetFromYaml(
                    ...
                    epoch_control,
                    {"# sleep_milliseconds"},
                    "Time to sleep before checking for new data - lower numbers are associated "
                    "with high idle cpu usage"
                );
                tryGetFromYaml(
                    wait_next_epoch,
                    epoch_control,
                ...
                tryGetFromYaml(
                    ...
                    epoch_control,
                    {"@ require_obs"},
                    "Exit the program if no observation sources are available"
                );
                tryGetFromYaml(
                    assign_closest_epoch,
                    epoch_control,
                    {"@ assign_closest_epoch"},
                    "Assign observations to the closest epoch - don't skip observations that fall "
                    "between epochs"
                );
                tryGetFromAny(
                    simulate_real_time,
                    ...
                    epoch_control,
                    {"@ simulate_real_time"},
                    "For RTCM playback - delay processing to match original data rate"
                );
            }

            // 			model_error_handling
            {
                auto model_error_handling = stringsToYamlObject(
                    processing_options,
                    {"5! model_error_handling"},
                    "The kalman filter is capable of automatic statistical integrity modelling"
                );

                {
                    auto meas_deweighting = stringsToYamlObject(
                        model_error_handling,
                        {"0! meas_deweighting"},
                        "Measurements that are outside the expected confidence bounds may be "
                        "deweighted so that "
                        "outliers do not contaminate the filtered solution"
                    );
                    ...
                    tryGetFromYaml(
                        measErrors.deweight_factor,
                        meas_deweighting,
                        {"! deweight_factor"},
                        "Factor to downweight the variance of measurements with statistically "
                        "detected errors"
                    );
                }

                {
                    auto state_deweighting = stringsToYamlObject(
                        model_error_handling,
                        {"0! state_deweighting"},
                        "Any \"state\" errors cause deweighting of all measurements that reference "
                        "the state"
                    );
                ...
                {
                    auto error_accumulation = stringsToYamlObject(
                        model_error_handling,
                        {"0! error_accumulation"},
                        "Any receivers or satellites that are consistently getting many "
                        "measurement rejections may be "
                        "reinitialiased"
                    );

                    tryGetFromYaml(
                        errorAccumulation.enable,
                        error_accumulation,
                        {"! enable"},
                        "Enable reinitialisation of receivers, satellites, or individual states "
                        "upon many rejections"
                    );
                    tryGetFromYaml(
                        errorAccumulation.receiver_error_count_threshold,
                        error_accumulation,
                        {"! receiver_error_count_threshold"},
                        "Number of referencing measurement errors for a receiver to be considered "
                        "as a receiver error "
                        "for a single epoch"
                    );
                    tryGetFromYaml(
                        errorAccumulation.receiver_error_epochs_threshold,
                        error_accumulation,
                        {"! receiver_error_epochs_threshold"},
                        "Number of consecutive epochs with receiver in error before it is removed "
                        "and reinitialised"
                    );
                    tryGetFromYaml(
                        errorAccumulation.satellite_error_count_threshold,
                        error_accumulation,
                        {"! satellite_error_count_threshold"},
                        "Number of referencing measurement errors for a satellite to be considered "
                        "as a satellite "
                        "error for a single epoch"
                    );
                    tryGetFromYaml(
                        errorAccumulation.satellite_error_epochs_threshold,
                        error_accumulation,
                        {"! satellite_error_epochs_threshold"},
                        "Number of consecutive epochs with satellite in error before it is "
                        "reinitialised using the "
                        "orbit_errors configs"
                    );
                    tryGetFromYaml(
                        errorAccumulation.state_error_count_threshold,
                        error_accumulation,
                        {"! state_error_count_threshold"},
                        "Number of referencing measurement errors for a state to be considered as "
                        "a state error for a "
                        "single epoch"
                    );
                }
                ...
                {
                    auto satellite_errors = stringsToYamlObject(
                        model_error_handling,
                        {"2@ satellite_errors"},
                        "Orbital states that are not consistent with measurements may be "
                        "reinitialised to allow for "
                        "dynamic maneuvers"
                    );

                    tryGetFromYaml(
                        satelliteErrors.enable,
                        satellite_errors,
                        {"@ enable"},
                        "Enable applying process noise impulses to satellites upon state errors"
                    );
                    tryGetFromYaml(
                        satelliteErrors.clk_proc_noise,
                        satellite_errors,
                        {"@ clk_process_noise"},
                        "Sigma to apply to satellite clock states as reinitialisation"
                    );
                    tryGetFromYaml(
                        satelliteErrors.pos_proc_noise,
                        satellite_errors,
                        {"@ pos_process_noise"},
                        "Sigma to apply to orbital position states as reinitialisation"
                    );
                    tryGetFromYaml(
                        satelliteErrors.vel_proc_noise,
                        satellite_errors,
                        {"@ vel_process_noise"},
                        "Sigma to apply to orbital velocity states as reinitialisation"
                    );
                    tryGetFromYaml(
                        satelliteErrors.vel_proc_noise_trail,
                        satellite_errors,
                        {"@ vel_process_noise_trail"},
                        "Initial sigma for exponentially decaying noise to apply for subsequent "
                        "epochs as soft "
                        "reinitialisation"
                    );
                    tryGetFromYaml(
                ...
                {
                    auto ambiguities = stringsToYamlObject(
                        model_error_handling,
                        {"1! ambiguities"},
                        "Cycle slips in ambiguities are primary cause of incorrect gnss modelling "
                        "and may be "
                        "reinitialised"
                    );

                    tryGetFromYaml(
                        ambErrors.phase_reject_limit,
                        ambiguities,
                        {"! phase_reject_limit"},
                        "Maximum number of phase measurements to reject before the ambiguity "
                        "associated with the "
                        "measurement is reset."
                    );

                    tryGetFromYaml(
                        ambErrors.resetOnSlip.LLI,
                        ambiguities,
                        {"@ reset_on", "@ lli"},
                        "Reset ambiguities if LLI   test is detecting a slip"
                    );
                    tryGetFromYaml(
                        ambErrors.resetOnSlip.retrack,
                        ambiguities,
                        {"@ reset_on", "@ retrack"},
                        "Reset ambiguities on retrack   test is detecting a slip"
                    );
                    tryGetFromYaml(
                        ambErrors.resetOnSlip.GF,
                        ambiguities,
                        {"@ reset_on", "@ gf"},
                        "Reset ambiguities if GF    test is detecting a slip"
                    );
                    tryGetFromYaml(
                        ambErrors.resetOnSlip.MW,
                        ambiguities,
                        {"@ reset_on", "@ mw"},
                        "Reset ambiguities if MW    test is detecting a slip"
                    );
                    tryGetFromYaml(
                        ambErrors.resetOnSlip.SCDIA,
                        ambiguities,
                        {"@ reset_on", "@ scdia"},
                        "Reset ambiguities if SCDIA test is detecting a slip"
                    );
                    tryGetFromYaml(
                        ambErrors.resetOnSlip.single_freq,
                        ambiguities,
                        {"@ reset_on", "@ single_freq"},
                        "Reset ambiguities on signle frequency detection"
                    );
                }

                {
                    auto ionospheric_components =
                        stringsToYamlObject(model_error_handling, {"1! ionospheric_components"});

                    tryGetFromYaml(
                        ionErrors.outage_reset_limit,
                        ionospheric_components,
                        {"! outage_reset_limit"},
                        "Maximum number of seconds without measurements before the ionosphere "
                        "associated with the "
                        "measurement is reset."
                    );
                }

                {
                    auto exclusions = stringsToYamlObject(
                        model_error_handling,
                        {"1@ exclusions"},
                        "Cycle slips may be detected by the preprocessor and measurements rejected "
                        "or ambiguities "
                        "reinitialised"
                    );
                    ...
                    tryGetFromYaml(
                        ...
                        exclusions,
                        {"@ bad_spp"},
                        "Exclude measurements that were associated with failed SPP"
                    );
                    tryGetFromYaml(
                        exclude.config,
                        exclusions,
                        {"@ config"},
                        "Exclude measurements that are configured as exclusions"
                    );
                    tryGetFromYaml(
                        exclude.eclipse,
                        exclusions,
                        {"@ eclipse"},
                        "Exclude measurements that are in eclipse"
                    );
                    tryGetFromYaml(
                        exclude.elevation,
                        exclusions,
                        {"@ elevation"},
                        "Exclude measurements that fall below elevation mask"
                    );
                    tryGetFromYaml(
                        exclude.outlier,
                        exclusions,
                        {"@ outlier"},
                        "Exclude measurements that were rejected as SPP outliers"
                    );
                    tryGetFromYaml(
                        exclude.system,
                        exclusions,
                        {"@ system"},
                        "Exclude measurements that have been excluded by system configs"
                    );
                    tryGetFromYaml(
                        exclude.svh,
                        exclusions,
                        {"@ svh"},
                        "Exclude measurements that are not specified as healthy"
                    );
                    tryGetFromYaml(
                        exclude.LLI,
                        exclusions,
                        {"@ lli"},
                        "Exclude measurements that fail LLI slip test in preprocessor"
                    );
                    tryGetFromYaml(
                        exclude.retrack,
                        exclusions,
                        {"@ retrack"},
                        "Exclude measurements that fail retrack slip test in preprocessor"
                    );
                    tryGetFromYaml(
                        exclude.single_freq,
                        exclusions,
                        {"@ single_freq"},
                        "Exclude measurements on signle frequency"
                    );
                    tryGetFromYaml(
                        exclude.GF,
                        exclusions,
                        {"@ gf"},
                        "Exclude measurements that fail GF  slip test in preprocessor"
                    );
                    tryGetFromYaml(
                        exclude.MW,
                        exclusions,
                        {"@ mw"},
                        "Exclude measurements that fail MW  slip test in preprocessor"
                    );
                    tryGetFromYaml(
                        exclude.SCDIA,
                        exclusions,
                        {"@ scdia"},
                        "Exclude measurements that fail SCDIA    test in preprocessor"
                    );
                }

                // 				{
                // 					auto clocks = stringsToYamlObject(model_error_handling, {"@
                // clocks"}, "Error responses specific to clock states");
                //
                // 					tryGetFromYaml(reinit_on_clock_error,		clocks,			{"@
                // reinit_on_clock_error"	}, "Any clock \"state\" errors cause removal and
                // reinitialisation of the clocks and all associated ambiguities");
                // 				}
            }

            auto getFilterOptions = [&](NodeStack& nodeStack, FilterOptions& filterOpts)
            {
                tryGetEnumOpt(
                    filterOpts.lsq_inverter,
                    nodeStack,
                    {"@ lsq_inverter"},
                    "Inverter to be used within the least squares estimater, which may provide "
...

                {
                    ...
                    tryGetFromYaml(
                        filterOpts.advanced_postfits,
                        nodeStack,
                        {"# advanced_postfits"},
                        "Use alternate calculation method to determine postfit residuals"
                    );
                }

                {
                    ...
                    tryGetFromYaml(
                        filterOpts.prefitOpts.max_iterations,
                        prefit,
                        {"! max_iterations"},
                        "Maximum number of measurements to exclude using prefit checks before "
                        "attempting to filter"
                    );
                    tryGetFromYaml(
                        filterOpts.prefitOpts.sigma_check,
                        prefit,
                        {"@ sigma_check"},
                        "Enable sigma check"
                    );
                    tryGetFromYaml(
                        filterOpts.prefitOpts.omega_test,
                        prefit,
                        {"@ omega_test"},
                        "Enable omega-test"
                    );
                    bool found = tryGetFromYaml(
                        filterOpts.prefitOpts.state_sigma_threshold,
                        prefit,
                        {"@ sigma_threshold"},
                        "Sigma threshold"
                    );
                    tryGetFromYaml(
                        filterOpts.prefitOpts.meas_sigma_threshold,
                        prefit,
                        {"@ sigma_threshold"},
                        "Sigma threshold"
                    );
...

                {
                    ...
                    tryGetFromYaml(
                        filterOpts.postfitOpts.max_iterations,
                        postfit,
                        {"! max_iterations"},
                        "Maximum number of measurements to exclude using postfit checks while "
                        "iterating filter"
                    );
                    tryGetFromYaml(
                        filterOpts.postfitOpts.sigma_check,
                        postfit,
                        {"@ sigma_check"},
                        "Enable sigma check"
                    );
                    tryGetFromYaml(
                        filterOpts.postfitOpts.omega_test,
                        postfit,
                        {"@ omega_test"},
                        "Enable omega-test"
                    );
                    bool found = tryGetFromYaml(
                        filterOpts.postfitOpts.state_sigma_threshold,
                        postfit,
                        {"@ sigma_threshold"},
                        "Sigma threshold"
                    );
                    tryGetFromYaml(
                        filterOpts.postfitOpts.meas_sigma_threshold,
                        postfit,
                        {"@ sigma_threshold"},
                        "Sigma threshold"
                    );
...

                {
                    auto rts = stringsToYamlObject(
                        nodeStack,
                        {"@ rts"},
                        "RTS allows reverse smoothing of estimates such that early estimates can "
                        "make use of later "
                        "data."
                    );
                    ...
                    tryGetFromYaml(
                        ...
                        rts,
                        {"0!  enable"},
                        "Perform backward smoothing of states to improve precision of earlier "
                        "states"
                    );
                    tryGetFromYaml(
                        filterOpts.rts_lag,
                        rts,
                        {"@ 1 lag"},
                        "Number of epochs to use in RTS smoothing. Negative numbers indicate full "
                        "reverse smoothing."
                    );
                    tryGetFromYaml(
                        filterOpts.rts_interval,
                        rts,
                        {"# interval"},
                        "Number of seconds to use between fixed lag in RTS smoothing."
                    );
                    conditionalPrefix(
                        "<OUTPUTS_ROOT>",
                        pppOpts.rts_directory,
                        tryGetFromYaml(
                            filterOpts.rts_directory,
                            rts,
                            {"@ directory"},
                            "Directory for rts intermediate files"
                        )
                    );
                    conditionalPrefix(
                        "<RTS_DIRECTORY>",
                        pppOpts.rts_filename,
                        tryGetFromYaml(
                            filterOpts.rts_filename,
                            rts,
                            {"@ filename"},
                            "Base filename for rts intermediate files"
                        )
                    );
                    tryGetFromYaml(
                        filterOpts.queue_rts_outputs,
                        rts,
                        {"@ queue_outputs"},
                        "Queue rts outputs so that processing is not limited by IO bandwidth"
                    );
                    tryGetFromYaml(
                        filterOpts.rts_smoothed_suffix,
                        rts,
                        {"@ suffix"},
            ...
            // 			minimum_constraints
            {
                auto minimum_constraints = stringsToYamlObject(
                    processing_options,
                    {"5! minimum_constraints"},
                    "Receiver coodinates may be aligned to reference frames with minimal external "
                    "constraints"
                );
                ...
                tryGetKalmanFromYaml(
                    minconOpts.delay_rate,
                    minimum_constraints,
                    "1! delay_rate",
                    "Estimation and application of clock delay adjustment rate"
                );
                tryGetKalmanFromYaml(
                    minconOpts.scale_rate,
                    minimum_constraints,
                    "1! scale_rate",
                    "Estimation and application of scaling factor rate"
                );
                tryGetKalmanFromYaml(
                    minconOpts.rotation_rate,
                    minimum_constraints,
                    "1! rotation_rate",
                    "Estimation and application of angular offsets rate"
                );
                tryGetKalmanFromYaml(
                    minconOpts.translation_rate,
                    minimum_constraints,
                    "1! translation_rate",
                    "Estimation and application of CoG offsets rate"
                );

                tryGetFromYaml(
                    minconOpts.once_per_epoch,
                    minimum_constraints,
                    {"2@ once_per_epoch"},
                    "Perform minimum constraints on a temporary filter and output results once per "
                    "epoch"
                );
                tryGetFromYaml(
                    minconOpts.full_vcv,
                    minimum_constraints,
                    {"2@ full_vcv"},
                    "! experimental ! Use full VCV for measurement noise in minimum constraints "
                    "filter"
                );
                tryGetFromYaml(
                    minconOpts.constrain_orbits,
                    minimum_constraints,
                    {"2@ constrain_orbits"},
                    "Enforce rigid transformations of orbital states"
                );
                tryGetEnumOpt(
                    minconOpts.application_mode,
                    minimum_constraints,
                    {"2@ application_mode"},
                    "Method of transforming positions "
                );
                tryGetFromYaml(
                    minconOpts.transform_unweighted,
                    minimum_constraints,
                    {"2@ transform_unweighted"},
                    "Add design entries for transformation of positions without weighting"
                );
            ...
            // 			ppp_filter
            {
                auto ppp_filter = stringsToYamlObject(
                    processing_options,
                    {"4! ppp_filter"},
                ...
                tryGetFromYaml(
                    pppOpts.simulate_filter_only,
                    ppp_filter,
                    {"@ simulate_filter_only"},
                    "Residuals will be calculated, but no adjustments to state or covariances will "
                    "be applied"
                );
                tryGetFromYaml(
                    pppOpts.assume_linearity,
                    ppp_filter,
                    {"@ assume_linearity"},
                    "Residuals will be adjusted during measurement combination rather than "
                    "performing 2 seperate state "
                    "transitions"
                );
                ...
                tryGetFromYaml(
                    ...
                    ppp_filter,
                    {"@ chunking", "@ by_receiver"},
                    "Split large filter and measurement matrices blockwise by receiver ID to "
                    "improve processing speed"
                );

                tryGetFromYaml(
                    pppOpts.filter_reset_enable,
                    ppp_filter,
                    {"@ periodic_reset", "@ enable"},
                    "Enable periodic reset of filter states"
                );
                tryGetFromYaml(
                    pppOpts.reset_interval,
                    ppp_filter,
                    {"@ periodic_reset", "@ interval"},
                    "Interval between reset of filter states"
                );
                tryGetFromYaml(
                    pppOpts.reset_epochs,
                    ppp_filter,
                    {"@ periodic_reset", "@ times"},
                    "Time to reset filter states in seconds of day [GPST]"
                );
                tryGetEnumVec(
                    pppOpts.reset_states,
                    ppp_filter,
                    {"@ periodic_reset", "@ states"},
                    "States to remove for periodic reset"
                );

                // 				ionospheric_component
                {
                    auto ionospheric_components = stringsToYamlObject(
                        ppp_filter,
                        {"! ionospheric_components"},
                    ...
                    tryGetEnumOpt(
                        pppOpts.ionoOpts.corr_mode,
                        ionospheric_components,
                        {"@ corr_mode"}
                    );
                    tryGetFromYaml(
                        pppOpts.ionoOpts.common_ionosphere,
                        ionospheric_components,
                        {"! common_ionosphere"},
            ...
            // 			ion_filter
            {
                auto ion_filter = stringsToYamlObject(
                    processing_options,
                    {"5@ ion_filter"},
                    "Configurations for the ionospheric model kalman filter and its sub processes"
                );

                tryGetEnumOpt(ionModelOpts.model, ion_filter, {"@ model"});
                tryGetFromYaml(
                    ionModelOpts.function_order,
                    ion_filter,
                    {"@ function_order"},
                    "Maximum order  of Spherical harmonics for Ionospheric mapping"
                );
                tryGetFromYaml(
                    ionModelOpts.function_degree,
                    ion_filter,
                    {"@ function_degree"},
                    "Maximum degree of Spherical harmonics for Ionospheric mapping"
                );
                tryGetFromYaml(
                    ionModelOpts.estimate_sat_dcb,
                    ion_filter,
                    {"@ estimate_sat_dcb"},
                    "Estimate satellite dcb alongside Ionosphere models, should be false for local "
                    "STEC"
                );
                tryGetFromYaml(
                    ionModelOpts.use_rotation_mtx,
                    ion_filter,
                    {"@ use_rotation_mtx"},
                    "Use 3D rotation matrix for spherical harmonics to maintain orientation toward "
                    "the sun"
                );
                tryGetFromYaml(
                    ionModelOpts.basis_sigma_limit,
                    ion_filter,
                    {"@ model_sigma_limit"},
                    "Ionosphere states are removed when their sigma exceeds this value"
                );

                bool found = tryGetFromYaml(
                    ionModelOpts.layer_heights,
                    ion_filter,
                    {"@ layer_heights"},
                    "List of heights of ionosphere layers to estimate"
                );
                if (found)
            ...
            // 			preprocessor
            {
                auto preprocessor = stringsToYamlObject(
                    processing_options,
                    {"1@ preprocessor"},
                    "Configurations for the kalman filter and its sub processes"
                );
                ...
                tryGetFromYaml(
                    preprocOpts.preprocess_all_data,
                    preprocessor,
                    {"@ preprocess_all_data"}
                );
                {
                    auto cycle_slips = stringsToYamlObject(
                        preprocessor,
                        {"2@ cycle_slips"},
                        "Cycle slips may be detected by the preprocessor and measurements rejected "
                        "or ambiguities "
                    ...
                    tryGetFromYaml(
                        ...
                        cycle_slips,
                        {"@ mw_process_noise"},
                        "Process noise applied to filtered Melbourne-Wubenna measurements to "
                        "detect cycle slips"
                    );
                }
            }
            ...
            // 			ambiguity_resolution
            {
                auto ambiguity_resolution =
                    stringsToYamlObject(processing_options, {"5@ ambiguity_resolution"});

                tryGetFromYaml(
                    ambrOpts.elevation_mask_deg,
                    ambiguity_resolution,
                    {"@ elevation_mask"},
                    "Minimum satellite elevation to perform ambiguity resolution"
                );
                tryGetFromYaml(
                    ambrOpts.lambda_set,
                    ambiguity_resolution,
                    {"@ lambda_set_size"},
                    "Maximum numer of candidate sets to be used in lambda_alt2 and lambda_bie modes"
                );
                tryGetFromYaml(
                    ambrOpts.AR_max_itr,
                    ambiguity_resolution,
                    {"@ max_rounding_iterations"},
                    "Maximum number of rounding iterations performed in iter_rnd and bootst modes"
                );
                ...
                tryGetFromYaml(
                    ...
                    "Thresold for integer validation, success rate test."
                );
                tryGetFromYaml(
                    ambrOpts.ratioThres,
                    ambiguity_resolution,
                    {"@ solution_ratio_threshold"},
                    "Thresold for integer validation, distance ratio test."
                );

                tryGetFromYaml(
                    ambrOpts.once_per_epoch,
                    ambiguity_resolution,
                    {"@ once_per_epoch"},
                    "Perform ambiguity resolution on a temporary filter and output results once "
                    "per epoch"
                );
                tryGetFromYaml(
                    ambrOpts.fix_and_hold,
                    ambiguity_resolution,
                    {"@ fix_and_hold"},
            ...
            // 			orbit_propagation
            {
                auto orbit_propagation =
                    stringsToYamlObject(processing_options, {"5@ orbit_propagation"});

                tryGetFromYaml(
                    propagationOptions.integrator_time_step,
                    orbit_propagation,
                    {"@ integrator_time_step"},
                    "Timestep for the integrator, must be smaller than the processing time step, "
                    "might be adjusted if "
                    "the processing time step isn't a integer number of time steps"
                );
                tryGetFromYaml(
                    propagationOptions.egm_degree,
                    orbit_propagation,
                    {"@ egm_degree"},
                    "Degree of spherical harmonics gravity model"
                );
                tryGetFromYaml(
                    propagationOptions.indirect_J2,
                    orbit_propagation,
                    {"@ indirect_J2"},
                    "J2 acceleration perturbation due to the Sun and Moon"
                );
                tryGetFromYaml(
                    propagationOptions.egm_field,
                    orbit_propagation,
                    {"@ egm_field"},
                    "Acceleration due to the high degree model of the Earth gravity model (exclude "
                    "degree 0, made by "
                    "central_force)"
                );
                tryGetFromYaml(
                    propagationOptions.solid_earth_tide,
                    orbit_propagation,
                    {"@ solid_earth_tide"},
                    "Model accelerations due to solid earth tides"
                );
                tryGetFromYaml(
                    propagationOptions.ocean_tide,
                    orbit_propagation,
                    {"@ ocean_tide"},
                    "Model accelerations due to ocean tides model"
                );
                tryGetFromYaml(
                    propagationOptions.atm_tide,
                    orbit_propagation,
                    {"@ atm_tide"},
                    "Model accelerations due to atmospheric tides model"
                );
                tryGetFromYaml(
                    propagationOptions.pole_tide_ocean,
                    orbit_propagation,
                    {"@ pole_tide_ocean"},
                    "Model accelerations due to ocean pole tide (degree 2 only)"
                );
                tryGetFromYaml(
                    propagationOptions.pole_tide_solid,
                    orbit_propagation,
                    {"@ pole_tide_solid"},
                    "Model accelerations due to solid pole tide (degree 2 only)"
                );
                tryGetFromYaml(
                    propagationOptions.aod,
                    orbit_propagation,
                    {"@ aod"},
                    "Model Atmospheric and Oceanic non tidal accelerations"
                );
                tryGetFromYaml(
                    propagationOptions.central_force,
                    orbit_propagation,
                    {"@ central_force"},
                    "Acceleration due to the central force"
                );
                tryGetFromYaml(
                    propagationOptions.general_relativity,
                    orbit_propagation,
                    {"@ general_relativity"},
                    "Model acceleration due general relativisty"
                );
            }
            ...
            // other
            {
                ...
                tryGetFromAny(
                    ...
                    processing_options,
                    {"user_aliases"},
                    "User definable alias pairs, eg 'myuser:mypass' (use quotes) will replace "
                    "instances of <MYUSER> "
                    "with mypass in relevant filenames"
                );
        ...
        {
            auto estimation_parameters =
                stringsToYamlObject({yaml, ""}, {estimation_parameters_str});
        ...
        // 		mongo
        {
            auto mongo = stringsToYamlObject(
                {yaml, ""},
                {"5!  mongo"},
                "Mongo is a database used to store results and intermediate values for later "
                "analysis and "
            ...
            tryGetEnumOpt(
                ...
                mongo,
                {"0! enable"},
                "Enable and connect to mongo database"
            );
            tryGetEnumOpt(
                mongoOpts.output_measurements,
                mongo,
                {"1! output_measurements"},
                "Output measurements and their residuals"
            );
            tryGetEnumOpt(
                mongoOpts.output_components,
                mongo,
                {"1! output_components"},
                "Output components of measurements"
            );
            tryGetEnumOpt(
                mongoOpts.output_cumulative,
                mongo,
                {"1! output_cumulative"},
                "Output cumulative residuals of components of measurements"
            );
            tryGetEnumOpt(mongoOpts.output_states, mongo, {"1! output_states"}, "Output states");
            tryGetEnumOpt(
                mongoOpts.output_state_covars,
                mongo,
                {"1! output_state_covars"},
                "Output covariance values of related states"
            );
            ...
            tryGetEnumOpt(
                mongoOpts.output_test_stats,
                mongo,
                {"2@ output_test_stats"},
                "Output test statistics"
            );
            tryGetEnumOpt(
                mongoOpts.output_logs,
                mongo,
                {"2@ output_logs"},
            ...
            tryGetEnumOpt(
                ...
                "Output orbits, clocks, and bias estimates to allow communication to ssr "
                "generating processes"
            );
            tryGetEnumOpt(
                mongoOpts.delete_history,
                mongo,
                {"1! delete_history"},
                "Drop the collection in the database at the beginning of the run to only show "
                "fresh data"
            );
            tryGetEnumOpt(
                ...
                mongo,
                {"1@ cull_history"},
                "Erase old database objects to limit the size and speed degredation over long runs"
            );
            ...
            tryGetFromYaml(
                mongoOpts.queue_outputs,
                mongo,
                {"2@ queue_outputs"},
                "Output data in a separate thread - may reduce latency"
            );
            tryGetFromYaml(
                mongoOpts.min_cull_age,
                mongo,
                {"2@ min_cull_age"},
                "Age of which to cull history"
            );

            tryGetEnumVec(
                mongoOpts.used_predictions,
                mongo,
                {"@ used_predictions"},
                "Filter states to retrieve from mongo"
            );
            tryGetEnumVec(
                mongoOpts.sent_predictions,
                mongo,
                {"@ sent_predictions"},
                "Filter states to predict and send to mongo"
            );
            ...
            tryGetFromYaml(
                mongoOpts[static_cast<int>(E_Mongo::PRIMARY)].database,
                mongo,
                {"3@ primary_database"}
            );
            tryGetFromYaml(
        ...
        // 		debug
        {
            auto debug = stringsToYamlObject(
                {yaml, ""},
                {"9@ debug"},
                "Debug options are designed for developers and should probably not be used by "
                "normal users"
            );
            ...
            tryGetFromAny(
                ...
                debug,
                {"# fatal_message_level"},
                "Threshold level for exiting the program early (0-2)"
            );
            tryGetFromYaml(
                check_plumbing,
                debug,
                {"# check_plumbing"},
                "Debugging option to show sizes of objects in memory to detect leaks"
            );
            tryGetFromYaml(
                explain_measurements,
                debug,
                {"# explain_measurements"},
                "Debugging option to show verbose measurement coefficients"
            );
            tryGetFromYaml(
                retain_rts_files,
                debug,
                {"# retain_rts_files"},
                "Debugging option to keep rts files for post processing"
            );
            tryGetFromYaml(
                rts_only,
                debug,
                {"# rts_only"},
                "Debugging option to only re-run rts from previous run"
            );
            tryGetFromYaml(
                mincon_only,
                debug,
                {"# mincon_only"},
                "Debugging option to re-run minimum constraints code"
            );
            tryGetFromYaml(
                output_mincon,
                debug,
                {"# output_mincon"},
                "Debugging option to only save pre-minimum constraints filter state"
            );
            tryGetFromYaml(
                mincon_filename,
                debug,
                {"# mincon_filename"},
                "Filename of pre-mincon filter state for backup/loading"
            );
    ...
    // 	Try to change all filenames to replace <YYYY> etc with other values.
    {
        ...
        globber(egm_files);
        replaceTags(igrf_files);
        globber(igrf_files);
        replaceTags(hfeop_files);
        globber(hfeop_files);
        replaceTags(gpt2grid_files);
        globber(gpt2grid_files);
        replaceTags(orography_files);
        globber(orography_files);
        replaceTags(atm_reg_definitions);
        globber(atm_reg_definitions);
        replaceTags(pseudo_filter_files);
        globber(pseudo_filter_files);
        replaceTags(space_weather_files);
        globber(space_weather_files);
        replaceTags(planetary_ephemeris_files);
        globber(planetary_ephemeris_files);
        replaceTags(ocean_tide_potential_files);
        globber(ocean_tide_potential_files);
        replaceTags(atmos_tide_potential_files);
        globber(atmos_tide_potential_files);
        replaceTags(ocean_tide_loading_blq_files);
        globber(ocean_tide_loading_blq_files);
        replaceTags(atmos_tide_loading_blq_files);
        globber(atmos_tide_loading_blq_files);
        replaceTags(ocean_pole_tide_loading_files);
        globber(ocean_pole_tide_loading_files);
        replaceTags(atmos_ocean_dealiasing_files);
        globber(atmos_ocean_dealiasing_files);
        replaceTags(ocean_pole_tide_potential_files);
        globber(ocean_pole_tide_potential_files);
        ...
        replaceTags(orbit_ics_directory);
        replaceTags(orbit_ics_filename);
        replaceTags(ntrip_log_directory);
        replaceTags(ntrip_log_filename);
        ...
        replaceTags(raw_custom_directory);
        replaceTags(raw_custom_filename);
        ...
        replaceTags(sp3_directory);
        replaceTags(predicted_sp3_filename);
        replaceTags(ems_directory);
        replaceTags(ems_filename);
        replaceTags(trace_directory);
        replaceTags(receiver_json_filename);
        replaceTags(trace_directory);
        replaceTags(receiver_trace_filename);
        replaceTags(trace_directory);
        replaceTags(network_trace_filename);
        replaceTags(trace_directory);
        replaceTags(satellite_trace_filename);
        replaceTags(trace_directory);
        replaceTags(ionosphere_trace_filename);
