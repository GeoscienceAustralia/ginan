...
bool satPosKalman(Trace& trace, GTime time, SatPos& satPos, const KFState* kfState_ptr)
{
    ...
    if (satNav.satPos0.rSatEci0.isZero() == false && satNav.satPos0.vSatEci0.isZero() == false &&
    ...
    else
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            bool found =
                (kfKey.num = i,
                 kfState.getKFValue(kfKey, rSat0(i), &dummy, &dummy, false) != E_Source::NONE) &&
                (kfKey.num = i + 3,
                 kfState.getKFValue(kfKey, vSat0(i), &dummy, &dummy, false) != E_Source::NONE);
    ...
    if (rSat0.isZero() == false && vSat0.isZero() == false)
    {
