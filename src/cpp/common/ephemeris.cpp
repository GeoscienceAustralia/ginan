    ...
    for (auto& ephType : ephTypes)
    {
        ...
        switch (ephType)
        {
            ...
            case E_Source::PRECISE:
                returnValue = satClkPrecise(trace, time, satPos, nav);
                break;
            case E_Source::KALMAN:
                returnValue = satClkKalman(trace, time, satPos, kfState_ptr);
                break;
            case E_Source::REMOTE:
                returnValue = satClkKalman(trace, time, satPos, remote_ptr);
                break;
            default:
                continue;
        }
...
)
{
    ...
    for (auto& ephType : ephTypes)
    {
        tracepdeex(
            4,
            trace,
            "\n%-10s: time=%s sat=%s ephType=%s offsetType=%d",
            __FUNCTION__,
            time.to_string().c_str(),
            satPos.Sat.id().c_str(),
        ...
        if (returnValue == false)
            switch (ephType)
            {
                ...
                case E_Source::SSR:
                    ...
                    break;
                case E_Source::PRECISE:
                    returnValue = satPosPrecise(trace, time, satPos, nav);
                    break;
                case E_Source::KALMAN:
                    returnValue = satPosKalman(trace, time, satPos, kfState_ptr);
                    break;
                case E_Source::REMOTE:
                    returnValue = satPosKalman(trace, time, satPos, remote_ptr);
                    break;
    ...
    if (antennaScalar)
    {
        ...
        if (attStatus.eXBody.isZero() || attStatus.eYBody.isZero() || attStatus.eZBody.isZero())
        {
            BOOST_LOG_TRIVIAL(debug) << "Satellite attitude of " << satPos.Sat.id()
                                     << " not available, antenna offset not corrected.";
            return returnValue;
        }
        ...
        if (satPos.satNav_ptr->lamMap.empty() || satPos.satNav_ptr->lamMap[j] == 0 ||
        ...
        if (acsConfig.common_sat_pco)
        {
            ...
            Vector3d bodyPCO =
                antPco(satPos.Sat.id(), satPos.Sat.sys, j, time, varDummy, E_Radio::TRANSMITTER);
        ...
        if (antennaScalar > 0)
            satPos.rSatApc = satPos.rSatCom + dAnt;
        if (antennaScalar < 0)
            satPos.rSatCom = satPos.rSatApc - dAnt;
    }
...
)
{
    ...
    tracepdeex(
        3,
        trace,
        "\n%s sat=%s rs=%13.3f %13.3f %13.3f dtSat=%12.3f varPos=%7.3f varClk=%7.3f "
        "ephPosValid=%1X %s ephClkValid=%1X "
        "%s",
        obs.time.to_string().c_str(),
        obs.Sat.id().c_str(),
        obs.rSatCom[0],
