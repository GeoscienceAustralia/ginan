...
void minimumTest(Trace& trace)
{
    ...
    // generate fake station/orbit data
    {
        ...
        for (auto orbit : {/*true, */ false})
            for (auto [id, a] : pointMap)
            {
                ...
                Vector3d v = posEci.cross(Vector3d::UnitZ()) / 10000 +
                             stationEntries.size() * Vector3d::UnitZ() * 10;
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (orbit)
                        meas.setValue(posEci(i));
                    else
                        meas.setValue(posEcef(i));
    ...
    // 	kfStateStations.P(45,30) = 1.9;			//this fails rigidity test when the orbits
    // constrained is false 	kfStateStations.P(13,13) = 2; 	kfStateStations.P(45,45) = 2;
    ...
    for (auto type : {KF::REC_POS, KF::ORBIT})
        for (auto [kfKey, index] : kfStateStations.kfIndexMap)
        {
            ...
            Vector3d point1 =
                kfStateStations.x.segment(index, 3) + pointMap["STN" + std::to_string(index / 3)];
            ...
            for (auto [kfKey, index] : kfStateStations.kfIndexMap)
            {
                ...
                Vector3d point2 = kfStateStations.x.segment(index, 3) +
                                  pointMap["STN" + std::to_string(index / 3)];
 ...
 * The pseudo-linear state transition in the filter (STM + adjustment) is mathematically equivalent
to setting a state value directly,
 * but numerical precision in a computer does not allow 100% correspondence - this checks its mostly
working
// */
// void checkOrbits(
//	Trace&				trace,
...
//							- subState.x.head(6);
//
//		trace << "\norbitPropagator.base.inertialState"		<< inertialState
//.transpose().format(HeavyFmt); 		trace << "\nsubState.x.head(6)                "		<<
// subState.x.head(6)	.transpose().format(HeavyFmt); 		trace << "\ndeltaState "		<<
// deltaState .transpose().format(HeavyFmt);
//
//  		MatrixXd transition = orbitPropagator.states.posVelSTM;
//
//  		//Convert the absolute transition matrix to an identity matrix (already populated
//  elsewhere) and stm-per-time matrix 		transition -= MatrixXd::Identity(transition.rows(),
//  transition.cols());
//
...
// 	MatrixXd transition = orbitPropagator.states.posVelSTM;
//
// 	//Convert the absolute transition matrix to an identity matrix (already populated elsewhere) and
// stm-per-time matrix 	transition -= MatrixXd::Identity(transition.rows(), transition.cols());
// 	transition /= 900;
//
// 	MatrixXd thingy = transition * subState.P * transition.transpose();
//
...
// 						continue;
// 					}
// 					case KF::SRP_SCALE:		{	acceleration = outputsMap["accSrp"];	break;	}
// //this should be whatever was used in the propagator above, for this srp component only
// 				}
//
...
void debugTime()
{
    ...
    std::cout << "GTime to GEpoch:    " << (int)gEpoch.year << "-" << std::setw(2)
              << (int)gEpoch.month << "-" << std::setw(2) << (int)gEpoch.day << " " << std::setw(2)
              << (int)gEpoch.hour << ":" << std::setw(2) << (int)gEpoch.min << ":" << std::setw(4)
              << gEpoch.sec << "\n";
    ...
    time2epoch(gTime, ep, E_TimeSys::GPST);
    std::cout << "GTime to GPST     epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochGPS = epoch2time(ep, E_TimeSys::GPST);
    std::cout << "GPST     epoch to GTime: " << gTimeEpochGPS.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::GLONASST);
    std::cout << "GTime to GLONASST epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochGLO = epoch2time(ep, E_TimeSys::GLONASST);
    std::cout << "GLONASST epoch to GTime: " << gTimeEpochGLO.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::GST);
    std::cout << "GTime to GST      epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochGST = epoch2time(ep, E_TimeSys::GST);
    std::cout << "GST      epoch to GTime: " << gTimeEpochGST.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::BDT);
    std::cout << "GTime to BDT      epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochBDT = epoch2time(ep, E_TimeSys::BDT);
    std::cout << "BDT      epoch to GTime: " << gTimeEpochBDT.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::QZSST);
    std::cout << "GTime to QZSST    epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochQZS = epoch2time(ep, E_TimeSys::QZSST);
    std::cout << "QZSST    epoch to GTime: " << gTimeEpochQZS.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::TAI);
    std::cout << "GTime to TAI      epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochTAI = epoch2time(ep, E_TimeSys::TAI);
    std::cout << "TAI      epoch to GTime: " << gTimeEpochTAI.to_string(1) << "\n";
    time2epoch(gTime, ep, E_TimeSys::UTC);
    std::cout << "GTime to UTC      epoch: " << (int)ep[0] << "-" << std::setw(2) << (int)ep[1]
              << "-" << std::setw(2) << (int)ep[2] << " " << std::setw(2) << (int)ep[3] << ":"
              << std::setw(2) << (int)ep[4] << ":" << std::setw(4) << ep[5] << "\n";
    GTime gTimeEpochUTC = epoch2time(ep, E_TimeSys::UTC);
    std::cout << "UTC      epoch to GTime: " << gTimeEpochUTC.to_string(1) << "\n";

    double yds[6];

    std::cout << "\n";
    std::cout << "GTime to yds to GTime:" << "\n";
    time2yds(gTime, yds, E_TimeSys::GPST);
    std::cout << "GTime to GPST     yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsGPS = yds2time(yds, E_TimeSys::GPST);
    std::cout << "GPST     yds to GTime: " << gTimeYdsGPS.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::GLONASST);
    std::cout << "GTime to GLONASST yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsGLO = yds2time(yds, E_TimeSys::GLONASST);
    std::cout << "GLONASST yds to GTime: " << gTimeYdsGLO.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::GST);
    std::cout << "GTime to GST      yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsGST = yds2time(yds, E_TimeSys::GST);
    std::cout << "GST      yds to GTime: " << gTimeYdsGST.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::BDT);
    std::cout << "GTime to BDT      yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsBDT = yds2time(yds, E_TimeSys::BDT);
    std::cout << "BDT      yds to GTime: " << gTimeYdsBDT.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::QZSST);
    std::cout << "GTime to QZSST    yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsQZS = yds2time(yds, E_TimeSys::QZSST);
    std::cout << "QZSST    yds to GTime: " << gTimeYdsQZS.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::TAI);
    std::cout << "GTime to TAI      yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsTAI = yds2time(yds, E_TimeSys::TAI);
    std::cout << "TAI      yds to GTime: " << gTimeYdsTAI.to_string(1) << "\n";
    time2yds(gTime, yds, E_TimeSys::UTC);
    std::cout << "GTime to UTC      yds: " << (int)yds[0] << " " << (int)yds[1] << " " << yds[2]
              << "\n";
    GTime gTimeYdsUTC = yds2time(yds, E_TimeSys::UTC);
    std::cout << "UTC      yds to GTime: " << gTimeYdsUTC.to_string(1) << "\n";
    ...
    std::cout << "nearGTow: " << std::setw(8) << nearGTow.val
              << "\tnearTime:              " << nearTime.to_string(1) << "\n";
    std::cout << "gTow:     " << std::setw(8) << gTow.val
              << "\tGTime(gTow, nearTime): " << gTimeG.to_string(1) << "\n";
    ...
    std::cout << "nearBTow: " << std::setw(8) << nearBTow.val
              << "\tnearTime:              " << nearTime.to_string(1) << "\n";
    std::cout << "bTow:     " << std::setw(8) << bTow.val
              << "\tGTime(bTow, nearTime): " << gTimeB.to_string(1) << "\n";
    ...
    std::cout << "nearSod:  " << std::setw(8) << nearYds.sod
              << "\tnearTime:              " << nearTime.to_string(1)
              << "\tnearTod:  " << nearTod.val << "\n";
    std::cout << "rTod:     " << std::setw(8) << rTod.val
              << "\tGTime(rTod, nearTime): " << gTimeR.to_string(1) << "\n";
...
void longDoubleTest()
{
    long double a;
    std::cout << "\n"
              << "This system uses " << sizeof(a) * 8
              << " bits for long doubles. (Hopefully that number is 128...)" << "\n";
}
...
void debugSSR(GTime t0, GTime targetTime, E_Sys sys, SsrOutMap& ssrOutMap)
{
    ...
    for (auto& Sat : getSysSats(sys))
    {
        ...
        posDeltaPass[1] = ssrPosDelta(t0, ephTime, obs, obs.satNav_ptr->receivedSSR,	dPos[1],
iodPos, iodEph, ephValidStart, ephValidStop); clkDeltaPass[1] = ssrClkDelta(t0, ephTime, obs,
obs.satNav_ptr->receivedSSR,	dClk[1], iodClk,			clkValidStart, clkValidStop);
            ...
            &&posDeltaPass[1] && clkDeltaPass[1])
        {
            ...
            std::cout	<< "Debugging ssr: "
                        ...
                        << "\tsat: "								<< Sat.id()
                        << "\tdecoded: "			<< std::setw(7)	<< dPos[1] .transpose()	<< "\t"
<< std::setw(8) << dClk[1]		<< "\t" << std::setw(8) << dClk[2]
                        << "\tencoded: "			<< std::setw(7)	<< dPos[0] .transpose()	<< "\t"
<< std::setw(8) << dClk[0]
                        << "\tencoded-decoded: "	<< std::setw(7)	<< dPosDiff.transpose()	<< "\t"
<< std::setw(8) << dClkDiff[1]
                                                    << "\n";
        }
    }
    ...
    for (auto& [sat, ssrOut] : ssrOutMap)
    {
        std::cout	<< "Straddle clks: "
                    ...
                    << "\tsat: "							<< sat.id()
                    << "\ttime[0]: "						<<
ssrOut.clkInput.vals[0].time.to_string(1)
                    ...
                    << "\ttime[1]: "						<<
ssrOut.clkInput.vals[1].time.to_string(1)
...
void debugIGRF()
{
    ...
    {
        Vector3d r = {
            -4.05205271694605e+06,
            4.21283598092691e+06,
            -2.54510460797403e+06
        };  // Cartesian - ALIC
        ...
        std::cout << "\n\tGeocentric pos: " << pos[0] * R2D << " " << pos[1] * R2D << " "
                  << pos[2] / 1000 << "\n";
        ...
        for (int year = 1981; year <= 2023; year++)
        {
            ...
            std::cout << std::setprecision(1) << std::fixed;
            std::cout << "\tX: " << std::setw(8) << intensity.x() << "\tY: " << std::setw(8)
                      << intensity.y() << "\tZ: " << std::setw(8) << intensity.z() << "\n";
        }
    }
    ...
    // test 3.2 - grid (including singularity)
    {
        ...
        for (int lat = -90; lat <= 90; lat += 10)
            for (int lon = -180; lon <= 180; lon += 20)
            {
                ...
                std::cout << std::setprecision(1) << std::fixed;
                std::cout << "\tGeocentric pos: " << std::setw(5) << pos[0] * R2D << " "
                          << std::setw(6) << pos[1] * R2D << " " << std::setw(4) << pos[2] / 1000;
                std::cout << std::setprecision(1) << std::fixed;
                std::cout << "\tX: " << std::setw(8) << intensity.x() << "\tY: " << std::setw(8)
                          << intensity.y() << "\tZ: " << std::setw(8) << intensity.z() << "\n";
            }
...
void debugAttitude()
{
    ...
    for (int i = 0; i < nEpoch; i++)
    {
        ...
        printf(
            "%d %8.1f\t%13.3f %13.3f %13.3f\t%9.6f %9.6f %9.6f\t%9.6f %9.6f %9.6f\t%9.6f %9.6f "
            "%9.6f\t%9.6f %9.6f "
            ...
            rSat.x(),
            rSat.y(),
            rSat.z(),  // for GNSS satellites
            // rec.pos			.x(),	rec.pos			.y(),	rec.pos			.z(), 	// (not
            // ready) for LEO satellites
            attStatus.eXBody.x(),
            attStatus.eXBody.y(),
            attStatus.eXBody.z(),
            attStatus.eYBody.x(),
...
void debugBlq()
{
    ...
    for (auto& [wave, disp] : otlDisplacementMap[id])
    {
        ...
        for (int i = 0; i < 3; i++)
            std::cout << "\t" << std::setprecision(1) << std::setw(9) << disp.phase[i];
        std::cout << "\n";
    }
...
void debugTideOcean()
{
    ...
    for (auto& [id, dispTimeMap] : dispRefMap)
    {
        ...
        for (auto& [mjdval, denuRef] : dispTimeMap)
        {
            ...
            std::cout << std::setprecision(7) << std::fixed << "\t" << id << "\t"
                      << mjdUtc.to_double() << "\t" << std::setw(10) << denuRef.e() << "\t"
                      << std::setw(10) << denuRef.n() << "\t" << std::setw(10) << denuRef.u()
                      << "\t" << std::setw(10) << denu.e() << "\t" << std::setw(10) << denu.n()
                      << "\t" << std::setw(10) << denu.u() << "\t" << std::setw(10) << diff.e()
                      << "\t" << std::setw(10) << diff.n() << "\t" << std::setw(10) << diff.u()
                      << "\n";
        }
        std::cout << "\n";
    }
...
void debugTideAtmos()
{
    ...
    for (auto& [mjdval, denuRef] : dispRefMap)
    {
        ...
        std::cout << std::setprecision(7) << std::fixed << "\t" << id << "\t" << mjdUt1.to_double()
                  << "\t" << std::setw(10) << denuRef.e() << "\t" << std::setw(10) << denuRef.n()
                  << "\t" << std::setw(10) << denuRef.u() << "\t" << std::setw(10) << denu.e()
                  << "\t" << std::setw(10) << denu.n() << "\t" << std::setw(10) << denu.u() << "\t"
                  << std::setw(10) << diff.e() << "\t" << std::setw(10) << diff.n() << "\t"
                  << std::setw(10) << diff.u() << "\n";
    }
...
void debugTideSolid()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        std::cout << std::setprecision(7) << std::fixed << "\t" << mjdUt1.to_double() << "\t"
                  << std::setw(10) << dxyzRef[i].x() << "\t" << std::setw(10) << dxyzRef[i].y()
                  << "\t" << std::setw(10) << dxyzRef[i].z() << "\t" << std::setw(10) << dxyz.x()
                  << "\t" << std::setw(10) << dxyz.y() << "\t" << std::setw(10) << dxyz.z() << "\t"
                  << std::setw(10) << diff.x() << "\t" << std::setw(10) << diff.y() << "\t"
                  << std::setw(10) << diff.z() << "\n";
    }
}
...
void debugTideSolidPole()
{
    ...
    for (auto& [mjdval, dispRef] : dispRefMap)
    {
        ...
        std::cout << std::setprecision(7) << std::fixed << "\t" << mjdUtc.to_double();
        std::cout << std::setprecision(7) << std::scientific << "\t" << std::setw(10) << dispRef(2)
                  << "\t" << std::setw(10) << denu.u() << "\t" << std::setw(10) << diff << "\n";
    }
}
...
void debugTideOceanPole()
{
    ...
    // Test case in opoleloadcmcor.test, note that the mean pole model used is different from IERS
    // 2010 Conventions
    ...
    for (auto& [mjdval, denuRef] : dispRefMap)
    {
        ...
        std::cout << std::setprecision(7) << std::fixed << "\t" << mjdUtc.to_double()
                  << std::setprecision(7) << std::scientific << "\t" << std::setw(10) << denuRef.e()
                  << "\t" << std::setw(10) << denuRef.n() << "\t" << std::setw(10) << denuRef.u()
                  << "\t" << std::setw(10) << denu.e() << "\t" << std::setw(10) << denu.n() << "\t"
                  << std::setw(10) << denu.u() << "\t" << std::setw(10) << diff.e() << "\t"
                  << std::setw(10) << diff.n() << "\t" << std::setw(10) << diff.u() << "\n";
...
void perEpochPropTest(GTime time)
{
    ...
    pass = satPosPrecise(std::cout, newTime, satPosPrec, nav);
    std::cout << " precise passed: " << pass;
    pass = satPosKalman(std::cout, newTime, satPosProp, &copy);
    std::cout << " prop Passed: " << pass;
    // pass = satPosKalman	(std::cout, newTime,	satPosEllipse,	&kfState);
    // std::cout << " ellp Passed: "		<< pass;
    pass = satPosKalman(std::cout, newTime, satPosJ2, &kfState);
    std::cout << " j2 Passed: " << pass;

    ERPValues    erpv = getErp(nav.erp, newTime);
    FrameSwapper frameSwapper(newTime, erpv);
    satPosPrec.rSatEci0 =
        frameSwapper(satPosPrec.rSatCom, &satPosPrec.satVel, &satPosPrec.vSatEci0);

    Matrix3d E = ecef2rac(satPosPrec.rSatEci0, satPosPrec.vSatEci0);

    VectorEci differenceProp    = satPosProp.rSatEciDt - satPosPrec.rSatEci0;
    Vector3d  rtnProp           = E * differenceProp;
    VectorEci differenceEllipse = satPosEllipse.rSatEciDt - satPosPrec.rSatEci0;
    Vector3d  rtnEllipse        = E * differenceEllipse;
    VectorEci differenceJ2      = satPosJ2.rSatEciDt - satPosPrec.rSatEci0;
    Vector3d  rtnJ2             = E * differenceJ2;

    std::cout << "\r\nPrecise:\t" << satPosPrec.rSatEci0.transpose().format(heavyFmt);
    std::cout << "\r\nPropFull:\t" << satPosProp.rSatEciDt.transpose().format(heavyFmt) << "\t"
              << rtnProp.transpose().format(heavyFmt) << " \t" << rtnProp.norm();
    std::cout << "\r\nEllipse:\t" << satPosEllipse.rSatEciDt.transpose().format(heavyFmt) << "\t"
              << rtnEllipse.transpose().format(heavyFmt) << " \t" << rtnEllipse.norm();
    std::cout << "\r\nEllipseJ2:\t" << satPosJ2.rSatEciDt.transpose().format(heavyFmt) << "\t"
              << rtnJ2.transpose().format(heavyFmt) << " \t" << rtnJ2.norm();
        ...
        kfState.outputStates(std::cout, "/Deleted");
        {
            ...
            if (1)
                for (int i = 0; i < 2; i++)
                {
                    ...
                    double omc = measuredAcceleration - stateAcceleration + stateBias;
