...
)
{
    ...
    while (1)
    {
        ...
        switch (type)
        {
            ...
            {
                bool pass =
                    getFilterObjectFromFile(type, metaDataMap, startPos, reversedStatesFilename);
                if (pass == false)
                {
            ...
            case E_SerialObject::MEASUREMENT:
            {
                KFMeas archiveMeas;
                bool   pass =
                    getFilterObjectFromFile(type, archiveMeas, startPos, reversedStatesFilename);
                if (pass == false)
                {
                    BOOST_LOG_TRIVIAL(error) << "BAD RTS OUTPUT read";
                    return;
            ...
            case E_SerialObject::FILTER_SMOOTHED:
            {
                KFState archiveKF;
                bool    pass =
                    getFilterObjectFromFile(type, archiveKF, startPos, reversedStatesFilename);
                ...
                Network       dummyNet;
                std::ofstream trace(
                    archiveKF.metaDataMap[TRACE_FILENAME_STR + SMOOTHED_SUFFIX],
        ...
        if (startPos < 0)
        {
            BOOST_LOG_TRIVIAL(error) << "Oopsie " << "\n";
...
void rtsSmoothing(KFState& kfState, ReceiverMap& receiverMap, bool write)
{
    ...
    while (lag != kfState.rts_lag)
    {
        ...
        switch (type)
        {
            ...
            case E_SerialObject::METADATA:
            {
                ...
                bool pass =
                    getFilterObjectFromFile(type, smoothedKF.metaDataMap, startPos, inputFile);
                if (pass == false)
                {
            ...
            case E_SerialObject::TRANSITION_MATRIX:
            {
                ...
                bool                   pass =
                    getFilterObjectFromFile(type, transistionMatrixObject, startPos, inputFile);
                if (pass == false)
                {
                    return;
                }
    ...
    if (kfState.rts_lag <= 0 && acsConfig.retain_rts_files == false)
    {
