...
void UbxParser::parse(std::istream& inputStream)
{
    ...
    while (inputStream)
    {
        ...
        while (true)
        {
            ...
            if (inputStream)
            {
                if (c1 == UBX_PREAMBLE1 && c2 == UBX_PREAMBLE2)
                {
                    break;
                }
            }
        ...
        unsigned char ubxClass = 0;
        inputStream.read((char*)&ubxClass, 1);
        CLEAN_UP_AND_RETURN_ON_FAILURE;
        unsigned char id = 0;
        inputStream.read((char*)&id, 1);
        CLEAN_UP_AND_RETURN_ON_FAILURE;
        unsigned short int payload_length = 0;
        inputStream.read((char*)&payload_length, 2);
        CLEAN_UP_AND_RETURN_ON_FAILURE;
        vector<unsigned char> payload(payload_length);
        inputStream.read((char*)payload.data(), payload_length);
