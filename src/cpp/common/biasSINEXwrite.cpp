...
)
{
    ...
    tracepdeex(
        ...
        "%3s %4d:%03d:%05d ",
        acsConfig.analysis_agency.c_str(),
        (int)startTimeofFile[0],
        (int)startTimeofFile[1],
        (int)startTimeofFile[2]
    );
    tracepdeex(
        0,
        trace,
...
)
{
    ...
    tracepdeex(
        0,
        trace,
        " OUTPUT            OSB estimates for day %3d, %4d\n",
        startTimeofFile[1],
        startTimeofFile[0]
    );
    ...
    tracepdeex(
        0,
        trace,
        " RECEIVER_CLOCK_REFERENCE_GNSS           %c\n",
...
)
{
    ...
    double updateRate = 0;
    if (measType == E_MeasType::CODE)
        updateRate = acsConfig.ambrOpts.code_output_interval;
    if (measType == E_MeasType::PHAS)
        updateRate = acsConfig.ambrOpts.phase_output_interval;
...
)
{
    ...
    if (acsConfig.bias_time_system == "G")
        tsys = E_TimeSys::GPST;
    else if (acsConfig.bias_time_system == "C")
        tsys = E_TimeSys::BDT;
    else if (acsConfig.bias_time_system == "R")
        tsys = E_TimeSys::GLONASST;
    else if (acsConfig.bias_time_system == "UTC")
        tsys = E_TimeSys::UTC;
    else if (acsConfig.bias_time_system == "TAI")
        tsys = E_TimeSys::TAI;
    ...
    if (acsConfig.ambrOpts.code_output_interval > 0)
        updateBiasOutput(trace, time, kfState, ionState, receiverMap, CODE);
    if (acsConfig.ambrOpts.phase_output_interval > 0)
        updateBiasOutput(trace, time, kfState, ionState, receiverMap, PHAS);
