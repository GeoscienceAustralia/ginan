...
)
{
    ...
    if (ephMap.find(sys) == ephMap.end())
        return nullptr;
    auto& sysMap = ephMap[sys];
    if (sysMap.find(type) == sysMap.end())
        return nullptr;
    auto& sysEphMap = sysMap[type];
...
)
{
    ...
    tracepdeex(
        5,
        trace,
        "\nno broadcast ephemeris: %s sat=%s within MAXDTOE: %f ",
        time.to_string().c_str(),
        Sat.id().c_str(),
        tmax
    );
...
{
    return selSatEphFromMap(trace, time, Sat, type, iode, nav.ephMap);
}
template <>
Geph* seleph<
    Geph>(Trace& trace, GTime time, SatSys Sat, E_NavMsgType type, int iode, Navigation& nav)
{
    return selSatEphFromMap(trace, time, Sat, type, iode, nav.gephMap);
}
template <>
Seph* seleph<
    Seph>(Trace& trace, GTime time, SatSys Sat, E_NavMsgType type, int iode, Navigation& nav)
{
    return selSatEphFromMap(trace, time, Sat, type, iode, nav.sephMap);
}
template <>
ION* seleph<ION>(Trace& trace, GTime time, E_Sys sys, E_NavMsgType type, Navigation& nav)
{
    return selSysEphFromMap(trace, time, sys, type, nav.ionMap);
}
template <>
EOP* seleph<EOP>(Trace& trace, GTime time, E_Sys sys, E_NavMsgType type, Navigation& nav)
{
    return selSysEphFromMap(trace, time, sys, type, nav.eopMap);
}
...
{
    double t = (time - eph.toe).to_double();
    t -= recurse ? eph2Clk(time - t, eph, recurse - 1) : 0;
...
double eph2Clk(GTime time, Geph& geph, int recurse = 0)
{
    ...
    t -= recurse ? eph2Clk(time - t, geph, recurse - 1) : 0;

    return -geph.taun + geph.gammaN * t;
}
...
{
    double t = (time - seph.toe).to_double();
    t -= recurse ? eph2Clk(time - t, seph, recurse - 1) : 0;
...
)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        rSat[i] = seph.pos[i] + seph.vel[i] * t + seph.acc[i] * t * t / 2;
    }
...
)
{
    ...
    if (eph_ptr == nullptr)
    {
        tracepdeex(
            2,
            trace,
            "\nCould not find Broadcast Ephemeris for sat: %s, %s",
            Sat.id().c_str(),
    ...
    tracepdeex(
        5,
        trace,
        "\nSelected ephemeris sat: %s, iode=%d, teph=%s, toe=%s",
        Sat.id().c_str(),
        iode,
        teph.to_string().c_str(),
