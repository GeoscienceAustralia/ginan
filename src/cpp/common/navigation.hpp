...
struct Navigation
{
    ...
    map<string, map<E_Sys, map<E_FType, map<GTime, PhaseCenterOffset, std::greater<GTime>>>>>
                                                                                            pcoMap;
    map<string, map<E_Sys, map<E_FType, map<GTime, PhaseCenterData, std::greater<GTime>>>>> pcvMap;

    map<SatSys, map<E_NavMsgType, map<GTime, Eph, std::greater<GTime>>>>
        ephMap;   ///< GPS/QZS/GAL/BDS ephemeris
    map<SatSys, map<E_NavMsgType, map<GTime, Geph, std::greater<GTime>>>>
        gephMap;  ///< GLONASS ephemeris
    map<SatSys, map<E_NavMsgType, map<GTime, Seph, std::greater<GTime>>>>
        sephMap;  ///< SBAS ephemeris
    map<SatSys, map<E_NavMsgType, map<GTime, Ceph, std::greater<GTime>>>>
        cephMap;  ///< GPS/QZS/BDS CNVX ephemeris
    map<E_Sys, map<E_NavMsgType, map<GTime, ION, std::greater<GTime>>>> ionMap;  ///< ION messages
    map<E_StoCode, map<E_NavMsgType, map<GTime, STO, std::greater<GTime>>>>
                                                                        stoMap;  ///< STO messages
