...
void InteractiveTerminal::keyboardHandler()
{
    while (1)
    {
        ...
        if (ch == '1')
            interactMode = E_InteractMode::Page;
        else if (ch == '2')
            interactMode = E_InteractMode::Scroll;
        else if (interactMode == +E_InteractMode::Page)
        {
            switch (ch)
            {
                ...
                case KEY_LEFT:
                {
                    ...
                    while (1)
                    {
                        // keep trying inc/decrement pages until the base prefix at the correct
                        // level changes
                        ...
                        bool changed =
                            (activePage.substr(0, baseChars) != testPage.substr(0, baseChars));
                    ...
                    // found something with a different base, check if the base's base is common or
                    // not to determine if we can get to it from here
                    auto& [testPage, page] = *it;
        ...
        else if (interactMode == +E_InteractMode::Scroll)
        {
            switch (ch)
            {
                ...
                case KEY_END:
                {
                    page.currentLine = page.lines.size() + 1;
                    break;
                }
                default:
                {
        ...
        if (activeLevel >= activeSplit.size())
        {
            activeLevel = activeSplit.size() - 1;
        }
        if (page.currentLine < 0)
        {
            page.currentLine = 0;
        }
        if (page.currentLine > page.lines.size())
        {
            page.currentLine = page.lines.size() - 1;
            page.followEnd   = true;
        }
...
void InteractiveTerminal::drawMenus()
{
    ...
    if (activePage.empty() == false)
        for (int level = 0; level < activeSplit.size(); level++)
        {
            ...
            // level 2 - print the 2 split of everything that matches active.split1 and active
            // split2 but only once
            string lastPrinted;
    ...
    for (int i = 1; i < E_InteractMode::_size(); i++)
    {
        ...
        if (interactMode == +mode)
        {
            wattron(bar, A_STANDOUT);
            wattron(bar, A_BOLD);
        }
        else
        {
...
void InteractiveTerminal::addString(string pageName, const string& str, bool updateWindow)
{
    if (enabled == false)
        return;
    ...
    if (updateWindow && activePage == pageName)
    {
        drawWindow();
    }
}
...
void InteractiveTerminal::setMode(E_InteractiveMode modeName)
{
    ...
    {
        modes[activeMode].duration =
            (modes[activeMode].stopTime - modes[activeMode].startTime).to_double();
    }
    ...
    {
        if (testMode < modeName && mode.active)
        {
            mode.active++;
        }
