...
{
    bool equal = time == rhs.time && xp == rhs.xp && yp == rhs.yp && ut1Utc == rhs.ut1Utc &&
                 lod == rhs.lod && xpr == rhs.xpr && ypr == rhs.ypr;
...
{
    Vector3d erp;
    erp << xp * R2MAS, yp * R2MAS, ut1Utc * S2MTS;
...
)
{
    ...
    int found = sscanf(
        line.c_str(),
        "%d %d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
        &date[0],
        &date[1],
        &date[2],
        &date[4],
...
{
    if (useFilter && erp.filterValues.isFiltered)
    {
    ...
    for (auto rit = erp.erpMaps.rbegin(); rit != erp.erpMaps.rend(); rit++)
    {
        ...
        if (dt.front() >= +dtMax || dt.back() <= -dtMax)
        {
        ...
        if (dt.front() > 0 && dt.front() < +dtMax)
        {
            dtMax = dt.front();
            continue;
        }
        else if (dt.back() < 0 && dt.back() > -dtMax)
        {
            dtMax = -dt.back();
            continue;
        }
...
void writeErp(string filename, ERPValues& erp)
{
    ...
    {
        erpStream << "VERSION 2" << "\n";
        erpStream << " Generated by GINAN " << ginanCommitVersion() << " branch "
                  << ginanBranchName() << "\n";
        ...
        erpStream << "-----------------------------------------------------------------------------"
                     ...
                     "---------------"
                  << "\n";
        erpStream << "       MJD    Xpole    Ypole  UT1-UTC      LOD     Xsig     Ysig    UTsig   "
                     "LODsig  Nr  Nf  Nt   "
                     "   Xrt      Yrt"
                  << "\n";
        erpStream << "             1E-6as   1E-6as    1E-7s  1E-7s/d   1E-6as   1E-6as    1E-7s  "
                     "1E-7s/d               "
                     "1E-6/d   1E-6/d"
                  << "\n";
    }
    ...
    tracepdeex(
        0,
        erpStream,
        "%8.4f %8d %8d %8d %8d %8d %8d %8d %8d %3d %3d %3d %8d %8d\n",
        mjd.to_double(),
        (int)(erp.xp * 1E6 * R2AS),
        (int)(erp.yp * 1E6 * R2AS),
        (int)(erp.ut1Utc * 1E7),
...
ERPValues getErpFromFilter(const KFState& kfState)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        switch (i)
        {
            case 0:
                erpv.xp       = +val * MAS2R;
                erpv.xpSigma  = sqrt(valVar) * MAS2R;
                erpv.xpr      = +rate * MAS2R;
                erpv.xprSigma = sqrt(rateVar) * MAS2R;
                break;
            case 1:
                erpv.yp       = +val * MAS2R;
                erpv.ypSigma  = sqrt(valVar) * MAS2R;
                erpv.ypr      = +rate * MAS2R;
                erpv.yprSigma = sqrt(rateVar) * MAS2R;
                break;
            case 2:
                erpv.ut1Utc      = +val * MTS2S;
                erpv.ut1UtcSigma = sqrt(valVar) * MTS2S;
                erpv.lod         = -rate * MTS2S;
                erpv.lodSigma    = sqrt(rateVar) * MTS2S;
                break;
            default:
                break;
        }
...
)
{
    ...
    while (filestream)
    {
        ...
        if ((line[16] == 'I' || line[16] == 'P') && (line[57] == 'I' || line[57] == 'P'))
            ...
            readIers20C04(line, erpMap);
        else if (line.size() == 155)
            readIers14C04(line, erpMap);
        else if (line.size() <= 146 && line.size() >= 106)
            readIgsErp(line, erpMap);
        else if (line.size() <= 79)
            readIersBulletinA(line, erpMap);
    }
