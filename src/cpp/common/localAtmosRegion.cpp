...
int checkSSRRegion(VectorPos& pos)
{
    ...
    {
        if (pos[0] > regData.maxLatDeg)
            continue;
        if (pos[0] < regData.minLatDeg)
            continue;

        double midLon = (regData.minLonDeg + regData.maxLonDeg) / 2;
        double staLon = pos[1];
        if ((staLon - midLon) > 180)
            staLon -= 360;
        else if ((staLon - midLon) < -180)
            staLon += 360;

        if (staLon > regData.maxLonDeg)
            continue;
        if (staLon < regData.minLonDeg)
            continue;
    ...
    for (auto& regionFile : acsConfig.atm_reg_definitions)
    {
        ...
        while (std::getline(inputStream, line))
        {
            ...
            if (strstr(comment, "GRID TYPE"))
            {
                ...
                tmp[5]   = '\0';
                gridType = atoi(tmp);
                strncpy(tmp, buff + 20, 5);
                tmp[5]       = '\0';
                int tropGrid = atoi(tmp);
                strncpy(tmp, buff + 40, 5);
                tmp[5]       = '\0';
                int ionoGrid = atoi(tmp);
            ...
            if (strstr(comment, "POLYNOMIAL SIZE"))
            {
                strncpy(tmp, buff, 5);
                tmp[5]        = '\0';
                int polySize1 = atoi(tmp);
                strncpy(tmp, buff + 20, 5);
                tmp[5]        = '\0';
                int polySize2 = atoi(tmp);
            ...
            if (strstr(comment, "GRID POINT") && gridType == 0)
            {
                strncpy(tmp, buff, 10);
                tmp[10]                         = '\0';
                regMaps[regID].gridLatDeg[nind] = atof(tmp);
                strncpy(tmp, buff + 20, 10);
                tmp[10]                         = '\0';
                regMaps[regID].gridLonDeg[nind] = atof(tmp);

                if ((regMaps[regID].gridLatDeg[nind] + 0.1) > regMaps[regID].maxLatDeg)
                    regMaps[regID].maxLatDeg = regMaps[regID].gridLatDeg[nind] + 0.1;
                if ((regMaps[regID].gridLonDeg[nind] + 0.1) > regMaps[regID].maxLonDeg)
                    regMaps[regID].maxLonDeg = regMaps[regID].gridLonDeg[nind] + 0.1;
                if ((regMaps[regID].gridLatDeg[nind] - 0.1) < regMaps[regID].minLatDeg)
                    regMaps[regID].minLatDeg = regMaps[regID].gridLatDeg[nind] - 0.1;
                if ((regMaps[regID].gridLonDeg[nind] - 0.1) < regMaps[regID].minLonDeg)
                    regMaps[regID].minLonDeg = regMaps[regID].gridLonDeg[nind] - 0.1;

                nind++;
            }

            if (strstr(comment, "REGION END"))
                if (latNgrid > 1 || lonNgrid > 1)
                {
                    nind = 0;
                    if (gridType == 1)
...
bool configAtmosRegions(Trace& trace, ReceiverMap& receiverMap)
{
    ...
    if (acsConfig.ssrOpts.grid_type < 0 && acsConfig.ssrOpts.npoly_trop < 0 &&
        acsConfig.ssrOpts.npoly_iono < 0)
    {
        return false;
    }

    trace << "gridtype: " << acsConfig.ssrOpts.grid_type
          << "; TropPoly: " << acsConfig.ssrOpts.npoly_trop
          << "; IonoPoly: " << acsConfig.ssrOpts.npoly_iono << "\n";
    ...
    {
        case 1:
            atmRegion.tropPolySize = 1;
            break;
        case 2:
        case 3:
            atmRegion.tropPolySize = 3;
            break;
        case 4:
            atmRegion.tropPolySize = 4;
            break;
        default:
            atmRegion.tropPolySize = -1;
    }
    ...
    {
        case 1:
            atmRegion.ionoPolySize = 1;
            break;
        case 2:
        case 3:
            atmRegion.ionoPolySize = 3;
            break;
        case 4:
            atmRegion.ionoPolySize = 4;
            break;
        case 5:
        case 6:
            atmRegion.ionoPolySize = 6;
            break;
        default:
            atmRegion.ionoPolySize = -1;
    }
    ...
    if (coordFromRec)
    {
        for (auto& [id, rec] : receiverMap)
        {
            ...
            if (atmRegion.minLatDeg > pos.lat())
                atmRegion.minLatDeg = pos.latDeg();
            if (atmRegion.maxLatDeg < pos.lat())
                atmRegion.maxLatDeg = pos.latDeg();

            double midLonDeg = (atmRegion.minLonDeg + atmRegion.maxLonDeg) / 2;
            double recLonDeg = pos.lonDeg();
            if ((recLonDeg - midLonDeg) > 180)
                recLonDeg -= 360;
            else if ((recLonDeg - midLonDeg) < -180)
                recLonDeg += 360;

            if (atmRegion.minLonDeg > recLonDeg)
                atmRegion.minLonDeg = recLonDeg;
            if (atmRegion.maxLonDeg < recLonDeg)
                atmRegion.maxLonDeg = recLonDeg;
    ...
    if (acsConfig.ssrOpts.grid_type > 0)
    {
        ...
        int nIntLat;
        int nIntLon;
        if (acsConfig.ssrOpts.lat_int > 0)
        {
            atmRegion.intLatDeg = acsConfig.ssrOpts.lat_int;
            nIntLat = floor((atmRegion.maxLatDeg - atmRegion.minLatDeg) / atmRegion.intLatDeg) + 1;
        }
        else
        {
            atmRegion.intLatDeg = atmRegion.maxLatDeg - atmRegion.minLatDeg;
            nIntLat             = 1;
        }
        if (acsConfig.ssrOpts.lon_int > 0)
        {
            atmRegion.intLonDeg = acsConfig.ssrOpts.lon_int;
            nIntLon = floor((atmRegion.maxLonDeg - atmRegion.minLonDeg) / atmRegion.intLonDeg) + 1;
        }
        else
        {
            atmRegion.intLonDeg = atmRegion.maxLonDeg - atmRegion.minLonDeg;
            nIntLon             = 1;
        }
    ...
    if (atmRegion.intLatDeg == 0)
        atmRegion.intLatDeg = atmRegion.maxLatDeg - atmRegion.minLatDeg;
    if (atmRegion.intLonDeg == 0)
        atmRegion.intLonDeg = atmRegion.maxLonDeg - atmRegion.minLonDeg;
