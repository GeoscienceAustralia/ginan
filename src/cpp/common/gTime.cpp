...
int str2time(const char* s, int i, int n, GTime& t, E_TimeSys tsys)
{
    ...
    *p = '\0';
    ...
    int readCount =
        sscanf(str, "%lf %lf %lf %lf %lf %lf", ep, ep + 1, ep + 2, ep + 3, ep + 4, ep + 5);
...
GTime yds2time(const double* yds, E_TimeSys tsys)
{
    ...
    int leapDays =
        (year - 1968 - 1) /
            4  // -1968 = last leap year before 1970; -1 = year must end before applying leap-day
...
GTime epoch2time(const double* ep, E_TimeSys tsys)
{
    ...
    double secOfDay = hour * 60 * 60 + min * 60 + sec;
...
string GTime::to_string(int n) const
{
    ...
    snprintf(
        buff,
        sizeof(buff),
        "%04.0f-%02.0f-%02.0f %02.0f:%02.0f:%0*.*f",
        ep.year,
        ep.month,
        ep.day,
        ep.hour,
...
string GTime::to_ISOstring(int n) const
{
...
MjDateUt1::MjDateUt1(GTime time, double ut1_utc)
{
...
GTime::GTime(GWeek gpsWeek, GTow tow)
{
    *this = GPS_t0 + gpsWeek * secondsInWeek + tow;
}
GTime::GTime(BWeek bdsWeek, BTow tow)
{
...
GTime::GTime(GTow tow, GTime nearTime)
{
    *this = nearestTime(GPS_t0, tow, nearTime, secondsInWeek);
}
GTime::GTime(BTow tow, GTime nearTime)
{
    *this = nearestTime(BDS_t0, tow, nearTime, secondsInWeek);
}
...
{
    Duration seconds = *this - GPS_t0;
    GTow     gTow    = fmod(seconds.to_double(), (double)secondsInWeek);
    return gTow;
}
GTime::operator BTow() const
{
    Duration seconds = *this - BDS_t0;
    BTow     bTow    = fmod(seconds.to_double(), (double)secondsInWeek);
    return bTow;
}
GTime::operator RTod() const
{
    Duration seconds = *this - GLO_t0;
    RTod     rTod    = fmod(seconds.to_double() - leapSeconds(*this), (double)secondsInDay);
    return rTod;
}
...
double ymdhms2jd(const double time[6])  ///< civil date time [YMDHMS]
{
    double i;
    double j;
    double yr  = time[0];
    ...
    double day = time[2] + hr / 24 + min / 24 / 60 + sec / secondsInDay;
    double jd  = floor(365.25 * i) + floor(30.6001 * (j + 1)) + day + 1720981.5;
    // 	BOOST_LOG_TRIVIAL(debug) << "YMDH to JD: year=" << yr << " mon=" << mon << " day=" << day <<
    // " hour=" << hr <<
    // ", jd=" << jd;
    return jd;
};