    ...
    while (1)
    {
        ...
        // Guarding while we splice the shared queue to the local queue, which should be quick
        {
            ...
            BOOST_LOG_TRIVIAL(debug)
                << "Mongo Queue: Splicing " << mongoQueue.size() << " entries for processing";
        ...
        for (auto& object : localQueue)
        {
            ...
            {
                case E_MongoType::STATES_AVAILABLE:
                    mongoStatesAvailable(object.kfState_optl->time, object.mongoStatesOpts);
                case E_MongoType::STATES:
                    mongoStates(*object.kfState_optl, object.mongoStatesOpts);
                    break;
                case E_MongoType::RESIDUALS:
                    mongoMeasResiduals(object.time, *object.kfMeas_optl, false, object.suffix);
                    break;
                case E_MongoType::TRACE:
                    traceJsons.push_back(std::move(object.suffix));
                    break;
                case E_MongoType::LIST:
                    mongoOutput(object.dbEntryList, false, object.instances, object.suffix);
                    break;
            }
        }
...
void queueMongo(QueuedMongo& object)  ///< Object to output
{
    ...
    for (auto instance : instances)
    {
        ...
        for (auto& [description, index] : lookup)
        {
            ...
            for (int& i : index)
            {
                ...
                if ((static_cast<int>(instance) &
                     static_cast<int>(acsConfig.mongoOpts.output_components)) ==
                        static_cast<int>(E_Mongo::NONE) ||
                ...
                for (auto& [component, details] : componentsMap)
                {
                    ...
                    string label =
                        name + " " + enum_to_string(obsKey.type) + " " + enum_to_string(component);
                    ...
                    label = name + " " + enum_to_string(obsKey.type) + "_" + resLabel + " " +
...
void mongoStates(KFState& kfState, MongoStatesOptions opts)
{
    ...
    for (auto instance : instances)
    {
        ...
        for (auto& [key, index] : kfState.kfIndexMap)
        {
            ...
            lookup[{key.str, key.Sat.id(), enum_to_string(key.type)}].push_back({index, key.num});
        }
        ...
        for (auto& [description, index] : lookup)
        {
            ...
            if (static_cast<int>(instance) &
                static_cast<int>(acsConfig.mongoOpts.output_state_covars))
            {
                array_builder = doc << toString(Constants::Mongo::COVAR_VAR) << open_array;
                for (auto& [i, numI] : index)
                    for (auto& [j, numJ] : index)
                        if (j > i)
                            array_builder << kfState.P(i, j);
                array_builder << close_array;
            }

            if (opts.upsert)
            {
                valdoc << close_document;
                bulk.append(mongocxx::model::update_one(keydoc.view(), valdoc.view()).upsert(true));
            }
            else
            {
                bulk.append(mongocxx::model::insert_one(keydoc.view()));
            }
...
void mongoCull(GTime time)
{
    ...
    for (auto instance : instances)
    {
        ...
        for (auto collection : {SSR_DB, REMOTE_DATA_DB})
        {
            ...
            b_date btime{std::chrono::system_clock::from_time_t(
                (time_t)((PTime)(time - acsConfig.mongoOpts.min_cull_age)).bigTime
            )};

            // Remove all documents that match a condition.
            auto docSys = document{} << SSR_EPOCH << open_document << "$lt" << btime
                                     << close_document << finalize;
...
document entryToDocument(DBEntry& entry, bool type)
{
    ...
    for (auto& [k, v] : entry.stringMap)
    {
        auto& [e, b] = v;
        if (type == b)
            doc << k << e;
    }
    for (auto& [k, v] : entry.intMap)
    {
        auto& [e, b] = v;
        if (type == b)
            doc << k << e;
    }
    for (auto& [k, v] : entry.doubleMap)
    {
        auto& [e, b] = v;
        if (type == b)
            doc << k << e;
    }
    for (auto& [k, v] : entry.timeMap)
    {
        auto& [e, b] = v;
        if (type == b)
            doc << k << b_date{std::chrono::system_clock::from_time_t((time_t)((PTime)e).bigTime)};
    }
    for (auto& [k, v] : entry.vectorMap)
    {
        auto& [e, b] = v;
        if (type == b)
            for (int i = 0; i < 3; i++)
                doc << k + std::to_string(i) << e[i];
    }
    for (auto& [k, v] : entry.doubleArrayMap)
    {
        auto& [e, b] = v;
        if (type == b)
        {
            auto builder = doc << k << open_array;
            for (auto& a : e)
                builder << a;
            builder << close_array;
        }
    }
    for (auto& [k, v] : entry.boolArrayMap)
    {
        auto& [e, b] = v;
        if (type == b)
        {
...
)
{
    ...
    for (auto instance : instances)
    {
        ...
        for (auto& entry : dbEntryList)
        {
            ...
            // 		std::cout << "\n" << bsoncxx::to_json(keys.view()) <<
            // bsoncxx::to_json(Vals.view()) << "\n";
...
)
{
    ...
    BOOST_LOG_TRIVIAL(info) << "Calculating SSR message precursors\n";
    ...
    for (E_Sys sys : magic_enum::enum_values<E_Sys>())
    {
        ...
        for (auto& Sat : sats)
        {
            ...
            for (int tpredict = 0; tpredict <= acsConfig.ssrOpts.prediction_duration;
                 tpredict += acsConfig.ssrOpts.prediction_interval)
            {
                ...
                pass &= satclk(
                    ...
                    pTime,
                    obs,
                    acsConfig.ssrOpts.clock_sources,
                    nav,
                    &kfState
                );
                pass &= satpos(
                    ...
                    pTime,
                    obs,
                    acsConfig.ssrOpts.ephemeris_sources,
                    E_OffsetType::APC,
                    nav,
                    &kfState
                ...
                {
                    BOOST_LOG_TRIVIAL(info) << Sat.id() << " IODE clock" << obs.iodeClk
                                            << " mismatch IODE orbit " << obs.iodePos << ".\n";
                ...
                // broadcast values, must come after precise values to associate the correct IODE
                // (when SSR in use)
                ...
                if (pass == false)
                {
                    BOOST_LOG_TRIVIAL(info) << Sat.id() << " failed broadcast predictions.\n";
            ...
            switch (source)
            {
                case E_Source::PRECISE:
                {
                    for (auto& obsCode : acsConfig.code_priorities[Sat.sys])
                    {
                        ...
                        entry.doubleMap["yawAngle"]       = {
                            0,
                            false
                        }; /* To Do: reflect internal yaw calculation here */
                        entry.doubleMap["yawRate"] = {
                            0,
                            false
                        }; /* To Do: reflect internal yaw calculation here */
                ...
                case E_Source::SSR:
                {
                    ...
                    for (auto& [obsCode, biasVar] : ssrPhasBias.obsCodeBiasMap)
                    {
                        ...
                        entry.intMap["signalWLIntInd"] = {
                            ssrPhaseChs[obsCode].signalWLIntInd,
                            false
                        };
                        entry.intMap["signalDisconCnt"] = {
                            ssrPhaseChs[obsCode].signalDisconCnt,
                            false
                        };
            ...
            if (igsIonoDetected)
            {
                ...
                for (auto& [hind, laydata] : atmGlob.layers)
                    for (auto& [bind, basdata] : laydata.sphHarmonic)
                    {
                        ...
                        entry.intMap[IGS_ION_PAR] =
                            std::make_tuple(static_cast<int>(basdata.trigType), false);
