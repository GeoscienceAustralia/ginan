...
void SinexParser::parseSinexEstimates(string& s)
{
    ...
    UYds yds;
    int  readcount;
    readcount = sscanf(s.c_str() + 27, "%2lf:%3lf:%5lf", &yds[0], &yds[1], &yds[2]);
    ...
    readcount = sscanf(s.c_str() + 47, "%21lf %11lf", &x, &P0);
...
void SinexParser::parseSinexEstimateMatrix(string& s)
{
    ...
    for (int i = 0; i < covars; i++)
    {
        ...
        if (timeA.bigTime == 0 || timeB.bigTime == 0)
        {
            continue;
        }

        auto& [valueA, covMapA] = valueMap[timeA][kfKeyA];
        covMapA[kfKeyB]         = values[i];
        auto& [valueB, covMapB] = valueMap[timeB][kfKeyB];
        covMapB[kfKeyA]         = values[i];
    }
}
...
void SinexParser::parseSinexDiscontinuities(string& s)
{
    ...
    if (startYds[0] != 0 || startYds[1] != 0 || startYds[2] != 0)
    {
        nearestYear(startYds[0]);
    }

    if (endYds[0] != 0 || endYds[1] != 0 || endYds[2] != 0)
    {
