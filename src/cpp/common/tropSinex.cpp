...
{
    if (!theSinex.tropDesc.isEmpty)
    {
        theSinex.blockComments["TROP/DESCRIPTION"].clear();
        theSinex.blockComments["TROP/DESCRIPTION"].push_back(
            "*_________KEYWORD_____________ __VALUE(S)_______________________________________"
        );
    }
    if (!theSinex.mapsiteids.empty())
    {
        theSinex.blockComments["SITE/ID"].clear();
    ...
    {
        theSinex.blockComments["SITE/RECEIVER"].clear();
        theSinex.blockComments["SITE/RECEIVER"].push_back(
            "*STATION__ PT SOLN T DATA_START____ DATA_END______ DESCRIPTION_________ "
            "S/N_________________ FIRMWARE___"
        );
    }
    if (!theSinex.mapantennas.empty())
    {
        theSinex.blockComments["SITE/ANTENNA"].clear();
        theSinex.blockComments["SITE/ANTENNA"].push_back(
            "*STATION__ PT SOLN T DATA_START____ DATA_END______ DESCRIPTION_________ "
            "S/N_________________ PCV_MODEL_"
        );
    }
    if (!theSinex.mapeccentricities.empty())
    {
        theSinex.blockComments["SITE/ECCENTRICITY"].clear();
        theSinex.blockComments["SITE/ECCENTRICITY"].push_back(
            "*                                                      _UP_____ _NORTH__ _EAST___"
        );
        theSinex.blockComments["SITE/ECCENTRICITY"].push_back(
            "*STATION__ PT SOLN T DATA_START____ DATA_END______ REF _MARKER->ARP(m)____________"
        );
    }
    if (!theSinex.tropSiteCoordMapMap.empty())
    {
        theSinex.blockComments["SITE/COORDINATES"].clear();
        theSinex.blockComments["SITE/COORDINATES"].push_back(
            "*STATION__ PT SOLN T __DATA_START__ __DATA_END____ __STA_X_____ __STA_Y_____ "
 ...
 */
void writeTropHeader(ofstream& out)  ///< stream to write out
{
    tracepdeex(
        0,
        out,
        "%%=TRO %4.2lf %3s %04d:%03d:%05d %3s %04d:%03d:%05d %04d:%03d:%05d %c %4s\n",
        theSinex.ver,
        theSinex.createagc,
        theSinex.filedate[0],
        theSinex.filedate[1],
 ...
 */
void setDescription(bool isSmoothed)  ///< if solution is smoothed (RTS or fixed-lag)
{
    string tropEstMethod;
    if (isSmoothed)
        tropEstMethod = "Smoother";
    else
        tropEstMethod = "Filter";
    ...
    theSinex.tropDesc.strings["ATMOSPH TIDE LOADING MODEL"] =
        acsConfig.atmospheric_tide_loading_model;
    ...
    if (theSinex.tropSolList.empty() == false)
        for (auto& entry : theSinex.tropSolList.front().solutions)
        {
            ...
            unitsSs << std::scientific << std::setprecision(0)
                    << entry.units;  // get into scientific format
            theSinex.tropDesc.vecStrings["TROPO PARAMETER UNITS"].push_back(unitsSs.str());
            theSinex.tropDesc.vecStrings["TROPO PARAMETER WIDTH"].push_back(
                std::to_string(entry.width)
            );
        }
 ...
 */
void writeTropDesc(ofstream& out)  ///< stream to write out
{
    Block block(out, "TROP/DESCRIPTION");

    writeAsComments(out, theSinex.blockComments[block.blockName]);

    for (auto& [key, entry] : theSinex.tropDesc.strings)
        tracepdeex(0, out, " %-29s %22s\n", key, entry);
    for (auto& [key, entry] : theSinex.tropDesc.ints)
        tracepdeex(0, out, " %-29s %22d\n", key, entry);
    for (auto& [key, entry] : theSinex.tropDesc.doubles)
        tracepdeex(0, out, " %-29s %22f\n", key, entry);
    for (auto& [key, entry] : theSinex.tropDesc.vecStrings)
    {
        tracepdeex(0, out, " %-29s", key);
        for (auto& str : entry)
            tracepdeex(0, out, " %6s", str);
        tracepdeex(0, out, "\n");
    }
}
...
)
{
    ...
    {
        if (markerName != "MIX" && id != markerName)
        {
            continue;
        }
        ...
        if (ssi_it != theSinex.mapsiteids.end() && ssi_it->second.sitecode == id)
        {
            ...
            tracepdeex(
                0,
                out,
                " %-9s %2s %9s %c %22s %10.6lf %10.6lf %9.3lf %9.3lf\n",
                ssi.sitecode,
                ssi.ptcode,
                ssi.domes,
                ssi.typecode,
    ...
    for (auto& [id, entry] : theSinex.tropSiteCoordMapMap)
    {
        ...
        if (timemap_ != theSinex.mapreceivers.end())
        {
            ...
            for (auto it = timeMap.rbegin(); it != timeMap.rend(); it++)
            {
                ...
                if (receiver.sn.empty())
                    receiver.sn = "-----";
                if (receiver.firm.empty())
                    receiver.firm = "-----";

                tracepdeex(
                    0,
                    out,
                    " %-9s %2s %4s %c %04d:%03d:%05d %04d:%03d:%05d %20s %-20s %s\n",
...
void setSiteAntCalib()
{
    ...
    for (auto& [site, antmap] : theSinex.mapantennas)
        ...
        {
            auto& [time, ant] = *it;
            if (ant.calibModel.empty() || ant.calibModel == defaultStr)
            {
        ...
        if (timeMap_it != theSinex.mapantennas.end())
        {
            ...
            for (auto it = timeMap.rbegin(); it != timeMap.rend(); it++)
            {
                ...
                tracepdeex(
                    0,
                    out,
                    " %-9s %2s %4s %c %04d:%03d:%05d %04d:%03d:%05d %20s %-20s %-10s\n",
                    ant.sitecode,
                    ant.ptcode,
                    ant.solnnum,
                    ant.typecode,
        ...
        if (timeMap_it != theSinex.mapeccentricities.end())
        {
            ...
            for (auto it = timeMap.rbegin(); it != timeMap.rend(); it++)
            {
                ...
                tracepdeex(
                    0,
                    out,
                    " %-9s %2s %4s %c %04d:%03d:%05d %04d:%03d:%05d %3s %8.4lf %8.4lf %8.4lf\n",
                    set.sitecode,
                    set.ptcode,
                    set.solnnum,
                    set.typecode,
...
)
{
    ...
    for (auto& [id, entry] : theSinex.tropSiteCoordMapMap)
    {
        ...
        tracepdeex(
            0,
            out,
            " %-9s %2s %4s %c %04d:%03d:%05d %04d:%03d:%05d %12.3lf %12.3lf %12.3lf %6s %5s\n",
            id,
            theSinex.mapsiteids[id].ptcode,
            1,
            'P',  // note: adjust if station is non-GNSS - see sinex_v201_appendix1_doc for other
                  // obs types (e.g. SLR)
            theSinex.solutionstartdate[0],
            theSinex.solutionstartdate[1],
            theSinex.solutionstartdate[2],
 ...
 */
void setTropSolFromFilter(KFState& kfState)  ///< KF state
{
    ...
    map<string, map<string, State>>
        tropSumMap;  // for summing similar components - eg trop and trop_gm

    for (auto& [key, index] : kfState.kfIndexMap)
    {
        if (key.type != KF::TROP && key.type != KF::TROP_GRAD)
        {
            continue;
        }
        string type;
        if (key.type == KF::TROP)
            type = "TRO";  // zenith
        else if (key.type == KF::TROP_GRAD && key.num == 0)
            type = "TGN";  // N gradient
        else if (key.type == KF::TROP_GRAD && key.num == 1)
            type = "TGE";  // E gradient
        ...
        if (key.type == KF::TROP)
        {
            tropSumMap[id][typeTot].x += x;
            tropSumMap[id][typeTot].var = newVar;
        }
        {
    ...
    for (auto& [id, entries] : tropSumMap)
    {
        ...
        for (auto& [type, entry] : entries)
        {
            ...
            if (type.substr(type.size() - 3) == "WET")
            {
                units = 1e3;
                wet   = true;
            }
            else if (type.substr(type.size() - 3) == "TOT")
            {
                units = 1e3;
            }

            if (wet && type.substr(0, 3) == "TRO")
            {
                auto it = theSinex.tropSiteCoordMapMap.find(id);
                if (it == theSinex.tropSiteCoordMapMap.end())
                {
                ...
                double modelledZhd =
                    tropDryZTD(nullStream, recOpts.tropModel.models, kfState.time, pos);
...
void setTropSolCommentList()
{
    ...
    theSinex.blockComments["TROP/SOLUTION"].push_back(
        "*STATION__ ____EPOCH_____" + headerFields.str()
    );
}

/** Set troposphere solution data
 */
void setTropSol(KFState& kfState)  ///< KF state
{
...
)
{
    ...
    {
        if (markerName != "MIX" && entry.site != markerName)
        {
            continue;
        }
...
)
{
    ...
    if (pos == 0)
    {
        ...
        if (!theSinex.mapsiteids.empty())
        {
            writeTropSiteId(fout, markerName);
        }
        if (!theSinex.mapreceivers.empty())
        {
            writeTropSiteRec(fout, markerName);
        }
        if (!theSinex.mapantennas.empty())
        {
            writeTropSiteAnt(fout, markerName);
        }
        if (!theSinex.mapeccentricities.empty())
        {
            writeTropSiteEcc(fout, markerName);
        }
    }
    if (!theSinex.tropSiteCoordMapMap.empty())
    {
        writeTropSiteCoord(fout, markerName, filename);
    }
    if (!theSinex.tropSolList.empty())
    {
        writeTropSol(fout, markerName, filename);
    }
...
)
{
    ...
    sinexCheckAddGaReference(acsConfig.trop_sinex_sol_type, ginanCommitVersion(), true);

    KFState sinexSubstate =
        mergeFilters({&kfState}, {KF::ONE, KF::REC_POS, KF::REC_POS_RATE, KF::TROP, KF::TROP_GRAD});
