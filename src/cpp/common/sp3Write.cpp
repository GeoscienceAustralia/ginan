...
)
{
    ...
    tracepdeex(
        0,
        sp3Stream,
        "#d%c%4.0f %2.0f %2.0f %2.0f %2.0f %11.8f ",
        velPos,
        ep[0],
        ep[1],
        ep[2],
    ...
    tracepdeex(
        ...
        acsConfig.reference_system.c_str(),
    ...
    while (lineNumber < 17 || lineEntries < 17)
    {
    ...
    tracepdeex(
        0,
        sp3Stream,
        "%%c %c  cc GPS ccc cccc cccc cccc cccc ccccc ccccc ccccc ccccc\n",
        syschar
    );
    tracepdeex(0, sp3Stream, "%%c cc cc ccc ccc cccc cccc cccc cccc ccccc ccccc ccccc ccccc\n");

    // first variable is the base for the sigma of position, x, y, z, sigma = 1.25^val in
    // millimeters. second variable is the base for the sigma of time sigma = 1.025^val in
    // picoseconds.
...
)
{
    ...
    tracepdeex(
        0,
        sp3Stream,
        "*  %4.0f %2.0f %2.0f %2.0f %2.0f %11.8f\n",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
        ...
        if (it != entryList.end())
        {
...

            {
                tracepdeex(
                    0,
                    sp3Stream,
                    "P%s%14.6f%14.6f%14.6f%14.6f%15s%c%3s%c\n",
                    entry.Sat.id().c_str(),
                    entry.satPos.x() / 1000,
                    entry.satPos.y() / 1000,
                    entry.satPos.z() / 1000,
            ...
            if (acsConfig.output_sp3_velocities)
            {
                tracepdeex(
                    0,
                    sp3Stream,
                    "V%s%14.6f%14.6f%14.6f%14.6f%15s%c%3s%c\n",
                    entry.Sat.id().c_str(),
                    entry.satVel.x() * 10,
                    entry.satVel.y() * 10,
                    entry.satVel.z() * 10,
...
)
{
    ...
    for (auto& [Sat, satNav] : nav.satNavMap)
    {
        ...
        bool posPass =
            satpos(nullStream, time, time, obs, sp3OrbitSrcs, E_OffsetType::COM, nav, &kfState);
