...
GTime RtcmDecoder::rtcmTime()
{
    ...
    if (rtcmTimestampTime != GTime::noTime())
 ...
 */
int RtcmDecoder::adjGpsWeek(int week)  ///< not-adjusted GPS week number
{
 ...
 */
int RtcmDecoder::adjGstWeek(int week)  ///< not-adjusted GST week number
{
    GWeek nowWeek = rtcmTime();
 ...
 */
int RtcmDecoder::adjBdtWeek(int week)  ///< not-adjusted BDT week number
{
...
GTime RtcmDecoder::decodeCustomTimestamp(vector<unsigned char>& data)
{
    int i = 0;
    ...
    long int milliseconds = ((unsigned long int)chunk1) + (((unsigned long int)chunk2) << 32);
 ...
 */
void RtcmDecoder::decodeSSR(vector<unsigned char>& data)  ///< stream data
{
    int i = 0;
    ...
    if (updateIntIndex == 0)
        referenceTime = epochTime1s;
    else
        referenceTime = epochTime1s + ssrUpdateInterval / 2.0;
    ...
    if (messTypeStr == "ORB_CORR" || messTypeStr == "COMB_CORR")
    {
    ...
    for (int sat = 0; sat < numSats; sat++)
    {
        ...
        if (messTypeStr == "ORB_CORR" || messTypeStr == "COMB_CORR")
        {
            ...
            ssrEph.deph[0] =
                getbitsInc(data, i, 22) * 0.1e-3;  // Position, radial, along track, cross track.
            ...
            tracepdeex(
                5,
                std::cout,
                "\n#RTCM_SSR ORBITS %s %s %4d %10.3f %10.3f %10.3f %d ",
                Sat.id().c_str(),
                ssrEph.t0.to_string().c_str(),
                ssrEph.iode,
                ssrEph.deph[0],
                ssrEph.deph[1],
                ssrEph.deph[2],
        ...
        if (messTypeStr == "CLK_CORR" || messTypeStr == "COMB_CORR")
        {
            SSRClk ssrClk;
            ...
            tracepdeex(
                5,
                std::cout,
                "\n#RTCM_SSR CLOCKS %s %s      %10.3f %10.3f %10.3f %d",
                Sat.id().c_str(),
                ssrClk.t0.to_string().c_str(),
                ssrClk.dclk[0],
                ssrClk.dclk[1],
                ssrClk.dclk[2],
                iod
 ...
 */
void RtcmDecoder::decodeEphemeris(vector<unsigned char>& data)  ///< stream data
{
    Eph  eph  = {};
    Geph geph = {};
    int  i    = 0;
    ...
    if (sys == +E_Sys::GPS)
    {
        ...
        eph.weekRollOver = getbituInc(data, i, 10);
        eph.week         = adjGpsWeek(eph.weekRollOver);  // rolled-over week -> full week number
        ...
        eph.fitFlag      = getbituInc(data, i, 1);
        eph.fit          = eph.fitFlag ? 0.0 : 4.0;  // 0:4hr,1:>4hr
    ...
    else if (sys == +E_Sys::GLO)
    {
        ...
        geph.N4 =
            getbituInc(data, i, 5);  // additional data and GLONASS-M only, may be arbitrary value
    ...
    else if (sys == +E_Sys::BDS)
    {
        ...
        int prn          = getbituInc(data, i, 6);
        eph.weekRollOver = getbituInc(data, i, 13);
        eph.week         = adjBdtWeek(eph.weekRollOver);  // rolled-over week -> full week number
        eph.sva          = getbituInc(data, i, 4);
        eph.ura[0]       = svaToUra(eph.sva);
        eph.idot         = getbitsInc(data, i, 14) * P2_43 * SC2RAD;
    ...
    else if (sys == +E_Sys::QZS)
    {
        ...
        eph.iodc         = getbituInc(data, i, 10);
        eph.fitFlag      = getbituInc(data, i, 1);
        eph.fit          = eph.fitFlag ? 0.0 : 2.0;  // 0:2hr,1:>2hr
    ...
    else if (sys == +E_Sys::GAL)
    {
        ...
        if (messageNumber == RtcmMessageType::GAL_FNAV_EPHEMERIS)
        {
            ...
            int svh  = (eph.e5a_hs << 4) + (eph.e5a_dvs << 3);
            eph.svh  = (E_Svh)svh;
        ...
        else if (messageNumber == RtcmMessageType::GAL_INAV_EPHEMERIS)
        {
            ...
            int svh = (eph.e5b_hs << 7) + (eph.e5b_dvs << 6) + (eph.e1_hs << 1) + (eph.e1_dvs << 0);
            eph.svh = (E_Svh)svh;
    ...
    if (sys == +E_Sys::GPS || sys == +E_Sys::GAL || sys == +E_Sys::BDS || sys == +E_Sys::QZS)
    {
        ...
        traceTrivialDebug(
            "#RTCM_BRD EPHEMR %s %s %d",
            eph.Sat.id().c_str(),
            eph.toe.to_string().c_str(),
            eph.iode
        );
...
const int defaultGloChannel[] = {0,  1,  -4, 5,  6, 1,  -4, 5, 6, -2, -7, 0, -1,
...
double highResLockTimeFromIndicator(int indicator)
{
    ...
    if (i < 64)
        ...
        lockTime = 128 * i - 28672;
    else if (i < 320)
        lockTime = 256 * i - 65536;
    else if (i < 352)
        lockTime = 512 * i - 147456;
    else if (i < 384)
        lockTime = 1024 * i - 327680;
    else if (i < 416)
        lockTime = 2048 * i - 720896;
    else if (i < 448)
        lockTime = 4096 * i - 1572864;
    else if (i < 480)
        lockTime = 8192 * i - 3407872;
    else if (i < 512)
        lockTime = 16384 * i - 7340032;
    else if (i < 544)
        lockTime = 32768 * i - 15728640;
    else if (i < 576)
        lockTime = 65536 * i - 33554432;
    else if (i < 608)
        lockTime = 131072 * i - 71303168;
    else if (i < 640)
        lockTime = 262144 * i - 150994944;
    else if (i < 672)
        lockTime = 524288 * i - 318767104;
    else if (i < 704)
        lockTime = 1048576 * i - 671088640;
    else if (i == 704)
...
ObsList RtcmDecoder::decodeMSM(vector<unsigned char>& data)
{
    ObsList obsList;
    int     i = 0;
    ...
    switch (sysind)
    {
        case 107:
            rtcmsys = E_Sys::GPS;
            tobs    = GTime(GTow(tow), nearTime);
            break;
        case 108:
            rtcmsys = E_Sys::GLO; /*see below*/
            break;
        case 109:
            rtcmsys = E_Sys::GAL;
            tobs    = GTime(GTow(tow), nearTime);
    ...
    int              ncell = 0;
    // create signals for observations according to existing observations, the list of signals, and
    // the cell mask
    ...
    for (auto& [indx, signalPointer] : signalPointermap)
    {
        ...
        double lockTime = 0;
        if (msmtyp <= 5)
            lockTime = lockTimeFromIndicator(lockTimeIndicator);
        else if (msmtyp == 6 || msmtyp == 7)
            lockTime = highResLockTimeFromIndicator(lockTimeIndicator);
...
E_ReturnType RtcmDecoder::decode(vector<unsigned char>& message)
{
    ...
    if (retVal == E_ReturnType::OK || retVal == E_ReturnType::GOT_OBS)
    {
        frameDecoded();
    }
...
)
{
    ...
    if (bits == invalid)
    {
        // 		std::cout << "warning: invalid number received on " << __FUNCTION__ << " " <<
        // invalid << " " << len <<
        // "\n";
        if (failure_ptr)
        {
