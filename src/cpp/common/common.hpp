...
struct IteratorType
{
    ...
    IteratorType(typename INTYPE::iterator startPtr_ptr, typename INTYPE::iterator endPtr_ptr)
        : ptr_ptr(startPtr_ptr), endPtr_ptr(endPtr_ptr)
    {
        ptr_ptr--;
        incrementUntilGood();
    }

    bool operator!=(IteratorType rhs) { return ptr_ptr != rhs.ptr_ptr; }

    OUTTYPE& operator*() { return static_cast<OUTTYPE&>(**ptr_ptr); }
    ...
    void operator++() { incrementUntilGood(); }
};
...
struct MapIteratorType
{
    ...
    bool operator!=(MapIteratorType rhs) { return ptr_ptr != rhs.ptr_ptr; }
...
struct Typer
{
    ...
    Typer(INTYPE& baseContainer) : baseContainer(baseContainer) {}

    ITERATOR<TYPE, INTYPE, KEYTYPE> begin()
    {
        return ITERATOR<TYPE, INTYPE, KEYTYPE>(baseContainer.begin(), baseContainer.end());
    }
    ...
    {
        return ITERATOR<TYPE, INTYPE, KEYTYPE>(baseContainer.end(), baseContainer.end());
    }
