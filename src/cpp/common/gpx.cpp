...
struct XmlCloser
{
    ...
    XmlCloser(Trace& trace, string id) : trace{trace}, id{id} { trace << "<" << id << ">"; }

    ~XmlCloser() { trace << "</" << id << ">"; }

    template <typename TYPE>
    Trace& operator<<(const TYPE& content)
    {
        trace << content;
...
void writeGPXEntry(Trace& output, Receiver& rec, KFState& kfState)
{
    ...
    {
        if (kfKey.type != KF::REC_POS || kfKey.str != rec.id)
        {
            continue;
        }
        ...
        {
            if (kfKey2.type != KF::REC_POS || kfKey2.str != rec.id)
            {
                continue;
            }
    ...
    output << "      <trkpt " << "lat='" << pos.latDeg() << "' " << "lon='" << pos.lonDeg() << "' "
           << ">";
