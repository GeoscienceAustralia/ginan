...
/** Write ORBEX header lines and header blocks including FILE/DESCRIPTION and
 * SATELLITE/ID_AND_DESCRIPTION block
 */
...
)
{
    ...
    tracepdeex(
        0,
        orbexStream,
        " START_TIME          %4.0f %2.0f %2.0f %2.0f %2.0f %15.12f\n",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
    ...
    tracepdeex(
        0,
        orbexStream,
        " END_TIME            %4.0f %2.0f %2.0f %2.0f %2.0f %15.12f\n",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
...
)
{
    ...
    if (fabs(clk) >= NO_CLK)
        clk = NO_CLK;
    if (pvStd.x() >= NO_PV_STD)
        pvStd.x() = NO_PV_STD;
    if (pvStd.y() >= NO_PV_STD)
        pvStd.y() = NO_PV_STD;
    if (pvStd.z() >= NO_PV_STD)
        pvStd.z() = NO_PV_STD;
    if (clkStd >= NO_CLK_STD)
        clkStd = NO_CLK_STD;
    ...
    tracepdeex(
        0,
        orbexStream,
        " %s %s              %d %16.4f %16.4f %16.4f %16.7f %7.1f %7.1f %7.1f %11.3f\n",
        recType,
        entry.Sat.id().c_str(),
        nRec,
        pv.x(),
...
)
{
    ...
    if (endFilePos == 0)
    {
        ...
        tracepdeex(
            0,
            orbexStream,
            "*PCS ID_    FLAGS_    N __X______(m)____ ______Y__(m)____ ______Z___(m)___ "
            "_SVCLK___(usec)_ _STD_X_ "
            "_STD_Y_ _STD_Z_ ____STD_CLK\n"
        );
        tracepdeex(
            0,
            orbexStream,
            "*VCS ID_    FLAGS_    N __VX_____(m/s)__ ______VY_(m/s)__ ______VZ__(m/s)_ "
            "_CLKRATE_(ns/s)_ _STD_VX "
            "_STD_VY _STD_VZ ____STD_CLK\n"
        );
        tracepdeex(
            0,
            orbexStream,
            "*POS ID_    FLAGS_    N __X______(m)____ ______Y__(m)____ ______Z___(m)___\n"
        );
        tracepdeex(
            0,
            orbexStream,
            "*VEL ID_    FLAGS_    N __VX_____(m/s)__ ______VY_(m/s)__ ______VZ__(m/s)_\n"
        );
        ...
        tracepdeex(
            0,
            orbexStream,
            "*ATT ID_    FLAGS_    N __q0_______________ ___q1______________ ____q2_____________ "
            "____q3_____________\n"
        );
        tracepdeex(
            0,
            orbexStream,
            "*ATT RECORDS: TRANSFORMATION FROM TERRESTRIAL FRAME COORDINATES (T) TO SAT. BODY "
            "FRAME ONES (B) SUCH AS\n"
        );
        tracepdeex(0, orbexStream, "*                                 (0,B) = q.(0,T).trans(q)\n");

        // tracepdeex(0, orbexStream, "*CPC ID_    FLAGS_    N ______xy_corr____ ______xz_corr____
        // ______xc_corr____
        // ______yz_corr____ ______yc_corr____ ______zc_corr____\n"); tracepdeex(0, orbexStream,
        // "*CVC ID_    FLAGS_ N
        // ____vxvy_corr____ ____vxvz_corr____ ____vxvc_corr____ ____vyvz_corr____ ____vyvc_corr____
        // ____vzvc_corr____\n");
    }
    else
    {
    ...
    tracepdeex(
        0,
        orbexStream,
        "## %4.0f %2.0f %2.0f %2.0f %2.0f %15.12f",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
    ...
    for (auto& [sat, isIncluded] : outFileDat.satList)
    {
        ...
        for (auto& recType : acsConfig.orbex_record_types)
            switch (recType)
            {
                case E_OrbexRecord::PCS:
                {
                    isIncluded |= writePVCS(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::VCS:
                {
                    isIncluded |= writePVCS(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::CPC:
                {
                    break;
                }  // to be added if needed in the future
                case E_OrbexRecord::CVC:
                {
                    break;
                }  // to be added if needed in the future
                case E_OrbexRecord::POS:
                {
                    isIncluded |= writePV(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::VEL:
                {
                    isIncluded |= writePV(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::CLK:
                {
                    isIncluded |= writeClk(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::CRT:
                {
                    isIncluded |= writeClk(orbexStream, entry, recType);
                    break;
                }
                case E_OrbexRecord::ATT:
                {
...
/** Retrieve satellite orbits, clocks and attitudes for all included systems and write out to an
 * ORBEX file
 */
...
)
{
    ...
    for (auto& [Sat, satNav] : nav.satNavMap)
    {
        ...
        orbPass &=
            satpos(nullStream, time, time, obs, orbDataSrcs, E_OffsetType::COM, nav, &kfState);
        ...
        if (attPass)
        {
            ...
            entry.q = quat.conjugate();  // satQuat() returns transformation from body to ECEF, but
                                         // Orbex req's ECEF to body - i.e. quat.conjugate()
        }
