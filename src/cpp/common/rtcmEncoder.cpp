...
)
{
    ...
    map<E_Sys, std::array<GTime, 2>>
        commonClockEpochsMap;  // 0: 1st straddle pt epoch; 1: 2nd straddle pt epoch
    ...
    for (auto& [Sat, ssrOut] : ssrOutMap)
    {
        if (ssrOut.clkInput.vals[0].iode != ssrOut.ephInput.vals[0].iode)
        {
            tracepdeex(
                3,
                std::cout,
                "IODE mismatch between clock and ephemeris for %s\n",
                Sat.id().c_str()
            );
            continue;
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            posCorrections[i] = ssrEphInput.vals[i].brdcPos - ssrEphInput.vals[i].precPos;
            clkCorrections[i] = ssrClkInput.vals[i].brdcClk - ssrClkInput.vals[i].precClk;
                ...
                .extrapolate_corrections)  // todo Eugene: check if ura can be interpolated
        {
            ...
            for (int dt : {0, 1})
            {
                double ephRatio = (ssrEph.t0 + dt - ssrEphInput.vals[0].time).to_double() /
                                  (ssrEphInput.vals[1].time - ssrEphInput.vals[0].time).to_double();
                double clkRatio = (ssrClk.t0 + dt - ssrClkInput.vals[0].time).to_double() /
                                  (ssrClkInput.vals[1].time - ssrClkInput.vals[0].time).to_double();

                Vector3d posCorrection =
                    posCorrections[0] + ephRatio * (posCorrections[1] - posCorrections[0]);
                double clkCorrection =
                    clkCorrections[0] + clkRatio * (clkCorrections[1] - clkCorrections[0]);
                double ura = uras[0] + ephRatio * (uras[1] - uras[0]);

                Vector3d satPosition =
                    ssrEphInput.vals[0].brdcPos +
                    ephRatio * (ssrEphInput.vals[1].brdcPos - ssrEphInput.vals[0].brdcPos);
                Vector3d satVelocity =
                    ssrEphInput.vals[0].brdcVel +
                    ephRatio * (ssrEphInput.vals[1].brdcVel - ssrEphInput.vals[0].brdcVel);
            ...
            tracepdeex(
                6,
                std::cout,
                "\n   RTCM_intp_Clk  %s %s %8.3f",
                referenceTime.to_string().c_str(),
                Sat.id().c_str(),
                ssrClk.dclk[0]
            );
        }
        else
        {
            ...
            tracepdeex(
                6,
                std::cout,
                "\n   RTCM_last_Eph  %s %s %8.4f %8.4f %8.4f",
                referenceTime.to_string().c_str(),
                Sat.id().c_str(),
                ssrEph.deph[0],
                ssrEph.deph[1],
    ...
    for (auto& [Sat, ssrOut] : ssrOutMap)
    {
        ...
        if (messTypeStr == "HR_CLK_CORR")
        {
            ...
            try
            {
                ...
                double dclkReg = lastRegSsrClk.dclk[0] + lastRegSsrClk.dclk[1] * tClk +
