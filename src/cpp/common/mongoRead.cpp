...
RETTYPE getStraddle(GTime referenceTime, deque<INTYPE>& ssrVec)
{
    RETTYPE ssr;
        ...
        for (auto Sat : sats)
        {
            ...
            for (string data : {SSR_EPHEMERIS, SSR_CLOCK})
                for (bool less : {false, true})
                {
                    ...
                    auto docSys = document{} << SSR_SAT << Sat.id() << SSR_DATA << data << SSR_EPOCH
                                             << open_document << moreLess << btime << close_document
                                             << finalize;

                    auto docSort = document{} << SSR_EPOCH << sortDir << finalize;
                    ...
                    for (auto doc : cursor)
                    {
                        ...
                        if (data == SSR_EPHEMERIS)
                        {
                            ...
                            {
                                ephValues.brdcPos(i) =
                                    doc[SSR_POS SSR_BRDC + std::to_string(i)].get_double();
                                ephValues.precPos(i) =
                                    doc[SSR_POS SSR_PREC + std::to_string(i)].get_double();
                                ephValues.brdcVel(i) =
                                    doc[SSR_VEL SSR_BRDC + std::to_string(i)].get_double();
                                ephValues.precVel(i) =
                                    doc[SSR_VEL SSR_PREC + std::to_string(i)].get_double();
                            }
                            ...
                            if (less)
                                ephVec.push_front(ephValues);
                            else
                                ephVec.push_back(ephValues);
                        ...
                        if (data == SSR_CLOCK)
                        {
                            ...
                            // 					std::cout << Sat.id() << " less:" << less << "
                            // brdc:" << broadcast << " " << clkValues.time.to_string(0) << "\n";

                            if (less)
                                clkVec.push_front(clkValues);
                            else
                                clkVec.push_back(clkValues);
            ...
            // 	{
            // 		std::cout << Sat.id() << "Final cbrdcs:" << " iode: " << a.iode <<  " "<<
            // a.time.to_string(0) <<
            // "\n";
            // 	}
            // 	for (auto& a : clkPreciseVec)
            // 	{
            // 		std::cout << Sat.id() << "Final cprecs:" << " iode: " << a.iode <<  " "<<
            // a.time.to_string(0) <<
            // "\n";
            // 	}
            // 	for (auto& a : ephBroadcastVec)
            // 	{
            // 		std::cout << Sat.id() << "Final ebrdcs:" << " iode: " << a.iode <<  " "<<
            // a.time.to_string(0) <<
            // "\n";
            // 	}
            // 	for (auto& a : ephPreciseVec)
            // 	{
            // 		std::cout << Sat.id() << "Final eprecs:" << " iode: " << a.iode <<  " "<<
            // a.time.to_string(0) <<
            // "\n";
            // 	}
            ...
            if (ssrOut.ephInput.valid == false)
            {
                tracepdeex(
                    3,
                    std::cout,
                    "Could not retrieve valid ephemeris for %s\n",
                    Sat.id().c_str()
                );
                continue;
            }
            if (ssrOut.clkInput.valid == false)
            {
                tracepdeex(
                    3,
                    std::cout,
                    "Could not retrieve valid clock     for %s\n",
                    Sat.id().c_str()
                );
                continue;
            }
 ...
 */
void mongoGroupSortBias(bsoncxx::builder::stream::document& doc)
{
    doc << "_id" << open_document << SSR_SAT << "$Sat" << SSR_OBSCODE << "$ObsCode"
        << close_document

        << "lastEpoch" << open_document << "$max" << open_document << "$mergeObjects" << open_array
        << open_document << SSR_EPOCH << "$Epoch" << close_document << "$$ROOT" << close_array
...
)
{
    ...
    for (auto instance : {E_Mongo::PRIMARY, E_Mongo::SECONDARY})
    {
        ...
        // Find the latest document according to t0_time.
        auto docSys = document{} << "Sat" << Sat.id() << finalize;
...
SSRAtm mongoReadCmpAtmosphere(GTime time, SSRMeta ssrMeta)
{
    ...
    for (auto instance : {E_Mongo::PRIMARY, E_Mongo::SECONDARY})
    {
        ...
        auto regDoc = document{} << SSR_DATA << CMP_ATM_META << finalize;

        auto regSort = document{} << "RegionID" << 1 << finalize;
        ...
        for (auto regDoc : cursor)
        {
            ...
            {
                int latNgrid =
                    ROUND((atmRegion.maxLatDeg - atmRegion.minLatDeg) / atmRegion.intLatDeg);
                int lonNgrid =
                    ROUND((atmRegion.maxLonDeg - atmRegion.minLonDeg) / atmRegion.intLonDeg);
                int nind = 0;
                for (int i = 0; i <= latNgrid; i++)
                    for (int j = 0; j <= lonNgrid; j++)
                    {
                        atmRegion.gridLatDeg[nind] =
                            atmRegion.gridLatDeg[0] - atmRegion.intLatDeg * i;
                        atmRegion.gridLonDeg[nind] =
                            atmRegion.gridLonDeg[0] + atmRegion.intLonDeg * j;
                        nind++;
                    }
            }
        ...
        {
            auto trpSel = document{} << SSR_DATA << CMP_TRP_ENTRY << "RegionID" << regId
                                     << SSR_EPOCH << open_document << "$lt" << btime
                                     << close_document << finalize;

            auto trpSort = document{} << SSR_EPOCH << 1 << finalize;
            ...
            auto ionSel = document{} << SSR_DATA << CMP_ION_META << "RegionID" << regId << SSR_EPOCH
                                     << open_document << "$lt" << btime << close_document
                                     << finalize;

            auto ionSort = document{} << SSR_EPOCH << 1 << finalize;
            ...
            {
                auto satSel = document{} << SSR_DATA << CMP_ION_ENTRY << "RegionID" << regId
                                         << SSR_SAT << sat.id() << SSR_EPOCH << open_document
                                         << "$lt" << btime << close_document << finalize;

                auto satSort = document{} << SSR_EPOCH << 1 << finalize;
                ...
                for (auto satDoc : satDocs)
                {
                    ...
                    for (int i = 0; i < regData.ionoPolySize; i++)
                    {
                        ...
                        tracepdeex(
                            6,
                            std::cout,
                            "\n   Mongo_ionP  %s %2d %s %1d %8.4f",
                            tatm.to_string().c_str(),
                            regId,
                            sat.id().c_str(),
                            i,
                            regData.stecData[sat][tatm].poly[i]
                        );
                    }
                    ...
                    if (regData.ionoGrid)
                        for (auto& [ind, lat] : regData.gridLatDeg)
                        {
                            ...
                            tracepdeex(
                                6,
                                std::cout,
                                "\n   Mongo_ionG  %s %2d %s %1d %8.4f",
                                tatm.to_string().c_str(),
                                regId,
                                sat.id().c_str(),
                                ind,
                                regData.stecData[sat][tatm].grid[ind]
                            );
                        }
                }
            }
...
SSRAtm mongoReadIGSIonosphere(GTime time, const SSRMeta& ssrMeta, int masterIod)
{
    ...
    for (auto instance : {E_Mongo::PRIMARY, E_Mongo::SECONDARY})
    {
        ...
        // Find the latest document according to t0_time.
        auto docSys = document{} << SSR_DATA << IGS_ION_META << SSR_EPOCH << open_document << "$lt"
                                 << btime << close_document << finalize;

        auto docSort = document{} << SSR_EPOCH << 1 << finalize;
        ...
        auto docEntr = document{} << SSR_DATA << IGS_ION_ENTRY << SSR_EPOCH << timobj << finalize;
        ...
        for (auto atmDoc : cursor2)
        {
            ...
            if (laydata.maxDegree < sphComp.degree)
                laydata.maxDegree = sphComp.degree;
            if (laydata.maxOrder < sphComp.order)
                laydata.maxOrder = sphComp.order;
        }
...
)
{
    for (auto instance : {E_Mongo::PRIMARY, E_Mongo::SECONDARY})
    {
        ...
        if (std::find(types.begin(), types.end(), KF::ALL) == types.end())
        {
            ...
            for (auto& type : types)
            {
                array << open_document << toString(Constants::Mongo::STATE_DB)
                      << enum_to_string(type) << close_document;
            }
