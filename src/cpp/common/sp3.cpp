...
E_Sys code2sys(char code)
{
    ...
    if (code == 'R')
        return E_Sys::GLO;
    if (code == 'E')
        return E_Sys::GAL; /* extension to sp3-c */
    if (code == 'J')
        return E_Sys::QZS; /* extension to sp3-c */
    if (code == 'C')
        return E_Sys::BDS; /* extension to sp3-c */
    if (code == 'L')
        return E_Sys::LEO; /* extension to sp3-c */
    return E_Sys::NONE;
}
...
)
{
    ...
    @todo change later.
        */
        ...
        if (buff[0] == '#')
        {
            ...
            if (hashCount == 1)
            {
                ...
                int error = str2time(
                    ...
                    28,
                    time
                );  // time system unknown at beginning but does not matter
                if (error)
                    return false;
        ...
        // 		{
        //			plusCount++;
        // 			//number and list of satellites included in the file - information only, sat ids
        // are included in epoch lines.. 			if (lineNum == 2)
        // 			{
