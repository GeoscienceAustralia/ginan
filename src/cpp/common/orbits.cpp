...
bool inertial2Keplers(Trace& trace, const VectorEci& r, const VectorEci& v, Vector6d& keplers)
{
    ...
    if (r.dot(v) >= 0)
        ...
        nu = 2 * PI - acos(e.dot(e_r));

    // Determine the eccentric anomaly
    double E = 2 * atan2(sqrt(1 - e_) * sin(nu / 2), sqrt(1 + e_) * cos(nu / 2));

    // Compute the mean anomaly with help of Kepler’s Equation from the eccentric anomaly E and the
    // eccentricity e
    double M = E - e_ * sin(E);
...
VectorEci keplers2Inertial(Trace& trace, const Vector6d& keplers0)
{
    ...
    double nu = 2 * atan2(sqrt(1 + e_) * sin(E / 2), sqrt(1 - e_) * cos(E / 2));
    ...
    VectorEci rSat;
    rSat.x() =
        x * (cos_w * cos_W - sin_w * cos_i * sin_W) - y * (cos_w * cos_i * sin_W + sin_w * cos_W);
    rSat.y() =
        x * (cos_w * sin_W + sin_w * cos_i * cos_W) + y * (cos_w * cos_i * cos_W - sin_w * sin_W);
...
void getKeplerPartials(Trace& trace, VectorXd& keplers0, MatrixXd& partials)
{
    ...
    double deltas[6] = {1, 1, 1, 0.00001, 0.00001, PI / 180 / 1000};
...
{
    double deltas[6] = {20000, 20000, 20000, 10, 10, 10};
...
)
{
    ...
    Vector3d L = Vector3d(
        keplers0[static_cast<int>(KEPLER::LX)],
        keplers0[static_cast<int>(KEPLER::LY)],
        keplers0[static_cast<int>(KEPLER::LZ)]
    );

    double h = L.norm();

    Vector2d E =
        Vector2d(keplers0[static_cast<int>(KEPLER::EU)], keplers0[static_cast<int>(KEPLER::EV)]);
    ...
    if (j2)
    {
        ...
        for (auto& rSat : {rSat, newPos1})
        {
            ...
            Vector3d vec =
                Vector3d(x * (commonTerm - 1), y * (commonTerm - 1), z * (commonTerm - 3));
...
VectorEci
propagateFull(Trace& trace, GTime time, double dt, VectorEci& rSat, VectorEci& vSat, SatPos& satPos)
{
