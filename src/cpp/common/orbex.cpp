        ...
        if (line[0] == ' ')
        {
            ...
            if (split[0] == "TIME_SYSTEM")
            {
                ...
                if (timeSysStr == "GPS")
                    ...
                    tsys = E_TimeSys::GLONASST;
                else if (timeSysStr == "TT")
                    tsys = E_TimeSys::TT;
                else
                {
                    BOOST_LOG_TRIVIAL(error) << "Unknown Orbex time system: " << timeSysStr;
                    return false;
                }
        ...
        else if (line[0] == '-')
        {
            ...
            if (line != closure)
            {
                BOOST_LOG_TRIVIAL(error)
                    << "Incorrect block closure line encountered: " << line << " != " << closure;
                return false;
            }
            else
            {
 ...
 * Note: This function is currently not fully implemented, satellite ID's are read from
 * EPHEMERIS/DATA block
 */
bool readOrbexSatId(ifstream& fileStream)  ///< Stream to read content from
{
    while (fileStream)
    {
        string line;
...
)
{
    ...
    while (fileStream)
    {
        ...
        if (line[0] == '#' && line[1] == '#')
        {
            ...
            if (error)
            {
                BOOST_LOG_TRIVIAL(error) << "Invalid epoch line in Orbex file: " << line;
                return false;
            }
        ...
        else if (line[0] == ' ')
        {
            if (nsat == 0)
            {
                BOOST_LOG_TRIVIAL(error) << "Epoch line invalid or not found before data records";
                return false;
            }
            ...
            if (recType == "ATT")
            {
                ...
                if (nRec != 4)
                {
                    BOOST_LOG_TRIVIAL(error) << "Invalid number of data columns: " << nRec;
                    return false;
                }

                double val[4];
                int    found =
                    sscanf(buff + 24, "%lf %lf %lf %lf", &val[0], &val[1], &val[2], &val[3]);
                ...
                {
                    BOOST_LOG_TRIVIAL(warning) << "The quaternion is not approximately unit norm";
                    continue;
                }
            ...
            */
            ...
            {
                BOOST_LOG_TRIVIAL(error) << "Unsupported record type: " << recType;
                return false;
            }
        }
        else if (line[0] == '-')
...
)
{
    ...
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "Orbex file open error " << filepath;
    ...
    if (failure)
    {
        BOOST_LOG_TRIVIAL(error) << "Error reading Orbex header lines";
        return;
    }
    ...
    while (fileStream)
    {
        ...
        else if (line == "+FILE/DESCRIPTION")
            pass = readOrbexFileDesc(fileStream, tsys, frame);
        else if (line == "+SATELLITE/ID_AND_DESCRIPTION")
            pass = readOrbexSatId(fileStream);
        else if (line == "+EPHEMERIS/DATA")
            pass = readOrbexEph(fileStream, nav, tsys, frame);
        else if (line == "%END_ORBEX")
            return;  // end of file

        if (pass == false)
        {
            BOOST_LOG_TRIVIAL(error) << "Error reading Orbex " << line << " block";
            return;
        }
    }
}
