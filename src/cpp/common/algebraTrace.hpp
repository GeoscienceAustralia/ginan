...
struct TransitionMatrixObject
{
    ...
    TransitionMatrixObject(const MatrixXd& rhs)
    {
 ...
 * Uses a binary archive which requires all of the relevant class members to have serialization
 * functions written. Output format is TypeId, ObjectData, NumBytes - this allows seeking backward
 * from the end of the file to the beginning of each object.
 */
template <class TYPE>
