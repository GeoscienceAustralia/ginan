...
)
{
    ...
    if (fabs(tEph) > ssrEph.udi * acsConfig.validity_interval_factor)
    {
        ...
        tracepdeex(
            2,
            std::cout,
            "age of ssr error: %s t=%.0f > %.0f\n",
            time.to_string().c_str(),
            tEph,
            ssrEph.udi * acsConfig.validity_interval_factor
        );
        return false;
    }

    dPos = ssrEph.deph + ssrEph.ddeph * tEph;
    ...
    if (dPos.norm() > MAXECORSSR)
    {
        ...
        tracepdeex(
            2,
            std::cout,
            "SSR pos correction too large : %s %s deph=%.1fm\n",
            time.to_string().c_str(),
            satPos.Sat.id().c_str(),
            dPos.norm()
        );
    ...
    tracepdeex(
        ...
        iodEph,
        tEph,
        validStart.to_string().c_str(),
        validStop.to_string().c_str(),
        dPos.norm()
    );
...
)
{
    ...
    if (clkIt == ssrMaps.ssrClk_map.end())
    {
        ...
        tracepdeex(
            4,
            std::cout,
            "No SSR corrections before %s sat=%s\n",
            ephTime.to_string().c_str(),
            satPos.Sat.id().c_str()
        );
        return false;
    }
    ...
    dclk = ssrClk.dclk[0] + ssrClk.dclk[1] * tClk + ssrClk.dclk[2] * tClk * tClk;
    ...
    if (fabs(dclk) > MAXCCORSSR)
    {
        ...
        tracepdeex(
            2,
            std::cout,
            "SSR clk correction too large : %s %s dclk=%.1f\n",
            time.to_string().c_str(),
            satPos.Sat.id().c_str(),
            dclk
        );
...
bool satPosSSR(Trace& trace, GTime time, GTime teph, SatPos& satPos, Navigation& nav)
{
    ...
    while (true)
    {
        ...
        if (posDeltaPass == false || clkDeltaPass == false)
        {
        ...
        if (ephValidStart >= clkValidStop || clkValidStart >= ephValidStop)
        {
            ...
            if (ephValidStart >= clkValidStop)
                ephTime = clkValidStop - 0.5;
            if (clkValidStart >= ephValidStop)
                ephTime = ephValidStop - 0.5;
            continue;
        }
        ...
        if (pass == false)
        {
            if (once)
            {
                ...
                if (clkValidStart < ephValidStart)
                    ephTime = clkValidStart - 0.5;
                else
    ...
    tracepdeex(
        4,
        trace,
        "\nBRDCEPH %s    %s    %13.3f %13.3f %13.3f %11.3f %2d",
        time.to_string().c_str(),
        Sat.id().c_str(),
        rSat0[0],
        rSat0[1],
    ...
    tracepdeex(
        3,
        trace,
        "\nSSR_EPH %s    %s    %13.3f %13.3f %13.3f %11.3f ",
        time.to_string().c_str(),
        Sat.id().c_str(),
        rSat[0],
        rSat[1],
    ...
    tracepdeex(
        5,
        trace,
        "\n%s: %s sat=%s deph=%6.3f %6.3f %6.3f dclk=%6.3f var=%6.3f iode=%d clktimes:%s %s "
        "ephtimes%s %s\n",
        __FUNCTION__,
        time.to_string().c_str(),
        Sat.id().c_str(),
    ...
    if (round(time.bigTime) == time.bigTime)
    {
        traceJson(
            ...
            nullStream,
            time,
            {{"data", "satPosSSR"}, {"Sat", satPos.Sat.id()}},
            {{"rSat0[0]", rSat0[0]},
             {"rSat0[1]", rSat0[1]},
             {"rSat0[2]", rSat0[2]},
             {"rSat[0]", rSat[0]},
