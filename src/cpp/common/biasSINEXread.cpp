 ...
 */
void read_biasSINEX_head(const char* buff)  ///< Line to read
{
...
)
{
    ...
    {
        BOOST_LOG_TRIVIAL(error) << "Unkown time system for bias SINEX file: "
                                 << enum_to_string(tsys);
        return false;
    }
    ...
    if (units == "ns ")
        fact = CLIGHT / 1e9;
    else if (units == "cyc" && entry.measType == PHAS && lam1 > 0)
        fact = lam1;
    else
    {
        return false;
 ...
 */
bool readBiasSinex(string& filename)  ///< File to read
{
    int       nbia     = 0;
    bool      datasect = false;
    ...
    while (std::getline(inputStream, line))
    {
        ...
        if (strstr(buff, "TIME_SYSTEM"))
        {
            ...
            if (timeSystem != " ")
            {
                if (timeSystem == "G")
                    tsys = E_TimeSys::GPST;
                else if (timeSystem == "C")
                    tsys = E_TimeSys::BDT;
                else if (timeSystem == "R")
                    tsys = E_TimeSys::GLONASST;
                else if (timeSystem == "U")
                    tsys = E_TimeSys::UTC;
                else if (timeSystem == "T")
                    tsys = E_TimeSys::TAI;
                else
                {
        ...
        if (strstr(buff, " DSB ") || strstr(buff, " OSB "))
        {
