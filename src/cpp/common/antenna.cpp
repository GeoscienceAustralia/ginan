...
Vector3d makeAntPco(string id, E_Sys sys, E_FType ftx, GTime time, double& var, E_Radio radio)
{
    ...
    for (auto& [ft, pcoFreq] : pcoFreqMap)
    {
        ...
        else
        {
            ...
            if (close1 > close2 && close1 > 0)
            {
                lam1 = lam;
                pco1 = pco;
            }
            if (close2 > close1 && close2 > 0)
            {
                lam2 = lam;
                pco2 = pco;
            }
    ...
    Vector3d pco3 = k32 * pco1 - k31 * pco2;
    return pco3;
}
...
)
{
    ...
    else
    {
        ...
        double yz3 = pcvMap2D[az_n - 1][zen_n - 1];
        double yz1 = pcvMap2D[az_n - 1][zen_n];
        double yz4 = pcvMap2D[az_n][zen_n - 1];
        double yz2 = pcvMap2D[az_n][zen_n];
...
void radome2none(string& antenna_type)
{
    ...
    {
        printf(
            "\n*** ERROR radome2none(): string length is less then 20 characters received %ld "
            "characters\n",
            length
        );
        return;
...
map<string, E_FType> antexCodes = {{"G01", F1}, {"G02", F2}, {"G05", F5}, {"R01", G1}, {"R02", G2},
                                   {"R04", G4}, {"R06", G6}, {"E01", F1}, {"E05", F5}, {"E06", F6},
                                   {"E07", F7}, {"E08", F8}, {"C01", F1}, {"C02", B1}, {"C05", F5},
                                   {"C06", B3}, {"C07", F7}, {"C08", F8}, {"J01", F1}, {"J02", F2},
                                   {"J05", F5}, {"J06", F6}, {"S01", F1}, {"S05", F5}, {"I05", F5},
                                   {"I09", I9}};
...
void readantexf(string filepath, Navigation& nav)
{
    ...
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "Error opening antex file" << filepath << "\n";
        return;
    }
    ...
    while (fileStream)
    {
        ...
        if (strstr(comment, "ANTEX VERSION / SYST"))
        {
            continue;
        }
        if (strstr(comment, "PCV TYPE / REFANT"))
        {
            continue;
        }
        ...
        if (strstr(comment, "END OF HEADER"))
        {
            continue;
        }
        ...
        if (strstr(comment, "METH / BY / # / DATE"))
        {
            ...
            // 			char cal_agency[20];
            // 			char cal_date[10];
            // 			strncpy(cal_method,	buff,		20);/* Should be CHAMBER or FIELD or ROBOT
            // or COPIED ot CONVERTED */ 			cal_method[19] = '\0';
            // strncpy(cal_agency, buff + 20,	20); cal_agency[19] = '\0'; 			strncpy(tmp,
            // buff + 40,	10); 			num_calibrated = atoi(tmp);
            // strncpy(cal_date, buff + 50,	10); 			cal_date[9] = '\0';
        ...
        if (strstr(comment, "DAZI"))
        {
            ...
            if (recPcv.aziDelta < 0.0001)
                recPcv.naz = 0;
            else
                recPcv.naz = (360 / recPcv.aziDelta) + 1;
        ...
        if (strstr(comment, "TYPE / SERIAL NO"))
        {
            ...
            if (satId.find_first_not_of(' ') == satId.npos)
            {
                id = recPcv.type;
            }
            else
        ...
        if (strstr(comment, "ZEN1 / ZEN2 / DZEN"))
        {
            ...
            strncpy(tmp, buff, 8);
            tmp[8]          = '\0';
            recPcv.zenStart = atof(tmp);
            strncpy(tmp, buff + 8, 7);
            tmp[8]         = '\0';
            recPcv.zenStop = atof(tmp);
            strncpy(tmp, buff + 16, 7);
            tmp[8]          = '\0';
            recPcv.zenDelta = atof(tmp);
        ...
        if (strstr(
                comment,
                "NORTH / EAST / UP"
            ))  // "NORTH / EAST / UP" for receiver and "X / Y / Z" for satellite
        {
            double neu[3];
        ...
        if (strstr(comment, "START OF FREQ RMS"))
        {
            irms = 1;
            continue;
        }
        if (strstr(comment, "END OF FREQ RMS"))
        {
            irms = 0;
            continue;
        }

        if (irms == 0 && strstr(buff, "NOAZI"))
        {
            char tmp[10];
