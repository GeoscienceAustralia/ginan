...
struct GTow : Double
{
    GTow(double in) { val = in; }
};

struct BTow : Double
{
    BTow(double in) { val = in; }
};

struct RTod : Double
{
    RTod(double in) { val = in; }
};
...
struct Duration
{
    ...
    double to_double() const { return (double)bigTime; }

    long int to_int() const { return (long int)bigTime; }
    ...
    long double operator-(const Duration& t2) const { return this->bigTime - t2.bigTime; }

    double operator/(const Duration& t2) const { return this->bigTime / t2.bigTime; }
...
struct MjDateUt1
{
    ...
    MjDateUt1(GTime time, double ut1_utc);

    double to_double() const { return (double)val; }

    double to_j2000() const { return (double)(val - MJD_j2000); }
};
...
struct UtcTime
{
    long double bigTime;  // Eugene: bigTime can be ambiguous, e.g. 1167264000.5, never know if GPST
                          // is 2017-01-01 00:00:17.5 or 2017-01-01 00:00:18.5
...
struct UYds : array<double, 3>
{
    ...
    UYds(double yearval = 0, double doyVal = 0, double sodVal = 0)
        : year{(*this)[0]}, doy{(*this)[1]}, sod{(*this)[2]}
    {
        year = yearval;
        doy  = doyVal;
        sod  = sodVal;
    ...
    UYds& operator+=(const double offset)
    {
        sod += offset;
