...
)
{
    ...
    if (firstWrite)
        filePosMap[filename][E_FilePos::COORD] = fout.tellp();
    ...
    for (int i = 0; i < 3; i++)
    {
        kfFound = kfFound && (kfState.getKFValue({KF::REC_POS, {}, rec.id, i}, recPosEcef(i)) !=
                              E_Source::NONE);
    }
    ...
    tracepdeex(
        0,
        fout,
        "%12.6lf%12.6lf%12.3lf%12.3lf%12.3lf\n",
        recPos.latDeg(),
        recPos.lonDeg(),
    ...
    if (firstWrite)
        filePosMap[filename][E_FilePos::CURR_TIME] = fout.tellp();
    ...
    if (firstWrite)
    {
        ...
        tracepdeex(
            ...
            acsConfig.cost_time_interval / 60
        );                                      // Batch updating interval (min)
    }

    if (firstWrite)
        filePosMap[filename][E_FilePos::TOTAL_TIME] = fout.tellp();
    fout.seekp(filePosMap[filename][E_FilePos::TOTAL_TIME]);
    tracepdeex(0, fout, "%5d\n", duration / 60);  // Total length of batch time series

    if (firstWrite)
        filePosMap[filename][E_FilePos::PCDH] = fout.tellp();
    ...
    union
    {
        ...
        struct
        {
            unsigned isRealTime : 1;    ///< Data processed in near-real time [false: (re-)processed
                                        ///< off-line]
    ...
    {
        if (acsConfig.process_sys[obs.Sat.sys] == false || obs.exclude)
        {
            continue;
        }
    ...
    for (auto& [key, index] : kfState.kfIndexMap)
    {
        ...
        if (acsConfig.pppOpts.equate_tropospheres == false && key.str != rec.id)
        {
            continue;
        }
    ...
    if (tropFound[1])
    {
        nsGrad    = gradM * tropStates[1];
        nsGradStd = sqrt(tropVars[1]);
    }
    if (tropFound[2])
    {
        ewGrad    = gradM * tropStates[2];
        ewGradStd = sqrt(tropVars[2]);
    }
    ...
    tracepdeex(
        ...
        fout,
        " %02d %02d %02d "
        "%08x%7.1lf%7.1lf%7.1lf%7.1lf%7.1lf%7.1lf%7.1lf%7.2lf%7.2lf%7.2lf%7.2lf%8.3lf\n",
        ...
        -99.999
    );                                       // Vertically integrated TEC (TECU)

    tracepdeex(0, fout, "%4d\n", obsCount);  // Number of slant samples to follow

    for (auto& obs : only<GObs>(rec.obsList))
    {
        if (acsConfig.process_sys[obs.Sat.sys] == false || obs.exclude || obs.tropSlant == 0)
        {
            continue;
        }
        ...
        tracepdeex(
            ...
            satStat.el * R2D
        );                                  // Slant ele angle (from local horizon)
    }
