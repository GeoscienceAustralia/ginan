...
)
{
    ...
    biasMaps[entry.measType][id][entry.cod1][entry.cod2][entry.tini] = entry;
...
bool calculateBias(
    ...
    const TimeBiasMap& timeBiasMap  ///< Bias map for given measrement type, device & observation
...
)
{
    ...
    // get the first bias in that map that comes after the desired time, if no entry comes after,
    // use the earlier one (same to bias1) for extrapolation purpose
...
{
    if (current == GTime::noTime() || (current > potential && potential != GTime::noTime()))
    {
        current = potential;
    }
}
...
bool biasRecurser(
    ...
    const ObsObsBiasMap& obsObsBiasMap,  ///< Bias map for given measrement type & device, as
                                         ///< obsObsBiasMap[code1][code2][time]
...
)
{
    ...
    if (acsConfig.ssrInOpts.one_freq_phase_bias && measType == PHAS)
    {
