 ...
 * 3.  D.A.Vallado, Fundamentals of Astrodynamics and Applications 2nd ed, Space Technology Library,
 * 2004
 ...
 * 6.  MacMillan et al., Atmospheric gradients and the VLBI terrestrial and celestial reference
 * frames, Geophys. Res. Let., 1997
 * 7.  G.Petit and B.Luzum (eds), IERS Technical Note No. 36, IERS Conventions (2010), 2010
 * 8.  J.Kouba, A simplified yaw-attitude model for eclipsing GPS satellites, GPS Solutions,
 * 13:1-12, 2009
 * 9.  F.Dilssner, GPS IIF-1 satellite antenna phase center and attitude modeling, InsideGNSS,
 * September, 2010
...
void testEclipse(ObsList& obsList)
{
    ...
    for (auto& obs : only<GObs>(obsList))
    {
        ...
        /* only block IIA */
        // 		if (obs.satNav_ptr->pcv.type == "BLOCK IIA")		//todo take from the satSys
        // object 			continue;
        ...
        /* test eclipse */
        if (ang < PI / 2 || r * sin(ang) > RE_WGS84)
            continue;

        // 		trace(3, "eclipsing sat excluded %s sat=%s\n", obs.time.to_string(0).c_str(),
        // obs.Sat.id().c_str());
...
struct SatGeom
{
    ...
    double mu = 0;  ///< Angle of sat from 'midnight' (when sat is at the furthest point from Sun in
                    ///< its orbit)
 ...
 */
SatGeom satOrbitGeometry(SatPos& satPos)  ///< Observation
{
    SatGeom satGeom;
    ...
    for (int dt : {DT, 0})
    {
        ...
        if (eSat.dot(eSun) <= 0)
            mu[dt] = PI / 2 - E;  // sat on dark side
        else
            mu[dt] = PI / 2 + E;  // sat on noon side
...
double nominalYawGps(
    ...
    double mu  ///< Angle of sat from 'midnight' (when sat is at the furthest point from Sun in its
               ///< orbit)
)
{
 ...
 */
double nominalYawRate(SatGeom& satGeom)  ///< Satellite geometry
{
...
)
{
    ...
    else  // partial overlap
    {
        ...
        intersection =
            SQR(r1) * acos(d1 / r1) - d1 * sqrt(SQR(r1) - SQR(d1)) + SQR(r2) * acos(d2 / r2) -
            d2 * sqrt(
                     SQR(r2) - SQR(d2)
                 );  // Ref: https://diego.assencio.com/?index=8d6ca3d82151bad815f78addf9b5c1c6
    }
...
)
{
    ...
    for (auto& bodyPosRadius : {earthPosRadius, moonPosRadius})
    {
        ...
        double fraction =
            circleAreaVisible(sunRadiusAngularSize, bodyRadiusAngularSize, separationAngularSize);
        if (fraction < 1)
            return fraction;
    }
 ...
 * Ref:
 * https://www.researchgate.net/publication/306924379_Observed_features_of_GPS_Block_IIF_satellite_yaw_maneuvers_and_corresponding_modeling
...
)
{
    ...
    wrapPlusMinusPi(dYaw);
    if (maxYawRateFound == false || modelYawTime == GTime::noTime())
    {
        modelYaw = nominalYaw;
        return true;
    }
    ...
    while (currTime < time)
    {
        ...
        wrapPlusMinusPi(dYaw);
        if (abs(dYaw) / dt > maxYawRate && startTime == GTime::noTime())  // Not exiting eclipse
        {
            if (startSign == 0)                                           // Entering catchup
            {
                double currBeta = satGeom.beta + satGeom.betaRate * dtFromTime;
                startSign       = SGN(dYaw);
                if (abs(currBeta) < abs(betaBias) && SGN(currBeta) == SGN(betaBias))
                {
                    startSign *= -1;  // beta angles between 0 & betaBias result in opposite
                                      // direction catch-up steering
                }
            }
...
)
{
    ...
    attStatus.excludeTime =
        time + 30 * 60;  // Recommended to exclude up to 30min after exiting eclipse
    return true;
}
...
)
{
    ...
    if (abs(betaModified) < betaThreshold)
        betaModified =
            beta + (SGN(beta) * betaThreshold - beta) / (1 + 13000 * pow(sin(satGeom.mu), 4));
 ...
 */
double
nominalYawGalIov(Vector3d& eSunOrf)  ///< Unit vector to Sun in Orbital reference frame {A,-C,-R}
{
    return atan2(-eSunOrf(1) / sqrt(1 - SQR(eSunOrf(2))), -eSunOrf(0) / sqrt(1 - SQR(eSunOrf(2))));
}
 ...
 * Ref: https://www.gsc-europa.eu/support-to-developers/galileo-satellite-metadata#3.1
 * Ref:
 * https://github.com/groops-devs/groops/blob/main/source/programs/simulation/simulateStarCameraGnss.cpp
 */
bool satYawGalIov(
    SatSys&    Sat,        ///< Satellite ID
    AttStatus& attStatus,  ///< Satellite att status
...
)
{
    ...
    attStatus.nominalYaw = nominalYawGalIov(eSunOrf);
    wrapPlusMinusPi(attStatus.nominalYaw);
    attStatus.modelYaw = attStatus.nominalYaw;

    // Midnight/noon turns
    double sinBetaX = sin(15 * D2R);
    double sinBetaY = sin(2 * D2R);
    if (abs(eSunOrf(0)) < sinBetaX && abs(eSunOrf(1)) < sinBetaY)
    {
        if (attStatus.modelYawTime ==
            GTime::noTime())  // Check sat not mid-way through eclipse on startup
            return false;
        ...
        eSunAux(0) = eSunOrf(0);
        eSunAux(1) =
            0.5 * (sinBetaY * startSign + eSunOrf(1)) +
 ...
 */
double colinearAngle(double mu)  ///< Mu angle (rad)
{
...
)
{
    ...
    if (abs(beta) < betaThresh && colinearAngle(mu) < colAngThresh)
    {
...

{
    ...
    while (time > earliestTime)
    {
        ...
        double yawFromMid = abs(PI / 2 - abs(currNominalYaw));
        wrapPlusMinusPi(yawFromMid);
        double angleFromNoon = PI - currMu;
        wrapPlusMinusPi(angleFromNoon);
        double angleFromMid = std::min(abs(currMu), abs(angleFromNoon));
        wrapPlusMinusPi(angleFromMid);  // Orbital angle
...
)
{
    ...
    double yawFromNoon = abs(PI / 2 - abs(nominalYaw));
    wrapPlusMinusPi(yawFromNoon);  // from current position to noon
    double angleFromNoon = PI - mu;
    wrapPlusMinusPi(angleFromNoon);
 ...
 * Will set nominalYaw & modelYaw to the orbit-normal yaw (0 or PI) closest to the given nominal
 * yaw.
 */
bool satYawOrbNor(
    AttStatus& attStatus,      ///< Satellite att status
...
)
{
    ...
    if (abs(nominalYaw) > PI / 2)
        offset = PI;
    attStatus.nominalYaw = orbitNormalYaw() + offset;
    wrapPlusMinusPi(attStatus.modelYaw);
    attStatus.modelYaw = orbitNormalYaw() + offset;
    wrapPlusMinusPi(attStatus.modelYaw);
    return true;
}

/** Yaw model for QZSS-1 satellites
 * Requires qzss_yaw_modes.snx - generated by scripts/qzss_ohi_merge.py and ohi-qzs*.txt's from
 * https://qzss.go.jp/en/technical/qzssinfo/index.html Ref:
...
)
{
    ...
    if (attModeFound && attMode.substr(0, 2) == "ON")
    {
...
)
{
    ...
    double maxYawRate =
        0.055 *
        D2R;  // Ref: https://qzss.go.jp/en/technical/qzssinfo/khp0mf0000000wuf-att/spi-qzs2_c.pdf
    double yawFromMid = abs(PI / 2 - abs(attStatus.nominalYaw));
    wrapPlusMinusPi(yawFromMid);
    double angleFromNoon = PI - mu;
    wrapPlusMinusPi(angleFromNoon);
    double angleFromMid = std::min(abs(mu), abs(angleFromNoon));
    wrapPlusMinusPi(angleFromMid);
    double timeTillMid = angleFromMid / muRate;
    double midYawRate  = yawFromMid / timeTillMid;
    if (abs(midYawRate) > maxYawRate)
    {
...
)
{
    ...
    attStatus.modelYaw   = attStatus.nominalYaw;
    double alpha         = PI - mu;
    wrapPlusMinusPi(alpha);
    double beta0 = 3 * D2R;
    double mu0   = 6 * D2R;

    if ((abs(beta) <= beta0) && (abs(alpha) <= mu0 || abs(mu) <= mu0))
    {
        if (alpha == 0 || beta == 0)
        {
        ...
        if (attStatus.startTime == GTime::noTime())  // start of switchover period
        {
            ...
            double currMu    = mu;
            GTime  currTime  = time;
            while ((abs(currBeta) <= beta0) && (abs(currAlpha) <= mu0 || abs(currMu) <= mu0))
            {
                currTime += -1;
                currBeta = beta + betaRate * (currTime - time).to_double();
                wrapPlusMinusPi(currBeta);
                currMu = mu + muRate * (currTime - time).to_double();
                wrapPlusMinusPi(currMu);
                currAlpha = PI - currMu;
                wrapPlusMinusPi(currAlpha);
            }
...
)
{
    ...
    string attMode;
    bool   attModeFound = getSnxSatAttMode(Sat.svn(), time, attMode);
    if (attModeFound && attMode.substr(0, 7) == "BETA_SY")
    {
...
)
{
    ...
    if (abs(beta) <= beta0)
    {
        ...
        if ((startSign == 0) || (startSign != SGN(beta) && absYaw < 5 * D2R))
        {
            startSign = SGN(beta);
        }
...
/** Calculates unit vectors of satellite-fixed coordinates (ECEF) given yaw (assuming Z+ is facing
 * toward Earth)
 */
void yawToAttVecs(
...
void updateSatYaw(
    ...
    AttStatus& attStatus  ///< Satellite att status. Use a disposable copy if calling inside
                          ///< multithreaded code
)
{
    if (satPos.posTime <= attStatus.modelYawTime)
        return;  // The requested time is in the past, all the models are designed to propagate
                 // forwards, so we're out of luck.
    ...
    switch (blockType)
    {
        ...
        case E_Block::GPS_II:
        case E_Block::GPS_IIA:
        {
            modelYawValid = satYawGpsIIA(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::GPS_IIR_A:
        case E_Block::GPS_IIR_B:
        case E_Block::GPS_IIR_M:
        {
            modelYawValid = satYawGpsIIR(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::GPS_IIF:
        {
            modelYawValid = satYawGpsIIF(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::GPS_IIIA:
        {
            modelYawValid = satYawGpsIII(attStatus, satGeom);
            break;
        }
        case E_Block::GLO_M:
        case E_Block::GLO_MP:
        case E_Block::GLO:
        {
            modelYawValid = satYawGlo(Sat, attStatus, time, satGeom);
            break;
        }
        ...
        case E_Block::GLO_K2:
        {
            modelYawValid = satYawGloK(Sat, attStatus, time, satGeom);
            break;
        }
        ...
        case E_Block::GAL_1:
        {
            modelYawValid = satYawGalIov(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::GAL_2:
        {
            modelYawValid = satYawGalFoc(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::BDS_2M:
        {
            modelYawValid = satYawBds2(Sat, attStatus, time, satGeom, 3090);
            break;
        }
        case E_Block::BDS_2G:
        {
            modelYawValid = satYawOrbNor(attStatus, PI);
            break;
        }
        case E_Block::BDS_2I:
        {
        ...
        case E_Block::BDS_3M_SECM_A:
        {
            modelYawValid = satYawBds3Secm(attStatus, satGeom);
            break;
        }
        case E_Block::BDS_3G:
        {
        ...
        case E_Block::QZS_1:
        {
            modelYawValid = satYawQzs1(Sat, attStatus, time, satGeom);
            break;
        }
        case E_Block::QZS_2G:
        {
        ...
        case E_Block::QZS_2I:
        {
            modelYawValid = satYawQzs2I(Sat, attStatus, time, satGeom);
            break;
        }
 ...
 * Returns false if no attitude available (e.g. not found in file, not enough data to interpolate,
 * etc.)
 */
...
{
    auto attMapItr = nav.attMapMap.find(id);
    if (attMapItr == nav.attMapMap.end())
        return false;
    auto& [dummy, attMap] = *attMapItr;
    auto entryItr2        = attMap.lower_bound(time);
    if (entryItr2 == attMap.end())
        return false;

    auto entryItr1 = entryItr2;
    if (entryItr1 != attMap.begin())
        entryItr1--;
    else if (entryItr2 != attMap.end())
        entryItr2++;
    else
        return false;
    ...
    double frac = (time - t1).to_double() / (t2 - t1).to_double();
    ...
    if (t1 < time -
                 6.0 * 60 * 60  // 6hrs away (180deg behind orbitwise) is too far to slerp reliably
        || t2 > time + 6.0 * 60 * 60)
    {
        BOOST_LOG_TRIVIAL(warning)
...
)
{
    ...
    for (auto attitudeType : attitudeTypes)
    {
        switch (attitudeType)
        {
            ...
            case E_Source::MODEL:
            {
                valid = satAttModel(rSat, vSat, attStatus, E_Source::MODEL);
                break;
            }
            case E_Source::PRECISE:
            {
                valid = preciseAttitude(Sat, time, attStatus);
                break;
            }
        }
 ...
 */
void updateSatAtts(SatPos& satPos)  ///< observation
{
...
)
{
    ...
    for (auto& attitudeType : attitudeTypes)
    {
        switch (attitudeType)
        {
            ...
            case E_Source::NOMINAL:
            {
                valid = basicRecAttitude(rec, attStatus);
                break;
            }
            case E_Source::PRECISE:
            {
                valid = preciseAttitude(rec.id, time, attStatus);
                break;
            }
            case E_Source::KALMAN:
            {
                valid = kalmanAttitude(rec.id, time, attStatus, kfState_ptr);
                break;
            }
            case E_Source::REMOTE:
            {
                valid = kalmanAttitude(rec.id, time, attStatus, remote_ptr);
                break;
            }
        }
...
)
{
    ...
    // Get angle offset by looking at receiver's new components alignment with satellite's new x
    // component
