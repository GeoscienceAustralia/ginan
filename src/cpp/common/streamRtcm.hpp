...
struct RtcmParser : Parser, RtcmDecoder
{
    ...
    void parse(std::istream& inputStream)
    {
        ...
        if (qzssL6)  // todo aaron move to own decoder type
        {
            ...
            while (inputStream)
            {
                ...
                inputStream.read((char*)data + 4, 2);
                CLEAN_UP_AND_RETURN_ON_FAILURE;
                inputStream.read((char*)data + 6, 244);
                CLEAN_UP_AND_RETURN_ON_FAILURE;
                ...
                while (frameBits > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (messType > 0)
                        {
                            ...
                            while (messType != 4073 && (frameBits + 13) < qzsL6BitsLeft)
                            {
                                frameBits++;
                                int j    = frameBits;
            ...
            while (inputStream)
            {
                ...
                vector<unsigned char> data(dataFrameLength);
                inputStream.read((char*)data.data() + 3, messageLength);
                CLEAN_UP_AND_RETURN_ON_FAILURE;
                ...
                if (rtcmReturnType == E_ReturnType::WAIT)
                {
                    inputStream.seekg(pos);
                    return;
                }
    ...
    string parserType() { return "RtcmParser"; }
};
