...
void outputNavRinexEph(Eph& eph, Trace& trace, const double rnxver)
{
    ...
    tracepdeex(
        0,
        trace,
        "%-3s %04.0f %02.0f %02.0f %02.0f %02.0f %02.0f",
        eph.Sat.id().c_str(),
        ep[0],
        ep[1],
        ep[2],
    ...
    if (sys != E_Sys::BDS)
    {
        traceFormatedFloat(trace, eph.iode, formatStr);
    } /* GPS/QZS: IODE, GAL: IODnav */
    else
    {
        traceFormatedFloat(trace, eph.aode, formatStr);
    } /* BDS: AODE */
    ...
    traceFormatedFloat(
        ...
        eph.week,
        formatStr
    ); /* GPS/QZS: GPS week, GAL: GAL week, BDS: BDT week */
    ...
    if (sys == E_Sys::GAL)
    {
        traceFormatedFloat(trace, svaToSisa(eph.sva), formatStr);
    }
    else
    {
        traceFormatedFloat(trace, svaToUra(eph.sva), formatStr);
    }
    ...
    traceFormatedFloat(
        ...
        eph.tgd[0],
        formatStr
    ); /* GPS/QZS:TGD, GAL:BGD E5a/E1, BDS: TGD1 B1/B3 */

    if (sys == E_Sys::GAL || sys == E_Sys::BDS)
    {
        traceFormatedFloat(trace, eph.tgd[1], formatStr);
    } /* GAL:BGD E5b/E1, BDS: TGD2 B2/B3 */
    else
    {
        traceFormatedFloat(trace, eph.iodc, formatStr);
    } /* GPS/QZS:IODC */
    trace << "\n";
    ...
    if (sys != E_Sys::BDS)
    {
        traceFormatedFloat(trace, GTow(eph.ttm), formatStr);
    }
    else
    {
        traceFormatedFloat(trace, BTow(eph.ttm), formatStr);
    }

    if (sys == E_Sys::GPS)
    {
        traceFormatedFloat(trace, eph.fit, formatStr);
    } /* fit interval in hours for GPS */
    else if (sys == E_Sys::QZS)
    {
        traceFormatedFloat(trace, eph.fitFlag, formatStr);
    } /* fit interval flag for QZS */
    else if (sys == E_Sys::BDS)
    {
        traceFormatedFloat(trace, eph.aodc, formatStr);
    } /* BDS: AODC */
    else
    {
        traceFormatedFloat(trace, 0, formatStr);
    } /* spare */
    trace << "\n";
}
...
void outputNavRinexCeph(Ceph& ceph, Trace& trace, const double rnxver)
{
    ...
    {
        traceFormatedFloat(trace, ceph.toes, formatStr);
    }
    ...
    {
        traceFormatedFloat(trace, static_cast<int>(ceph.orb), formatStr);
        traceFormatedFloat(trace, ceph.tops, formatStr);
        trace << "\n";
...
void outputNavRinexSTO(STO& sto, Trace& trace, const double rnxver)
{
    ...
    if (sto.Sat.prn > 0)
        tracepdeex(0, trace, "> STO %-3s %-4s", sto.Sat.id().c_str(), sto.type);
    else
        tracepdeex(0, trace, "> STO %-3c %-4s", sto.Sat.sysChar(), sto.type);
    trace << "\n";
    ...
    tracepdeex(
        0,
        trace,
        "    %04.0f %02.0f %02.0f %02.0f %02.0f %02.0f",
        ep[0],
        ep[1],
        ep[2],
        ep[3],
    ...
    if (sys != E_Sys::BDS)
    {
        ttm     = GTow(sto.ttm);
        week    = GWeek(sto.ttm);
        weekRef = GWeek(sto.tot);
    }
    else
    {
        ttm     = BTow(sto.ttm);
        week    = BWeek(sto.ttm);
        weekRef = BWeek(sto.tot);
    }

    trace << "    ";
    traceFormatedFloat(
        trace,
        ttm + (week - weekRef) * 604800.0,
        formatStr
    ); /* align ttm to tot week */
...
void outputNavRinexEOP(EOP& eop, Trace& trace, const double rnxver)
{
    ...
    if (eop.Sat.prn > 0)
        tracepdeex(0, trace, "> EOP %-3s %-4s", eop.Sat.id().c_str(), eop.type);
    else
        tracepdeex(0, trace, "> EOP %-3c %-4s", eop.Sat.sysChar(), eop.type);
    trace << "\n";
    ...
    if (sys != E_Sys::BDS)
    {
        ttm     = GTow(eop.ttm);
        week    = GWeek(eop.ttm);
        weekRef = GWeek(eop.teop);
    }
    else
    {
        ttm     = BTow(eop.ttm);
        week    = BWeek(eop.ttm);
        weekRef = BWeek(eop.teop);
    }
...
void outputNavRinexION(ION& ion, Trace& trace, const double rnxver)
{
    ...
    if (ion.Sat.prn > 0)
        tracepdeex(0, trace, "> ION %-3s %-4s", ion.Sat.id().c_str(), ion.type);
    else
        tracepdeex(0, trace, "> ION %-3c %-4s", ion.Sat.sysChar(), ion.type);
    trace << "\n";
    ...
    else if (type == E_NavMsgType::LNAV || type == E_NavMsgType::D1D2 || type == E_NavMsgType::CNVX)
    {
...
void outputNavRinexBody(RinexNavFileOutput& outFileData, Trace& rinexStream, const double rnxver)
{
    for (auto& [sys, output] : outFileData.sysMap)
    {
        ...
        if (rnxver >= 4)
        {
            ...
            auto* ion_ptr = seleph<ION>(std::cout, tsync, sys, type, nav);
            if (ion_ptr != nullptr)
                outputNavRinexION(*ion_ptr, rinexStream, rnxver);
            auto* eop_ptr = seleph<EOP>(std::cout, tsync, sys, type, nav);
            if (eop_ptr != nullptr)
                outputNavRinexEOP(*eop_ptr, rinexStream, rnxver);
        }
...
void rinexNavHeader(map<E_Sys, bool>& sysMap, Trace& rinexStream, const double rnxver)
{
    ...
    string sysDesc;
    if (sysMap.size() == 1)
        sysDesc = rinexSysDesc(sysMap.begin()->first);
    else
        sysDesc = rinexSysDesc(E_Sys::COMB);
    ...
    if (int(rnxver) == 3)
    {
        for (auto& [sys, output] : sysMap)
        {
            ...
            for (auto& [dummy, ion] : ionList)
            {
                switch (sys)
                {
                    ...
                    case E_Sys::BDS:
                        ion_arr1 = &ion.vals[0];
                        ion_str1 = enum_to_string(sys);
                        ion_str1 += 'A';
                        ion_arr2 = &ion.vals[4];
                        ion_str2 = enum_to_string(sys);
                        ion_str2 += 'B';
                        break;
                    case E_Sys::GAL:
                        ion_arr1 = &ion.vals[0];
                ...
                if (ion_arr1)
                    tracepdeex(
                        0,
                        rinexStream,
                        "%s %12.4E%12.4E%12.4E%12.4E%7s%-20s\n",
                        ion_str1.c_str(),
                        ion_arr1[0],
                        ion_arr1[1],
                        ion_arr1[2],
        ...
        for (auto& [sys, output] : sysMap)
        {
            ...
            {
                case E_Sys::GPS:
                    code1 = E_StoCode::GPUT;
                    break;
                case E_Sys::GLO:
                    code1 = E_StoCode::GLUT;
                    code2 = E_StoCode::GLGP;
                    break;
                case E_Sys::GAL:
                    code1 = E_StoCode::GAUT;
                    code2 = E_StoCode::GAGP;
                    break;
                case E_Sys::QZS:
                    code1 = E_StoCode::QZUT;
                    code2 = E_StoCode::QZGP;
                    break;
                case E_Sys::BDS:
                    code1 = E_StoCode::BDUT;
                    break;
            ...
            for (auto& code : {code1, code2})
            {
                ...
                for (auto& [dummy, sto] : stoList)
                {
                    ...
                    tracepdeex(
                        0,
                        rinexStream,
                        "%s %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n",
                        enum_to_string(code),
                        sto.A0,
                        sto.A1,
                        tow,
