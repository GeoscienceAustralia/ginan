...
)
{
    ...
    for (auto& clkEntry : clkEntryList)
    {
        string dataType;
        if (clkEntry.isRec)
            dataType = "AR";  // Result for receiver clock.
        else
            dataType = "AS";  // Result for satellite clock.

        int numData = 2;      // Number of data values is 2, clock and sigma.
        tracepdeex(
            0,
            clockFile,
            "%2s %-4s %4d%3d%3d%3d%3d%10.6f%3d   %19.12E %19.12E\n",
            dataType.c_str(),
            clkEntry.id.c_str(),
            (int)ep[0],
            (int)ep[1],
...
void getKalmanSatClks(ClockList& clkEntryList, map<E_Sys, bool>& outSys, KFState& kfState)
{
    ...
    {
        if (key.type != KF::SAT_CLOCK || outSys[key.Sat.sys] == false)
        {
            continue;
        }
...
)
{
    ...
    tracepdeex(
        0,
        clockFile,
        "%-20s%-20s%4d%02d%02d %02d%02d%02d %4s%s\n",
    ...
    if (sysMap[E_Sys::GPS])
        for (int prn = 1; prn <= NSATGPS; prn++)
        {
            k++;
            SatSys s(E_Sys::GPS, prn);
            tracepdeex(0, clockFile, "%3s ", s.id().c_str());
            if (k % 15 == 0)
                tracepdeex(0, clockFile, "%s\n", "PRN LIST");
        }
    if (sysMap[E_Sys::GLO])
        for (int prn = 1; prn <= NSATGLO; prn++)
        {
            k++;
            SatSys s(E_Sys::GLO, prn);
            tracepdeex(0, clockFile, "%3s ", s.id().c_str());
            if (k % 15 == 0)
                tracepdeex(0, clockFile, "%s\n", "PRN LIST");
        }
    if (sysMap[E_Sys::GAL])
        for (int prn = 1; prn <= NSATGAL; prn++)
        {
            k++;
            SatSys s(E_Sys::GAL, prn);
            tracepdeex(0, clockFile, "%3s ", s.id().c_str());
            if (k % 15 == 0)
                tracepdeex(0, clockFile, "%s\n", "PRN LIST");
        }
    if (sysMap[E_Sys::BDS])
        for (int prn = 1; prn <= NSATBDS; prn++)
        {
            k++;
            SatSys s(E_Sys::BDS, prn);
            tracepdeex(0, clockFile, "%3s ", s.id().c_str());
            if (k % 15 == 0)
                tracepdeex(0, clockFile, "%s\n", "PRN LIST");
        }
    if (sysMap[E_Sys::QZS])
        for (int prn = 1; prn <= NSATQZS; prn++)
        {
            k++;
            SatSys s(E_Sys::QZS, prn);
            tracepdeex(0, clockFile, "%3s ", s.id().c_str());
            if (k % 15 == 0)
                tracepdeex(0, clockFile, "%s\n", "PRN LIST");
        }
    /*finish the line*/ while (k % 15 != 0)
    {
        k++;
        tracepdeex(0, clockFile, "%3s ", "");
        if (k % 15 == 0)
            tracepdeex(0, clockFile, "%s\n", "PRN LIST");
    }
...
)
{
    ...
    switch (clkDataRecSrcs.front())
    {
        ...
        default:
            BOOST_LOG_TRIVIAL(error)
                << "Printing receiver clocks for " << enum_to_string(clkDataRecSrcs.front())
                << " not implemented.";
            return;
    }
...
getSysOutputFilenames(string filename, GTime logtime, bool replaceSys, string id)
{
    logtime = logtime.floorTime(acsConfig.rotate_period);

    boost::posix_time::ptime logptime =
        boost::posix_time::from_time_t((time_t)((PTime)logtime).bigTime);
    ...
    for (auto& [sys, output] : acsConfig.process_sys)
    {
        ...
        string sysChar;
        if (acsConfig.split_sys)
            sysChar = string(1, t_Sat.sysChar());
        else
            sysChar = "M";
