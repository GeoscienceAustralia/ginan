#pragma once
#define EIGEN_DENSEBASE_PLUGIN "3rdparty/EigenDenseBaseAddons.h"
...
{
    VectorEnu() { Vector3d::setZero(); }

    VectorEnu(const Vector3d& in) { Vector3d::operator=(in); }
    ...
    {
        return Vector3d(((Vector3d) * this) - ((Vector3d)rhs));
    }
    VectorEnu operator+(const VectorEnu& rhs)
    {
        return Vector3d(((Vector3d) * this) + ((Vector3d)rhs));
...
{
    VectorEcef() { Vector3d::setZero(); }

    VectorEcef(const Vector3d& in) { Vector3d::operator=(in); }
    ...
    VectorEcef operator-(const VectorEcef& rhs)
    {
        return Vector3d(((Vector3d) * this) - ((Vector3d)rhs));
    }
    VectorEcef operator+(const VectorEcef& rhs)
...
{
    VectorEci() { Vector3d::setZero(); }

    VectorEci(const Vector3d& in) { Vector3d::operator=(in); }
    ...
    VectorEci operator-(const VectorEci& rhs) const
    {
        return Vector3d(((Vector3d) * this) - ((Vector3d)rhs));
    }
    VectorEci operator+(const VectorEci& rhs) const
...
{
    VectorPos() { Vector3d::setZero(); }

    VectorPos(const Vector3d& in) { Vector3d::operator=(in); }
