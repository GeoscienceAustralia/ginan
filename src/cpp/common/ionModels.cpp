...
)
{
    ...
    switch (mapFn)
    {
        ...
        case E_IonoMapFn::MLM:
            break;              // same to SLM but need to call the function multiple times
        case E_IonoMapFn::MSLM:
            alpha = 0.9782;
            break;
        case E_IonoMapFn::KLOBUCHAR:
            return 1 + 16 * pow(0.53 - azel.el / PI, 3);
    }
 ...
 *          double *posp     O   pierce point position {lat,lon,r} (rad,m) in geocentric spherical
 *coordinate system return : slant factor notes  : see ref [2], only valid on the earth surface
...
int interpTec(const TEC& tec, int k, const VectorPos& posp, double& value, double& rms)
{
    ...
    if (tec.lons[2] > 0)
        dlon -= floor(dlon / 360) * 360;  /*  0<=dlon<360 */
    else
        dlon += floor(-dlon / 360) * 360; /* -360<dlon<=0 */
    ...
    if (d[0] > 0 && d[1] > 0 && d[2] > 0 && d[3] > 0)
    {
        ...
        tracepdeex(
            6,
            std::cout,
            "  gridpoints: %8.2f %8.2f %8.2f %8.2f -> %9.3f\n",
            d[0],
            d[1],
            d[2],
            d[3],
    ...
    else if (a <= 0.5 && b <= 0.5 && d[0] > 0)
    {
        value = d[0];
        rms   = r[0];
    }
    else if (a > 0.5 && b <= 0.5 && d[1] > 0)
    {
        value = d[1];
        rms   = r[1];
    }
    else if (a <= 0.5 && b > 0.5 && d[2] > 0)
    {
        value = d[2];
        rms   = r[2];
    }
    else if (a > 0.5 && b > 0.5 && d[3] > 0)
    {
        value = d[3];
        rms   = r[3];
    }
...
)
{
    ...
    if (azel.el < MIN_EL || pos.hgt() < MIN_HGT)
    {
        return true;
    }
    ...
    switch (mode)
    {
        case E_IonoMode::TOTAL_ELECTRON_CONTENT:
        {
            int pass = iontec(
                ...
                mapFn,
                layerHeight,
                E_IonoFrame::SUN_FIXED,
                dion,
                var
            );
            if (pass)
                var += VAR_IONEX;  // adding some extra errors to reflect modelling errors
            else
                var = VAR_IONO;
