 ...
 * 2.1 IERS Conventions (2010) Working Version 1.3.0 — Most Recent,
 * https://iers-conventions.obspm.fr/conventions_material.php 2.2 IERS Conventions (2010) Working
 * Version 1.3.0 — Chapter 7, https://iers-conventions.obspm.fr/content/chapter7/icc7.pdf
 ...
 * 6.  H.-G. Scherneck, Explanatory Supplement to the Section “Local Site Displacement due to Ocean
 * loading” of the IERS Conventions (1996), 1999
...
map<E_TidalComponent, int>
    blqSignMap =  ///< Map of signs of BLQ records (positive east, north and upwards)
    {
...
)
{
    ...
    while (fileStream)
    {
        ...
        {
            if (n < 3)
                tideLoading[wave].amplitude[index] =
                    v[i] * sign;                        // Positive east, north and upwards
            else
                tideLoading[wave].phase[index] = v[i];  // Phase signs won't change
            i++;
        }
...
)
{
    ...
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "BLQ file open error " << file;
    ...
    while (fileStream)
    {
        ...
        else if (line.substr(0, 2) == "$$" || line.size() < 2)
        {
            continue;
        }
        ...
        {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": Unspported file type" << "\n";
 ...
 */
bool readOceanPoleCoeff(string file)  ///< Ocean pole tide coefficient file
{
    ifstream fileStream(file);
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "Ocean pole tide coefficient file open error " << file;
    ...
    while (fileStream)
    {
        ...
        if (headerDone == false)
        {
            if (to_lower_copy(line.substr(0, 30)) == "number_longitude_grid_points =")
            {
                oceanPoleGrid.numLonGrid = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "first_longitude_degrees      =")
            {
                oceanPoleGrid.firstLonDeg = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "last_longitude_degrees       =")
            {
                oceanPoleGrid.lastLonDeg = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "longitude_step_degrees       =")
            {
                oceanPoleGrid.lonStepDeg = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "number_latitude_grid_points  =")
            {
                oceanPoleGrid.numLatGrid = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "first_latitude_degrees       =")
            {
                oceanPoleGrid.firstLatDeg = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "last_latitude_degrees        =")
            {
                oceanPoleGrid.lastLatDeg = str2num(buff, 30, 10);
                continue;
            }
            else if (to_lower_copy(line.substr(0, 30)) == "latitude_step_degrees        =")
            {
                oceanPoleGrid.latStepDeg = str2num(buff, 30, 10);
                continue;
            }
        }
...
/** Step 1 of solid Earth tide computation: Corrections to be computed in the time domain
 * (solar/lunar tides) See ref [1] 7.1.1
 */
 ...
 * See ref [1] 7.1.1
 * Note: permanent deformation not removed, i.e. the tidal model "in principle contains a
 * time-independent part so that the coordinates obtained by taking into account this model in the
 * analysis will be 'conventional tide free' values." See ref above for details
 */
...
)
{
    ...
    double a[7];
    a[1] =
        (218.3166560 + 481267.881342 * T - 0.00133000 * T2 + 0.0040 * cos((133 * T + 29) * D2R)) *
        D2R;                                                         // s
    a[2] = (280.4664490 + 36000.769822 * T + 0.00030360 * T2 + 0.0018 * cos((19 * T + 159) * D2R)) *
           D2R;                                                      // h
    ...
    a[6] = PI / 2;
    a[0] = (280.4606184 + 36000.7700536 * U + 0.00038793 * U2 - 0.0000000258 * U3 + sod / 240.0) *
               D2R -
           a[1];  // tao
    ...
    for (auto& [wave, disp] : otlDisplacement)
    {
        ...
        for (int i = 0; i < 7; i++)
            ang += fmod(a[i], PI2) * q[wave][i];
        for (int j = 0; j < n; j++)
        {
            f *= (1 + Q[wave][j] * cos(a[4]));
            u += Q[wave][j] * sin(q5[wave][j] * a[4]);
        }
        for (int k = 0; k < 3; k++)
            denu[k] += disp.amplitude[k] * f * cos(ang + u - disp.phase[k] * D2R);
    }
...
)
{
    ...
    VectorEnu denu;
    denu[0] = +9E-3 * cos(1 * theta) *
              (m1 * sinl - m2 * cosl);  // de = +Slambda (m), Slambda positive east
    denu[1] = +9E-3 * cos(2 * theta) *
              (m1 * cosl + m2 * sinl);  // dn = -Stheta  (m), Stheta positive south
    denu[2] = -33E-3 * sin(2 * theta) *
              (m1 * cosl + m2 * sinl);  // du = +Sr      (m), Sr positive upwards
...
)
{
    ...
    vector<double> dlat;
    vector<double> dlon;
    dlat.push_back(lat1 - latDeg);
    dlon.push_back(lon1 - lonDeg);
    dlat.push_back(lat2 - latDeg);
    dlon.push_back(lon2 - lonDeg);
    ...
    if (lon1 < oceanPoleGrid.firstLonDeg)
    {
        index[0] += oceanPoleGrid.numLonGrid;
        index[2] += oceanPoleGrid.numLonGrid;
    }
    if (lon2 > oceanPoleGrid.lastLonDeg)
    {
        index[1] -= oceanPoleGrid.numLonGrid;
        index[3] -= oceanPoleGrid.numLonGrid;
    }

    // nearest-neighbour extrapolation along latitude (outside of grid)
    if (lat1 < oceanPoleGrid.firstLatDeg)
    {
        index[0] = index[2];
        index[1] = index[3];
    }
    if (lat2 > oceanPoleGrid.lastLatDeg)
    {
        index[2] = index[0];
        index[3] = index[1];
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uRLon1.push_back(coeff1.uR);
        uRLon2.push_back(coeff2.uR);
        uILon1.push_back(coeff1.uI);
        uILon2.push_back(coeff2.uI);
    }

    // linear interpolation along longitude
    vector<VectorEnu> uRLat;
    vector<VectorEnu> uILat;
    uRLat.push_back(interpolate(dlon, uRLon1));
    uILat.push_back(interpolate(dlon, uILon1));
    uRLat.push_back(interpolate(dlon, uRLon2));
    uILat.push_back(interpolate(dlon, uILon2));
    ...
    double K = 5.3394043696E+03;  // K = 4 * pi * G * aE * rhow * Hp / (3 * ge), Hp = sqrt(8 * pi /
                                  // 15) * OmgE^2 * aE^4 / GM;
    double gamma2R = 0.6870;
    double gamma2I = 0.0036;

    VectorEnu denu =
        (uR * (m1 * gamma2R + m2 * gamma2I) + uI * (m2 * gamma2R - m1 * gamma2I)) * AS2R * K;
...
)
{
    ...
    if (recOpts.tideModels.solid)
    {
        solid = tideSolidEarthDehant(trace, time, rSun, rMoon, recPos);
    }
    if (recOpts.tideModels.otl)
    {
        VectorEnu denu = tideOceanLoadHardisp(trace, time, otlMap);
        otl            = (Vector3d)enu2ecef(pos, denu);
    }
    if (recOpts.tideModels.atl)
    {
        VectorEnu denu = tideAtmosLoad(trace, mjdUt1, atlMap);
        atl            = (Vector3d)enu2ecef(pos, denu);
    }
    if (recOpts.tideModels.spole)
    {
        VectorEnu denu = tideSolidPole(trace, mjdTT, pos, erpv);
        spole          = (Vector3d)enu2ecef(pos, denu);
    }
    if (recOpts.tideModels.opole)
    {
        VectorEnu denu = tideOceanPole(trace, mjdTT, pos, erpv);
        opole          = (Vector3d)enu2ecef(pos, denu);
    }

    tracepdeex(
        lv,
        trace,
        "\n%s   SOLID       %14.6f %14.6f %14.6f",
        timeStr.c_str(),
        solid[0],
        solid[1],
        solid[2]
    ...
    tracepdeex(
        lv,
        trace,
        "\n%s   ATMOSPHERIC %14.6f %14.6f %14.6f",
        timeStr.c_str(),
        atl[0],
        atl[1],
        atl[2]
    ...
    tracepdeex(
        lv,
        trace,
        "\n%s   SOLID POLE  %14.6f %14.6f %14.6f",
        timeStr.c_str(),
        spole[0],
        spole[1],
        spole[2]
    ...
    tracepdeex(
        lv,
        trace,
        "\n%s   OCEAN POLE  %14.6f %14.6f %14.6f",
        timeStr.c_str(),
        opole[0],
        opole[1],
        opole[2]
