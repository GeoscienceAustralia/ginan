...
struct Base64
{
    static string encode(string in) { return encode(in.c_str(), in.length()); }
...
struct URL
{
    ...
    URL(const string& url)
    {
        ...
        else
        {
            ...
            subUrl   = url.substr(protocolPos + 3);
        }
        ...
        {
            serverStr   = subUrl.substr(0, slashPos);
            pathNParams = subUrl.substr(slashPos + 1);
        }
        ...
        {
            user     = serverStr.substr(0, colonPos);
            passNUrl = serverStr.substr(colonPos + 1);
        }
        ...
        {
            pass    = passNUrl.substr(0, atPos);
            justUrl = passNUrl.substr(atPos + 1);
        }
        ...
        {
            host    = justUrl.substr(0, colonPos);
            portStr = justUrl.substr(colonPos + 1);
        }
        ...
        {
            pathNParams       = pathNParams.substr(questionPos + 1);
            colonPos          = pathNParams.find(":");
            string paramName  = pathNParams.substr(0, colonPos);
            pathNParams       = pathNParams.substr(colonPos + 1);
            questionPos       = pathNParams.find("?");
            string paramValue = pathNParams.substr(0, questionPos);
...
struct TcpSocket : NetworkStatistics, SerialStream
{
   ...
   public:
    ...
    virtual void connected() { startRead(true); }
