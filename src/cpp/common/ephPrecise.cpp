...
int readdcb(string file)
{
    ...
    while (std::getline(inputStream, line))
    {
        ...
        if (type.empty() || sscanf(buff, "%31s %31s", str1, str2) < 1)
            continue;
        ...
        if (Sat.sys == E_Sys::GPS)
        {
            ...
            {
                entry.cod1 = E_ObsCode::L2W;
                entry.cod2 = E_ObsCode::L2D;
            }
            else
                continue;
        }
        else if (Sat.sys == E_Sys::GLO)
        {
            ...
            {
                entry.cod1 = E_ObsCode::L2P;
                entry.cod2 = E_ObsCode::L2C;
            }
            else
                continue;
        }
        ...
        string id;
        if (!strcmp(str1, "G") || !strcmp(str1, "R"))
        {
            /* receiver dcb */
            entry.Sat  = Sat;
            entry.name = str2;
...
bool pephpos(Trace& trace, GTime time, SatSys Sat, Navigation& nav, Vector3d& rSat, double* vare)
{
    ...
    //     time.to_string(3).c_str(),Sat.id().c_str());
    ...
    if ((pephMap.size() < NMAX + 1) || (time < firstTime - MAXDTE) || (time > lastTime + MAXDTE))
    {
        tracepdeex(
            3,
            std::cout,
    ...
    if (vare)
    {
        ...
        if (t[0] > 0)
            std += EXTERR_EPH * SQR(t[0]) / 2;
        else if (t[NMAX] < 0)
            std += EXTERR_EPH * SQR(t[NMAX]) / 2;
...
)
{
    ...
    if (c[0] == INVALID_CLOCK_VALUE)
    {
        use0 = false;
    }
    if (c[1] == INVALID_CLOCK_VALUE)
    {
        use1 = false;
    }
    ...
    else if (dt > nav.pclkInterval)
    {
        use0 = false;
        use1 = false;
    }  // Exclude interpolation within data gaps

    if (use0 == false && use1 == false)
    {
    ...
    if (use0 && use1)
    {
        clk    = (c[1] * t[0] - c[0] * t[1]) / dt;
        clkVar = (SQR(point1.clkStd * t[0]) + SQR(point0.clkStd * t[1])) / SQR(dt);
    }
    else if (use0)
    {
        clk    = c[0];
        clkVar = SQR(point0.clkStd);
    }
    else if (use1)
    {
        clk    = c[1];
        clkVar = SQR(point1.clkStd);
    }
...
bool pephclk(Trace& trace, GTime time, string id, Navigation& nav, double& clk, double* varc)
{
    ...
    bool pass;
    pass = pclkMapClk(trace, time, id, nav, clk, varc, nav.pclkMap);
    if (pass)
        return true;
    pass = pclkMapClk(trace, time, id, nav, clk, varc, nav.pephMap);
    if (pass)
        return true;
...
)
{
    ...
    bool pass =
        pephclk(trace, time, Sat, nav, clk, &clkVar) && pephclk(trace, time + tt, Sat, nav, clk2);

    if (pass == false || clk == INVALID_CLOCK_VALUE)
    {
