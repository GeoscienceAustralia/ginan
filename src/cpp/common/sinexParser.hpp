...
struct SinexParser : Parser, ObsLister
{
    ...
    void parse(std::istream& inputStream)
    {
        ...
        while (inputStream)
        {
            ...
            if (line[0] == '+')
            {
                ...
                if (lineName == "+SOLUTION/ESTIMATE")
                {
                    parseFunction = &SinexParser::parseSinexEstimates;
                }
                else if (lineName == "+SOLUTION/DISCONTINUITY")
                {
                    parseFunction = &SinexParser::parseSinexDiscontinuities;
                }
                else if (lineName == "+SOLUTION/MATRIX_ESTIMATE")
                {
                    parseFunction = &SinexParser::parseSinexEstimateMatrix;
                }
                else
                {
                    // 				BOOST_LOG_TRIVIAL(error)
    ...
    string parserType() { return "SinexParser"; }
};
