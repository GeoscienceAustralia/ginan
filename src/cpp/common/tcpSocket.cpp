...
void TcpSocket::logChunkError()
{
    ...
    // 	message << ", ratio (error/total) : " << (double)numberErroredChunks
    // /(double)(numberErroredChunks+numberValidChunks);
...
void TcpSocket::startRead(bool chunked)
{
    ...
    if (chunked)
        function_ptr = &TcpSocket::readHandlerChunked;
    else
        function_ptr = &TcpSocket::readHandlerContent;
    ...
    if (url.protocol == "https")
    {
        boost::asio::async_read(
            *_sslsocket,
            ...
            boost::bind(function_ptr, this, bp::error)
        );
    }
    else
    {
...
void TcpSocket::readHandlerContent(const boost::system::error_code& err)
{
    if (err)
    {
...
void TcpSocket::readHandlerChunked(const boost::system::error_code& err)
{
    if (err)
    {
        ERROR_OUTPUT_RECONNECT_AND_RETURN;
    ...
    if (receivedTcpData.empty() == false)
        while (true)
        {
            ...
            vector<char> chunk(
                &receivedTcpData[startOfMessage],
                &receivedTcpData[endOfMessage] + 1
            );
...
void TcpSocket::reconnectTimerHandler(const boost::system::error_code& err)
{
    if (err)
    {
        ERROR_OUTPUT_RECONNECT_AND_RETURN;
...
void TcpSocket::delayedReconnect()
{
    ...
    // BOOST_LOG_TRIVIAL(debug) << " " << __FUNCTION__ << " " << url.sanitised() << " Started
    // Timer.\n";
...
void NtripResponder::requestResponseHandler(const boost::system::error_code& err)
{
    if (err)
    {
        ERROR_OUTPUT_RECONNECT_AND_RETURN;
...
void TcpSocket::writeRequestHandler(const boost::system::error_code& err)
{
    if (err)
    {
        ERROR_OUTPUT_RECONNECT_AND_RETURN;
    ...
    if (url.protocol == "https")
    {
        boost::asio::async_read_until(
            *_sslsocket,
            downloadBuf,
            readUntilString,
            boost::bind(&TcpSocket::requestResponseHandler, this, bp::error)
        );
    }
    else
    {
...
void TcpSocket::sslHandshakeHandler(const boost::system::error_code& err)
{
    if (err)
    {
        ERROR_OUTPUT_RECONNECT_AND_RETURN;
    ...
    boost::asio::async_write(
        *_sslsocket,
        request,
        boost::bind(&TcpSocket::writeRequestHandler, this, bp::error)
    );
}
...
)
{
    ...
    if (url.protocol == "https")
    {
        _sslsocket->async_handshake(
            ...
            boost::bind(&TcpSocket::sslHandshakeHandler, this, bp::error)
        );
...
void TcpSocket::connect()
{
    ...
    if (url.protocol == "https")
        socket_ptr = &_sslsocket->next_layer();
    else
        socket_ptr = _socket.get();
