...
Legendre::Legendre(int nmax) : nmax(nmax)
{
    init();
}
...
void Legendre::calculate(double x)
{
    if (abs(x) >= 1)
        throw std::runtime_error(
            "Legendre polynomial computation arguments should be between -1 and 1."
        );
...
)
{
    ...
    {
        *dAdPos_ptr +=
            GM / pow(observer.norm(), 5) *
            (3 * observer * observer.transpose() - Matrix3d::Identity() * observer.squaredNorm());
    }
 ...
 * @note This function does not contain the degree 0 acceleration need to be done via
 * "accelCentralForce"
 */
Vector3d accelSPH(
...
)
{
    ...
    Vector3d acc;
    acc.x() =
        dVr * r.x() / R + dVtheta * r.z() / SQR(R) * r.x() / Rxy - dVlambda * r.y() / SQR(Rxy);
    acc.y() =
        dVr * r.y() / R + dVtheta * r.z() / SQR(R) * r.y() / Rxy + dVlambda * r.x() / SQR(Rxy);
