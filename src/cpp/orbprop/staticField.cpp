 ...
 */
void StaticField::read(const string& filename, int degMax)
{
    if (filename.empty())
    {
        return;
    ...
    if (!infile)
    {
        BOOST_LOG_TRIVIAL(error) << "EGM file open error " << filename;
    ...
    {
        BOOST_LOG_TRIVIAL(warning) << "Maximum degree requested is higher than the model " << degMax
                                   << ">" << maxDegreeRead;
    }
    this->toTideFree();
    initialised = true;
...
void StaticField::toTideFree()
{
    ...
    if (isTideFree == false)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Earth gravity field converted to tide free system (compatible "
                                   "with orbit propagation)";
        isTideFree = true;
    }
}
