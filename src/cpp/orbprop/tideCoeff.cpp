...
void Tide::read(const string& filename, int degMax)
{
    ...
    if (!infile)
    {
        BOOST_LOG_TRIVIAL(error) << "Tide file open error " << filename;
...
TidalWave::TidalWave(string name, string darw, int degmax) : waveName(name)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << " Creating new wave " << waveName << " degmax "
                             << degmax;
 ...
 */
void Tide::setBeta(GTime time, double ut1_utc)
{
...
void Tide::getSPH(Array6d& dood, MatrixXd& Cnm, MatrixXd& Snm)
{
    for (auto& wave : tidalWaves)
    {
