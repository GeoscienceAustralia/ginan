...
FrameSwapper::FrameSwapper(GTime time, const ERPValues& erpv) : time0{time}, erpv{erpv}
{
    ...
    {
        if (time0 == cache.time0 && erpv == cache.erpv)
        {
            *this = cache;
            return;
        }
 ...
 */
VectorEcef pos2ecef(const VectorPos& pos)  ///< geodetic position {lat,lon,h} (rad,m)
{
...
{
    // Ref: RTCM c10403.3, equation (3.12-5), p188 (this rotation matrix performs RAC->ECEF, so
    // ECEF->RAC is simply the transpose of this)
    Vector3d ea = satVel.normalized();
    Vector3d rv = rSat.cross(satVel);
