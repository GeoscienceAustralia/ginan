    ...
    for (auto& [sys, nsat] : sysNsat)
    {
        ...
        for (int s = 1; s < 41; s++)
        {
            ...
            if (acsConfig.ssrOpts.cmpssr_cell_mask)
            {
                ...
                {
                    bool noCodes =
                        (codeBiasMap.find(Sat) == codeBiasMap.end() ||
                         codeBiasMap[Sat].obsCodeBiasMap.find(code) ==
                             codeBiasMap[Sat].obsCodeBiasMap.end());

                    bool noPhase =
                        (phaseBiasMap.find(Sat) == phaseBiasMap.end() ||
                         phaseBiasMap[Sat].obsCodeBiasMap.find(code) ==
                             phaseBiasMap[Sat].obsCodeBiasMap.end());
    ...
    for (auto& [ind, sat] : currentSatMap)
    {
        ...
        if (orbClkMap.find(sat) != orbClkMap.end())
        {
            ...
            if (iode == currentSatIodes[sat])
            {
                ...
                if (abs(dRad) > 16383 || abs(dRad) > 4095 || abs(dRad) > 4095)
                {
                    dRad = -16384;
                    dAlg = -4096;
                    dCrs = -4096;
...
/**
    ...
    warning: Requires both orbit and clocks use subtype 2 and 3 for orbit-only or clock-only
   messages
*/
    ...
    for (auto& [ind, sat] : currentSatMap)
    {
        ...
        if (orbClkMap.find(sat) != orbClkMap.end())
        {
            ...
            if (iode == currentSatIodes[sat])
            {
                ...
                if (abs(dRad) > 16383 || abs(dRad) > 4095 || abs(dRad) > 4095 || abs(dClk) > 16383)
                {
                    dRad = -16384;
                    dAlg = -4096;
                    dCrs = -4096;
...
vector<uint8_t>
encodecompactCOD(map<SatSys, SSRCodeBias>& codeBiasMap, int updateIntIndex, bool last)
{
    vector<uint8_t> buffer;
...
vector<uint8_t>
encodecompactPHS(map<SatSys, SSRPhasBias>& phaseBiasMap, int updateIntIndex, bool last)
{
    vector<uint8_t> buffer;
           ...
           )
{
    ...
    for (auto& [ind, sat] : regSat)
    {
        ...
        {
            case 3:
                C02_tmp = (int)round((stecData.poly[4] - stecPolyCommonMode[regId][4]) * 200);
                if (abs(C02_tmp) > 127)
                {
                    inRange = false;
                    break;
                }
                C20_tmp = (int)round((stecData.poly[5] - stecPolyCommonMode[regId][5]) * 200);
                if (abs(C20_tmp) > 127)
                {
                    inRange = false;
                    break;
            ...
            case 2:
                C11_tmp = (int)round((stecData.poly[3] - stecPolyCommonMode[regId][3]) * 50);
                if (abs(C11_tmp) > 511)
                {
                    inRange = false;
                    break;
            ...
            case 1:
                C01_tmp = (int)round((stecData.poly[1] - stecPolyCommonMode[regId][1]) * 50);
                if (abs(C01_tmp) > 2047)
                {
                    inRange = false;
                    break;
                }
                C10_tmp = (int)round((stecData.poly[2] - stecPolyCommonMode[regId][2]) * 50);
                if (abs(C10_tmp) > 2047)
                {
                    inRange = false;
                    break;
            ...
            case 0:
                C00_tmp = (int)round((stecData.poly[0] - stecPolyCommonMode[regId][0]) * 20);
                if (abs(C00_tmp) > 8191)
                {
                    inRange = false;
                    break;
...
)
{
    ...
    if (ssrAtmReg.ionoGrid && ssrAtmReg.tropGrid)
        return buffer;
...
)
{
    ...
    if (tropType > 0)
    {
        ...
        if (nBitTrpPoly > 0)
        {
            ...
            {
                case 2:
                    T11_tmp = (int)round(tropData.polyDry[3] * 1000);
                    if (abs(T11_tmp) > 63)
                    {
                        inRange = false;
                        break;
                ...
                case 1:
                    T01_tmp = (int)round(tropData.polyDry[1] * 500);
                    if (abs(T01_tmp) > 63)
                    {
                        inRange = false;
                        break;
                    }
                    T10_tmp = (int)round(tropData.polyDry[2] * 500);
                    if (abs(T10_tmp) > 63)
                    {
                        inRange = false;
                        break;
                ...
                case 0:
                    T00_tmp = (int)round(tropData.polyDry[0] * 250);
                    if (abs(T00_tmp) > 255)
                    {
                        inRange = false;
                        break;
    ...
    if (ionoType)
    {
        ...
        for (auto& [j, sat] : regSat)
        {
            ...
            if (nBitIonPoly > 0)
            {
                ...
                {
                    case 3:
                        C02_tmp = (int)round(stecData.poly[4] * 200);
                        if (abs(C02_tmp) > 127)
                        {
                            inRange = false;
                            break;
                        }
                        C20_tmp = (int)round(stecData.poly[5] * 200);
                        if (abs(C20_tmp) > 127)
                        {
                            inRange = false;
                            break;
                    ...
                    case 2:
                        C11_tmp = (int)round(stecData.poly[3] * 50);
                        if (abs(C11_tmp) > 511)
                        {
                            inRange = false;
                            break;
                    ...
                    case 1:
                        C01_tmp = (int)round(stecData.poly[1] * 50);
                        if (abs(C01_tmp) > 2047)
                        {
                            inRange = false;
                            break;
                        }
                        C10_tmp = (int)round(stecData.poly[2] * 50);
                        if (abs(C10_tmp) > 2047)
                        {
                            inRange = false;
                            break;
                    ...
                    case 0:
                        C00_tmp = (int)round(stecData.poly[0] * 20);
                        if (abs(C00_tmp) > 8191)
                        {
                            inRange = false;
                            break;
...
vector<uint8_t> encodeGridInfo(SSRAtm& ssrAtm)
{
    ...
    for (auto& [regID, regData] : ssrAtm.atmosRegionsMap)
    {
        ...
        {
            case 0:
                bitlen += 6 + 21 * regData.gridLatDeg.size();
                break;
            case 1:
                bitlen += 31;
                break;
        ...
        switch (regData.gridType)
        {
            case 0:
            {
                ...
                for (auto& [ind, lat] : regData.gridLatDeg)
                {
                    ...
                    tmp         = (int)round(dLat * 100 * D2R);
                    i           = setbitsInc(buf, i, 10, tmp);
                    thisLatDeg += 0.01 * tmp * R2D;

                    double dLon = regData.gridLonDeg[ind] - thisLonDeg;
                    tmp         = (int)round(dLon * 100 * D2R);
                    i           = setbitsInc(buf, i, 11, tmp);
            ...
            case 2:
            {
                if (regData.intLatDeg <= 0 || regData.intLonDeg <= 0)
                {
                    continue;
                }
                ...
                tmp          = (int)round(regData.gridLatDeg[0] * D2R * 16284 / PI);
                i            = setbitsInc(buf, i, 15, tmp);
                tmp          = (int)round(regData.gridLonDeg[0] * D2R * 16284 / PI);
                i            = setbitsInc(buf, i, 16, tmp);
                i            = setbituInc(buf, i, 6, nGridLat);
                i            = setbituInc(buf, i, 6, nGridLon);
                tmp          = (int)round(regData.intLatDeg * D2R * 100);
                i            = setbituInc(buf, i, 9, tmp);
                tmp          = (int)round(regData.intLonDeg * D2R * 100);
                i            = setbituInc(buf, i, 10, tmp);
                break;
            }
            default:
            {
