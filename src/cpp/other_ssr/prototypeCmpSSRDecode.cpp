...
double
decodeCmpSsrField(vector<unsigned char>& data, int& i, int bitLen, double scale, double offset)
{
...
double checkDisc(SatSys Sat, E_ObsCode code, double bias, int disc, int regionID = -1)
{
    ...
    if (compactSsrPhaseDisc.find(regionID) == compactSsrPhaseDisc.end())
        newBias = true;
...
{
    if (regID == -1 || acsConfig.ssrOpts.region_id == regID)
    ...
    if (ssrAtmUpdated)
    {
        ...
        for (auto& [regInd, regData] : compactSsrAtmStorage.atmosRegionsMap)
        {
            ...
            if (regData.regionDefIOD > 0 && navAtm.regionDefIOD != regData.regionDefIOD)
            {
            ...
            if (regData.tropData.find(tAtm) != regData.tropData.end())
            {
                ...
                if (regInd == acsConfig.ssrOpts.region_id)
                {
                    tracepdeex(
                        ...
                        std::cout,
                        "\n#CMP_SSR TRP     %s  %.4f\n    Poly:",
                        tAtm.to_string().c_str(),
                        regData.tropData[tAtm].sigma
                    );
                    for (auto& [ind, val] : regData.tropData[tAtm].polyDry)
                        tracepdeex(CMPSSRTRCLVL + 1, std::cout, " %.4f", val);
                    tracepdeex(CMPSSRTRCLVL + 1, std::cout, "\n    Grid:");
                    for (auto& [ind, val] : regData.tropData[tAtm].gridWet)
                        tracepdeex(CMPSSRTRCLVL + 1, std::cout, " %.4f", val);
                    tracepdeex(CMPSSRTRCLVL + 1, std::cout, "\n");
                }
            }
            ...
            for (auto& [Sat, stecMap] : regData.stecData)
                if (stecMap.find(tAtm) != stecMap.end())
                {
                    ...
                    if (regInd == acsConfig.ssrOpts.region_id)
                    {
                        ...
                        for (auto& [ind, val] : stecMap[tAtm].poly)
                            tracepdeex(CMPSSRTRCLVL + 1, std::cout, " %.4f", val);
                        tracepdeex(CMPSSRTRCLVL + 1, std::cout, "\n    Grid:");
                        for (auto& [ind, val] : stecMap[tAtm].grid)
                            tracepdeex(CMPSSRTRCLVL + 1, std::cout, " %.4f", val);
                        tracepdeex(CMPSSRTRCLVL + 1, std::cout, "\n");
                    }
                }
        ...
        switch (gridType)
        {
            case 0:
            {
                ...
                for (int grd = 0; grd < ngrid; grd++)
                {
                    ...
                    if (atmRegion.minLatDeg > (thisLatDeg))
                        atmRegion.minLatDeg = thisLatDeg;
                    if (atmRegion.maxLatDeg < (thisLatDeg))
                        atmRegion.maxLatDeg = thisLatDeg;
                    if (atmRegion.minLonDeg > (thisLonDeg))
                        atmRegion.minLonDeg = thisLonDeg;
                    if (atmRegion.maxLonDeg < (thisLonDeg))
                        atmRegion.maxLonDeg = thisLonDeg;

                    atmRegion.gridLatDeg[grd] =
                        thisLatDeg < -180 ? (thisLatDeg + 360)
                                          : (thisLatDeg > 180 ? (thisLatDeg - 360) : thisLatDeg);
                    atmRegion.gridLonDeg[grd] =
                        thisLonDeg < -180 ? (thisLonDeg + 360)
                                          : (thisLonDeg > 180 ? (thisLonDeg - 360) : thisLonDeg);
                }
            ...
            case 1:
            {
                ...
                for (int nlat = 0, grd = 0; nlat < ngridLat; nlat++)
                    ...
                    {
                        atmRegion.gridLatDeg[grd] =
                            atmRegion.gridLatDeg[0] - nlat * atmRegion.intLatDeg;
                        atmRegion.gridLonDeg[grd] =
                            atmRegion.gridLonDeg[0] + nlon * atmRegion.intLonDeg;
                        grd++;
                    }
                break;
            }
...
void processServiceData(vector<unsigned char>& data)
{
    ...
    switch (srvMessageType)
    {
        case 3:
            ...
            break;
        default:
            tracepdeex(4, std::cout, "Unsupported compact SSR service message\n");
    }
}
...
int decodeSSR_mask(vector<unsigned char>& data, GTime now)
{
    ...
    for (int s = 0, satind = 0, cellind = 0; s < nsys; s++)
    {
        ...
        if (getbituInc(data, i, 1) == 1)
        {
            if (nsat * ncod > 128)
            {
                tracepdeex(
                    2,
                    std::cout,
                    "Warning cell size for compact SSR execeds maximum size\n"
                );
                compactSSRIod = -1;
                return 0;
            }
        ...
        for (auto& [sind, Sat] : sysSatIndex)
            ...
            {
                if (cellField && getbituInc(data, i, 1) == 0)
                {
                    continue;
                }
...
int decodeSSR_orbit(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_ORB %s  iod: %2d  nsat: %2d           udi: %2d",
        ssrMeta.receivedTime.to_string().c_str(),
        compactSSRIod,
        compactSsrSatelliteIndex[-1].size(),
        ssrEph.udi
    );
...
int decodeSSR_clock(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_CLK %s  iod: %2d  nsat: %2d            udi: %2d",
        ssrMeta.receivedTime.to_string().c_str(),
        compactSSRIod,
        compactSsrSatelliteIndex[-1].size(),
        ssrClk.udi
...
int decodeSSR_combined(vector<unsigned char>& data, GTime now)
{
    ...
    if (regionID == -1)
        tracepdeex(
            CMPSSRTRCLVL,
            std::cout,
            "\n#CMPSSR_CMB %s %s %s global   , nsat:%d",
            ssrMeta.receivedTime.to_string().c_str(),
            orbitAvailable ? "orb" : "   ",
            clockAvailable ? "clk" : "   ",
            compactSsrSatelliteIndex[regionID].size()
        );
    else
        tracepdeex(
            CMPSSRTRCLVL,
            std::cout,
            "\n#CMPSSR_CMB %s %s %s region %2d, nsat:%d",
            ssrMeta.receivedTime.to_string().c_str(),
            orbitAvailable ? "orb" : "   ",
            clockAvailable ? "clk" : "   ",
            regionID,
...
int decodeSSR_code_bias(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_COD %s iod: %2d udi: %2d nsig:%d",
        ssrMeta.receivedTime.to_string().c_str(),
        compactSSRIod,
        ssrCodeBias.udi,
        compactSsrSignalIndex[-1].size()
...
int decodeSSR_phas_bias(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_PHS %s  iod: %2d  nsig:%d  udi: %2d",
        ssrMeta.receivedTime.to_string().c_str(),
        compactSSRIod,
        compactSsrSignalIndex[-1].size(),
        ssrPhasBias.udi
...
int decodeSSR_comb_bias(vector<unsigned char>& data, GTime now)
{
    ...
    if (regionID == -1)
        tracepdeex(
            CMPSSRTRCLVL,
            std::cout,
            "\n#CMPSSR_BIA %s %s %s global   , nsig:%d",
            ssrMeta.receivedTime.to_string().c_str(),
            code_Available ? "cod" : "   ",
            phaseAvailable ? "phs" : "   ",
            compactSsrSignalIndex[regionID].size()
        );
    else
        tracepdeex(
            CMPSSRTRCLVL,
            std::cout,
            "\n#CMPSSR_BIA %s %s %s region %2d, nsig:%d",
            ssrMeta.receivedTime.to_string().c_str(),
            code_Available ? "cod" : "   ",
            phaseAvailable ? "phs" : "   ",
            regionID,
...
int decodeSSR_URA(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_URA %s  iod: %2d  nsat: %2d  udi: %2d",
        ssrMeta.receivedTime.to_string().c_str(),
        compactSSRIod,
        compactSsrSatelliteIndex[-1].size(),
        ssrUra.udi
...
int decodeSSR_slant_TEC(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_TEC %s %d region %2d, nsat: %d",
        tAtm.to_string().c_str(),
        STECType,
        regionID,
        compactSsrIonoIndex[regionID].size()
    );
...
int decodeSSR_grid_ATM(vector<unsigned char>& data, GTime now)
{
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_GRD %s %d %d region %2d, nsig:%d",
        tAtm.to_string().c_str(),
        tropType,
        STECtype,
        regionID,
...
int decodeSSR_comp_ATM(vector<unsigned char>& data, GTime now)
{
    ...
    if (tropType > 0)
    {
        ...
        if (tropType & 1)
        {
            ...
            tracepdeex(
                ...
                std::cout,
                "  dry: %.3f",
                atmRegion.tropData[tAtm].polyDry[0]
            );
            ...
            if (dry_Type > 0)
            {
                ...
                tracepdeex(
                    ...
                    std::cout,
                    " %.3f %.3f",
                    atmRegion.tropData[tAtm].polyDry[1],
                    atmRegion.tropData[tAtm].polyDry[2]
                );
            }
            if (dry_Type > 1)
            {
                ...
                tracepdeex(
                    ...
                    std::cout,
                    " %.3f",
                    atmRegion.tropData[tAtm].polyDry[3]
                );
            }
        }
        if (tropType > 1)
        {
            ...
            for (int grd = 0; grd < numbGrid; grd++)
            {
                ...
                atmRegion.tropData[tAtm].gridWet[grd] =
                    decodeCmpSsrField(data, i, nw, 0.004, wet_offset);
                tracepdeex(
                    CMPSSRTRCLVL + 2,
                    std::cout,
                    "  %.3f",
    ...
    if (stecType == 0)
    {
        tracepdeex(
            CMPSSRTRCLVL,
            std::cout,
            "\n#CMPSSR_ATM %s %s     region %2d, nsig:%d",
            ssrMeta.receivedTime.to_string().c_str(),
            (tropType > 0) ? "trp" : "   ",
            regionID,
            compactSsrSignalIndex[regionID].size()
    ...
    for (auto& [indx, Sat] : compactSsrIonoIndex[regionID])
    {
        ...
        if (stecType & 1)
        {
            ...
            tracepdeex(
                ...
                std::cout,
                "  poly: %.3f",
                atmRegion.stecData[Sat][tAtm].poly[0]
            );
            ...
            if (polyType > 0)
            {
                ...
                tracepdeex(
                    ...
                    std::cout,
                    " %.3f %.3f",
                    atmRegion.stecData[Sat][tAtm].poly[1],
                    atmRegion.stecData[Sat][tAtm].poly[2]
                );
            }
            ...
            if (polyType > 1)
            {
                ...
                tracepdeex(
                    ...
                    std::cout,
                    " %.3f",
                    atmRegion.stecData[Sat][tAtm].poly[3]
                );
            }
            ...
            if (polyType == 3)
            {
                ...
                tracepdeex(
                    ...
                    std::cout,
                    " %.3f %.3f",
                    atmRegion.stecData[Sat][tAtm].poly[4],
                    atmRegion.stecData[Sat][tAtm].poly[5]
                );
            }
        }
    ...
    tracepdeex(
        CMPSSRTRCLVL,
        std::cout,
        "\n#CMPSSR_ATM %s %s ion region %2d, nsig:%d",
        ssrMeta.receivedTime.to_string().c_str(),
        (tropType > 0) ? "trp" : "   ",
        regionID,
        compactSsrSignalIndex[regionID].size()
...
int decodeSSR_service(vector<unsigned char>& data)
{
    ...
    if (lastServiceMessage >= 0 && lastServiceMessage != (messageCount - 1))
    {
        tracepdeex(
            2,
            std::cout,
            "Missing packets in compact SSR service messages %d -> %d\n",
            lastServiceMessage,
            messageCount
        );
    ...
    if (lastServiceMessage < 0 && messageCount != 0)
    {
        tracepdeex(
            2,
            std::cout,
...
{
    if (compactSsrLastTime != GTime::noTime() && now < compactSsrLastTime)
    {
        return E_ReturnType::WAIT;
    }
    ...
    switch (subtype)
    {
        case CompactSSRSubtype::MSK:
            return decodeSSR_mask(data, now);
        case CompactSSRSubtype::ORB:
            return decodeSSR_orbit(data, now);
        case CompactSSRSubtype::CLK:
            return decodeSSR_clock(data, now);
        case CompactSSRSubtype::COD:
            return decodeSSR_code_bias(data, now);
        case CompactSSRSubtype::PHS:
            return decodeSSR_phas_bias(data, now);
        case CompactSSRSubtype::BIA:
            return decodeSSR_comb_bias(data, now);
        case CompactSSRSubtype::URA:
            return decodeSSR_URA(data, now);
        case CompactSSRSubtype::TEC:
            return decodeSSR_slant_TEC(data, now);
        case CompactSSRSubtype::GRD:
            return decodeSSR_grid_ATM(data, now);
        case CompactSSRSubtype::SRV:
            return decodeSSR_service(data);
        case CompactSSRSubtype::CMB:
            return decodeSSR_combined(data, now);
        case CompactSSRSubtype::ATM:
            return decodeSSR_comp_ATM(data, now);

        default:
            tracepdeex(2, std::cout, "Submessage type %d not supported\n", stype);
