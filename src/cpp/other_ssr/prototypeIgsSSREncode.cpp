    ...
    switch (sys)
    {
        case E_Sys::GPS:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::GPS_OFFSET)
            );
        case E_Sys::GLO:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::GLO_OFFSET)
            );
        case E_Sys::GAL:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::GAL_OFFSET)
            );
        case E_Sys::QZS:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::QZS_OFFSET)
            );
        case E_Sys::BDS:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::BDS_OFFSET)
            );
        case E_Sys::SBS:
            return static_cast<unsigned short>(
                static_cast<int>(type) + static_cast<int>(IgsSSRSubtype::SBS_OFFSET)
            );
    }
...
vector<uint8_t> encodeIGS_ORB(map<SatSys, SSROut>& orbClkMap, E_Sys sys, bool last)
{
    ...
    for (auto& [Sat, ssrOut] : orbClkMap)
    {
        ...
        int d;
        d = (int)round(ssrEph.deph[0] / 0.1e-3);
        i = setbitsInc(buf, i, 22, d);
        d = (int)round(ssrEph.deph[1] / 0.4e-3);
        i = setbitsInc(buf, i, 20, d);
        d = (int)round(ssrEph.deph[2] / 0.4e-3);
        i = setbitsInc(buf, i, 20, d);
        d = (int)round(ssrEph.ddeph[0] / 0.001e-3);
        i = setbitsInc(buf, i, 21, d);
        d = (int)round(ssrEph.ddeph[1] / 0.004e-3);
        i = setbitsInc(buf, i, 19, d);
        d = (int)round(ssrEph.ddeph[2] / 0.004e-3);
        i = setbitsInc(buf, i, 19, d);
    }
...
vector<uint8_t> encodeIGS_CLK(map<SatSys, SSROut>& orbClkMap, E_Sys sys, bool last)
{
    ...
    for (auto& [Sat, ssrOut] : orbClkMap)
    {
        ...
        int d;
        d = (int)round(ssrClk.dclk[0] / 0.1e-3);
        i = setbitsInc(buf, i, 22, d);
        d = (int)round(ssrClk.dclk[1] / 0.001e-3);
        i = setbitsInc(buf, i, 21, d);
        d = (int)round(ssrClk.dclk[2] / 0.00002e-3);
        i = setbitsInc(buf, i, 27, d);
...
vector<uint8_t> encodeIGS_HRC(map<SatSys, SSROut>& orbClkMap, E_Sys sys, bool last)
{
    ...
    for (auto& [Sat, ssrOut] : orbClkMap)
    {
        ...
        if (last_clock.find(Sat) == last_clock.end())
            continue;

        if (last_clock[Sat].iod != ssrOut.ssrClk.iod)
            continue;

        double dt = (ssrClk.t0 - last_clock[Sat].t0).to_double();
        if (fabs(dt) > ssrUdi[ssrClk.ssrMeta.updateIntIndex])
            continue;

        hrClocks[Sat] = ssrClk.dclk[0] - (last_clock[Sat].dclk[0] + last_clock[Sat].dclk[1] * dt +
...
vector<uint8_t> encodeIGS_ATM(SSRAtm& ssrAtm, bool last)
{
    ...
    if (ssrGlobAtm.layers.size() > 4 || ssrGlobAtm.layers.size() != ssrGlobAtm.numberLayers)
    {
        return buffer;
    }
    int bitLen = 83 + ssrGlobAtm.numberLayers * 16;

    map<int, map<int, map<int, map<E_TrigType, double>>>>
        basisMaps;  // basis coefficients indexed by layer, degree, order and parity
