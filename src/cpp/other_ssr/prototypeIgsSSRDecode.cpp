    ...
    for (auto& [Sat, ssrBlock] : igsSSRStorage)
    {
        ...
        if (ssrBlock.ephUpdated)
            if (ssr.ssrEph_map.find(ssrBlock.ssrEph.t0) == ssr.ssrEph_map.end())
            {
                ...
                tracepdeex(
                    ...
                    Sat.id().c_str(),
                    ssrBlock.ssrEph.t0.to_string().c_str(),
        ...
        if (ssrBlock.clkUpdated)
            if (ssr.ssrClk_map.find(ssrBlock.ssrClk.t0) == ssr.ssrClk_map.end())
            {
                ...
                tracepdeex(
                    ...
                    Sat.id().c_str(),
                    ssrBlock.ssrClk.t0.to_string().c_str(),
                    ssrBlock.ssrClk.dclk[0],
                    ssrBlock.ssrClk.dclk[1],
                    ssrBlock.ssrClk.dclk[2],
                    ssrBlock.ssrClk.iod
                );
            }
        ...
        if (ssrBlock.codeUpdated)
        {
            ...
            tracepdeex(
                ...
                "\n#IGS_SSR CODBIA %s %s: ",
                Sat.id().c_str(),
                ssrBlock.ssrCodeBias.t0.to_string().c_str()
            );
        ...
        if (ssrBlock.phaseUpdated)
        {
            ...
            tracepdeex(
                ...
                "\n#IGS_SSR PHSBIA %s %s: ",
                Sat.id().c_str(),
                ssrBlock.ssrPhasBias.t0.to_string().c_str()
            );
...
int decodeigsSSR_header(vector<unsigned char>& data, GTime now, int opt, SSRHeader& ssrHead)
{
...
/* orbit */
void decodeigsSSR_type1(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
...
/* clock */
void decodeigsSSR_type2(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
...
/* combined */
void decodeigsSSR_type3(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
    int       i = decodeigsSSR_header(data, now, 1, ssrHead);
...
/* HR clocks */
void decodeigsSSR_type4(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
    int       i = decodeigsSSR_header(data, now, 0, ssrHead);
...
/* Code Bias */
void decodeigsSSR_type5(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
    int       i = decodeigsSSR_header(data, now, 0, ssrHead);
...
/* Phase Bias */
void decodeigsSSR_type6(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
...
/* URA message */
void decodeigsSSR_type7(vector<unsigned char>& data, GTime now, E_Sys sys)
{
    SSRHeader ssrHead;
    int       i = decodeigsSSR_header(data, now, 0, ssrHead);
...
/* Iono VTEC */
void decodeigsSSR_type8(vector<unsigned char>& data, GTime now)
{
    SSRHeader ssrHead;
...
E_ReturnType decodeigsSSR(vector<unsigned char>& data, GTime now)
{
    ...
    switch (group)
    {
        case IgsSSRSubtype::GROUP_ORB:
            decodeigsSSR_type1(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_CLK:
            decodeigsSSR_type2(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_CMB:
            decodeigsSSR_type3(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_HRC:
            decodeigsSSR_type4(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_COD:
            decodeigsSSR_type5(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_PHS:
            decodeigsSSR_type6(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_URA:
            decodeigsSSR_type7(data, now, sys);
            break;
        case IgsSSRSubtype::GROUP_ION:
            decodeigsSSR_type8(data, now);
            break;
    }
