...
// this will ust copy and paste the type for now, this is undefined later to remove the type names
// to pass in the parameters in the same order
#define COMMON_ARG(type) type

#define COMMON_PPP_ARGS                                                                         \
    COMMON_ARG(Trace&)                                                                          \
    trace, COMMON_ARG(LObs&) obs, COMMON_ARG(GTime&) time, COMMON_ARG(SatStat&) satStat,        \
        COMMON_ARG(ReceiverOptions&) recOpts, COMMON_ARG(SatelliteOptions&) satOpts,            \
        COMMON_ARG(SatNav&) satNav, COMMON_ARG(VectorEcef&) rRec, COMMON_ARG(VectorEcef&) rSat, \
        COMMON_ARG(double&) rRecSat, COMMON_ARG(VectorPos&) pos, COMMON_ARG(Receiver&) rec,     \
        COMMON_ARG(KFState&) kfState, COMMON_ARG(ERPValues&) erpv,                              \
        COMMON_ARG(FrameSwapper&) frameSwapper, COMMON_ARG(KFMeasEntry&) measEntry
...
inline void slrReflectorPCO(COMMON_PPP_ARGS)
{
    ...
    if (isSpherical(obs.satName))
        satReflectorCom = satComOffSphere(obs) * satStat.e * -1;
    else
        satReflectorCom = satComOffGnss(obs);

    double satReflectorDelta = satReflectorCom.dot(satStat.e);

    measEntry.componentsMap[E_Component::SAT_REFLECTOR_DELTA] = {
        satReflectorDelta * 2,
        "+ 2*satRefl",
        0
    };
...
inline void slrTrop(COMMON_PPP_ARGS)
{
    ...
    measEntry.componentsMap[E_Component::TROPOSPHERE] = {
        troposphere_m * 2,
        "+ 2*" + std::to_string(dTropDx.dryMap) + ".T",
        varTrop
    };
}
...
inline void slrRecRangeBias(COMMON_PPP_ARGS)
{
    ...
    measEntry.componentsMap[E_Component::REC_RANGE_BIAS] = {
        recRangeBias,
        "+ recRangeBias",
        recRangeBiasVar
    };
...
inline void slrRecTimeBias(COMMON_PPP_ARGS)
{
    ...
    measEntry.componentsMap[E_Component::REC_TIME_BIAS] = {
        recTimeBiasPartial * recTimeBias * 2,
        "+ 2*recTimeBiasPartial*recTimeBias",
        recTimeBiasVar
    };
}
...
)
{
    ...
    {
        tracepdeex(1, trace, "\n\nReceiver not ready for SLR. Obs=%d", rec.obsList.size());
        return;
    }
    ...
    for (auto& obs : only<LObs>(rec.obsList))
    {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (init.estimate)
                {
                    ...
                    delayedInits.push_back(
                        [kfKey, init, i, &measEntry](Vector3d satStat_e, Vector3d eSatInertial)
        ...
        if (initialStateFromConfig(satOpts.orbit).estimate)
        {
            ...
            {
                // dont obliterate obs.rSat below, we dont need it, but maintain consistency with
                // ppp_obs.cpp
                SatPos satPos0 = obs;
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                delayedInits.push_back(
                    [posKey, velKey, posInit, velInit, i, &measEntry, tgap](
                        Vector3d satStat_e,
