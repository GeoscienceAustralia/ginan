...
)
{
    for (auto& inputName : inputNames)
    {
        ...
        {
            mountpoint = inputName.substr(lastSlashPos + 1);
        };
        ...
        if (recOpts.exclude || recOpts.kill)
        {
            continue;
        }
        ...
        {
            protocol     = "file";
            subInputName = inputName;
        }
        else
        {
            protocol     = inputName.substr(0, protocolPos);
            subInputName = inputName.substr(protocolPos + 3);
        }

        std::unique_ptr<Stream> stream_ptr;
        std::unique_ptr<Parser> parser_ptr;

        if (protocol == "file" || protocol == "serial")
        {
        ...
        if (protocol == "file")
        {
            stream_ptr = make_unique<FileStream>(subInputName);
        }
        else if (protocol == "serial")
        {
            stream_ptr = make_unique<SerialStream>(subInputName);
        }
        else if (protocol == "http")
        {
            stream_ptr = make_unique<NtripStream>(inputName);
        }
        else if (protocol == "https")
        {
            stream_ptr = make_unique<NtripStream>(inputName);
        }
        else if (protocol == "ntrip")
        {
            stream_ptr = make_unique<NtripStream>(inputName);
        }
        else if (protocol == "sisnet")
        {
            stream_ptr = make_unique<SisnetStream>(inputName);
        }
        else
        {
        ...
        {
            parser_ptr                                             = make_unique<RinexParser>();
            static_cast<RinexParser*>(parser_ptr.get())->rnxRec.id = id;
        }
        else if (inputFormat == "UBX")
        {
        ...
        else if (inputFormat == "CUSTOM")
        {
            ...
            static_cast<CustomParser*>(parser_ptr.get())->recId = id;
        }
        else if (inputFormat == "RTCM")
        {
            parser_ptr                                                 = make_unique<RtcmParser>();
            static_cast<RtcmParser*>(parser_ptr.get())->rtcmMountpoint = mountpoint;
            if (id == "QZSL6")
            {
                static_cast<RtcmParser*>(parser_ptr.get())->qzssL6 = true;
            }
        }
        else if (inputFormat == "SP3")
        {
            parser_ptr = make_unique<Sp3Parser>();
        }
        else if (inputFormat == "SINEX")
        {
            parser_ptr = make_unique<SinexParser>();
        }
        else if (inputFormat == "SLR")
        {
            parser_ptr = make_unique<SlrParser>();
        }
        else if (inputFormat == "DS2DC")
        {
            parser_ptr = make_unique<DS2DCParser>();
        }
        else
        {
...
void reloadInputFiles()
{
    ...
    for (auto& atxfile : acsConfig.atx_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading ATX file " << atxfile;
    ...
    for (auto& sp3file : acsConfig.sp3_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading SP3 file " << sp3file;
    ...
    for (auto& obxfile : acsConfig.obx_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading OBX file " << obxfile;
    ...
    for (auto& navfile : acsConfig.nav_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading NAV file " << navfile;

        auto rinexStream =
            make_unique<StreamParser>(make_unique<FileStream>(navfile), make_unique<RinexParser>());
    ...
    for (auto& erpfile : acsConfig.erp_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading ERP file " << erpfile;
    ...
    for (auto& clkfile : acsConfig.clk_files)
    {
        ...
        /* CLK file - RINEX 3 */
        BOOST_LOG_TRIVIAL(info) << "Loading CLK file " << clkfile;

        auto rinexStream =
            make_unique<StreamParser>(make_unique<FileStream>(clkfile), make_unique<RinexParser>());
    ...
    for (auto& dcbfile : acsConfig.dcb_files)
    {
        ...
        /* DCB file */
        BOOST_LOG_TRIVIAL(info) << "Loading DCB file " << dcbfile;
    ...
    for (auto& bsxfile : acsConfig.bsx_files)
    {
        ...
        /* BSX file*/
        BOOST_LOG_TRIVIAL(info) << "Loading BSX file " << bsxfile;
    ...
    for (auto& ionfile : acsConfig.ion_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading ION file " << ionfile;
    ...
    for (auto& igrffile : acsConfig.igrf_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading IGRF file " << igrffile;
    ...
    for (auto& snxfile : acsConfig.snx_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading SNX file " << snxfile;
    ...
    for (auto& pseudo_filter_file : acsConfig.pseudo_filter_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Pseudo filter file " << pseudo_filter_file;
    ...
    for (auto& orographyfile : acsConfig.orography_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading ORO from " << orographyfile;
    ...
    for (auto& vmffile : acsConfig.vmf_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading VMF file " << vmffile;
    ...
    for (auto& gpt2gridfile : acsConfig.gpt2grid_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading GPT file " << gpt2gridfile;
    ...
    for (auto& optfile : acsConfig.ocean_pole_tide_loading_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Ocean Pole Tide file " << optfile;
    ...
    for (auto& otlfile : acsConfig.ocean_tide_loading_blq_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Ocean Tide Loading BLQ file " << otlfile;
    ...
    for (auto& atlfile : acsConfig.atmos_tide_loading_blq_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Atmos Tide Loading BLQ file " << atlfile;
    ...
    for (auto& sidfile : acsConfig.sid_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Sat ID file " << sidfile;
    ...
    for (auto& crdfile : acsConfig.crd_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading CRD file " << crdfile;
    ...
    for (auto& comfile : acsConfig.com_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading CoM file " << comfile;
    ...
    for (auto& egmfile : acsConfig.egm_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading EGM file " << egmfile;
    ...
    for (auto& swfile : acsConfig.space_weather_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading SW file " << swfile;
    ...
    for (auto& tidefile : acsConfig.ocean_tide_potential_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading Tide file " << tidefile;
    ...
    for (auto& jplfile : acsConfig.planetary_ephemeris_files)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "Loading planetary ephemeris file " << jplfile;

        nav.jplEph_ptr = (struct jpl_eph_data*)jpl_init_ephemeris(
            jplfile.c_str(),
            nullptr,
            nullptr
        );  // a Pointer to The jpl_eph_data Structure
    ...
    for (auto& [id, slrinputs] : slrObsFiles)
    {
        addReceiverData(id, slrinputs, "SLR", "OBS");
    }
    ...
    {
        addReceiverData(id, custominputs, "CUSTOM", "OBS");
    }
    for (auto& [id, rnxinputs] : acsConfig.rnx_inputs)
    {
        addReceiverData(id, rnxinputs, "RINEX", "OBS");
    }
    for (auto& [id, rtcminputs] : acsConfig.obs_rtcm_inputs)
    {
        addReceiverData(id, rtcminputs, "RTCM", "OBS");
    }
    for (auto& [id, pseudosp3inputs] : acsConfig.pseudo_sp3_inputs)
    {
        addReceiverData(id, pseudosp3inputs, "SP3", "PSEUDO");
    }
    for (auto& [id, pseudosnxinputs] : acsConfig.pseudo_snx_inputs)
    {
        addReceiverData(id, pseudosnxinputs, "SINEX", "PSEUDO");
    }
    {
