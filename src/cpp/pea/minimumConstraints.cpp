...
)
{
    ...
    tracepdeex(
        0,
        trace,
        "#\t%4s\t%9s\t%9s\t%12s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\t%s\n",
        "Sat",
        "Lat",
        "Lon",
        "Height",
    ...
    for (auto& [key, index] : kfStateStations.kfIndexMap)
    {
        ...
        tracepdeex(
            0,
            trace,
            "@\t%4s\t%9.4f\t%9.4f\t%12.3f\t%10.3f\t%10.3f\t%10.3f\t%10.3f\t%10.3f\t%10.3f\t%s\n",
            key.Sat.id().c_str(),
            pos.latDeg(),
            pos.lonDeg(),
            pos.hgt(),
...
{
    // Reference: Estimating regional deformation from a combination of space and terrestrial
    // geodetic data - Appendix E Perform LSQ/Kalman filter to determine transformation state

    if (acsConfig.output_mincon)
    {
        std::cout << "\n"
                  << "Writing backup point for minimum constraints to "
                  << acsConfig.mincon_filename;
    ...
    if (kfStateTrans.id != "MINIMUM")
    {
        ...
        kfStateTrans.outputMongoMeasurements =
    ...
    for (auto& [key, index] : kfStateStations.kfIndexMap)
    {
        ...
        if (key.type == KF::ORBIT &&
            (key.num >= 3 || acsConfig.minconOpts.constrain_orbits == false))
        {
            zeroAndPush(index, R, kfStateTrans, measList);
            continue;
        }
        ...
        if (aprioriPosVar(0, 0) <= 0 && acsConfig.minconOpts.transform_unweighted == false)
        {
            zeroAndPush(index, R, kfStateTrans, measList);
            continue;
        }

        if (aprioriPosVar(0, 0) <= 0 || aprioriPos.isZero())
        {
            R.row(index).setZero();
            R.col(index).setZero();
        ...
        if (acsConfig.minconOpts.full_vcv == false && used)
        {
            R.block(index, index, 3, 3) = noise;
        }

        auto addRate = [&](const InitialState& rateInit, const KFKey& key)
        {
            if (rateInit.estimate == false)
                return;

            KFKey rateKey = key;
            rateKey.type  = static_cast<KF>(
                static_cast<int>(rateKey.type) +
                (static_cast<int>(KF::XFORM_XLATE_RATE) - static_cast<int>(KF::XFORM_XLATE))
            );
            rateKey.comment += "/DAY";
        ...
        for (short xyz = 0; xyz < 3; xyz++)
        {
            ...
            {
                {
                    KFKey key{KF::XFORM_XLATE, {}, "", 0, "M"};
                    meas.addDsgnEntry(key, dRdX(xyz), xlateInit);
                    addRate(xlateRateInit, key);
                }
                {
                    KFKey key{KF::XFORM_XLATE, {}, "", 1, "M"};
                    meas.addDsgnEntry(key, dRdY(xyz), xlateInit);
                    addRate(xlateRateInit, key);
                }
                {
                    KFKey key{KF::XFORM_XLATE, {}, "", 2, "M"};
                    meas.addDsgnEntry(key, dRdZ(xyz), xlateInit);
                    addRate(xlateRateInit, key);
                }
            }
            ...
            {
                {
                    KFKey key{KF::XFORM_RTATE, {}, "", 0, "MAS"};
                    meas.addDsgnEntry(key, dRdThetaX(xyz) * MAS2R, rtateInit);
                    addRate(rtateRateInit, key);
                }
                {
                    KFKey key{KF::XFORM_RTATE, {}, "", 1, "MAS"};
                    meas.addDsgnEntry(key, dRdThetaY(xyz) * MAS2R, rtateInit);
                    addRate(rtateRateInit, key);
                }
                {
                    KFKey key{KF::XFORM_RTATE, {}, "", 2, "MAS"};
                    meas.addDsgnEntry(key, dRdThetaZ(xyz) * MAS2R, rtateInit);
                    addRate(rtateRateInit, key);
                }
            }
            ...
            {
                {
                    KFKey key{KF::XFORM_SCALE, {}, "", 0, "PPB"};
                    meas.addDsgnEntry(key, aprioriPos(xyz) * 1e-9, scaleInit);
                    addRate(scaleRateInit, key);
                }
            }
            ...
            {
                {
                    KFKey key{KF::XFORM_DELAY, {}, "", 0, "S"};
                    meas.addDsgnEntry(key, dRdT(xyz), delayInit);
                    addRate(delayRateInit, key);
                }
            }
            ...
            if (used)
            {
                ...
                meas.metaDataMap["otherIndex"] =
                    (void*)(measList.size() - 1);  // need an index into the big measurement matrix
                                                   // for deweighting to get applied in 2 places
    ...
    kfStateTrans = kfStateTrans.getSubState(
        {KF::ONE, KF::XFORM_RTATE, KF::XFORM_XLATE, KF::XFORM_DELAY, KF::XFORM_SCALE},
        &combinedMeas
    );
    ...
    switch (acsConfig.minconOpts.application_mode)
    {
        case E_Mincon::PSEUDO_OBS:
        {
            ...
            MatrixXd Omega =
                T * kfStateTrans.P * T.transpose();  // will be pseudoobservations variances
            ...
            // do some algebra to compute the required v values such that a subset of kalman state
            // adjustments are the desired dx calculated previously ie, find v
    ...
    // standard kalman filter things again, but do manually since already have K calculated
    {
        ...
        if (kfState.rts_basename.empty() == false)
        {
            spitFilterToFile(
                ...
                kfState.rts_basename + FORWARD_SUFFIX,
    ...
    {
        if (type == KF::REC_POS && hasStations == false)
            continue;
        if (type == KF::ORBIT && hasSatellites == false)
            continue;
        ...
        for (auto before : {true, false})
            for (auto& [key, index] : kfStateStations.kfIndexMap)
            {
                ...
                {
                    if (before)
                        filterPos(i) = oldStateStations.x(index + i);
                    else
                        filterPos(i) = kfStateStations.x(index + i);
                }
                ...
                if (type == KF::ORBIT)
                    ...
                    {
                        if (before)
                            filterVel(i) = oldStateStations.x(index + i + 3);
                        else
                            filterVel(i) = kfStateStations.x(index + i + 3);
                    }
                ...
                {
                    E = ecef2rac(filterPos, filterVel);
                }
                ...
                for (auto& rmsStatisticsMap_ptr : {minconStatistics_ptr0, minconStatistics_ptr1})
                    for (auto& entry : {aggregatedUsed, aggregatedAll, str})
                    {
                        ...
                        if (entry == aggregatedUsed && usedMap[index] == false)
                        {
                            continue;
                        }
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            if (i < 3)
                                rmsStatisticsMap[entry][component][before].sum +=
                                    SQR(frameResidual(i));
                            else
                                rmsStatisticsMap[entry][component][before].sum +=
                                    frameResidual.squaredNorm();
    ...
    if (commentSinex)
    {
        for (auto& [key, index] : kfStateStations.kfIndexMap)
        {
            if (key.num == 0 && key.type == KF::REC_POS)
            {
                sinexAddComment(
                    (string) " Minimum Constraints Stations: " + key.str +
                    (usedMap[index] ? "   used" : " unused")
                );
            }
        }
    ...
    for (auto& [str, rmsStatistics] : minconStatistics)
        for (auto& [component, statistics] : rmsStatistics)
            for (auto before : {true, false})
            {
                ...
                if (before)
                    tracepdeex(lvl, trace, "& %-10s - from: %12.6f ", str.c_str(), val);
                else
                    tracepdeex(
                        lvl,
                        trace,
                        "to: %12.6f in %2s over %5d entries\n",
                        val,
                        component.c_str(),
                        entry.count
                    );
