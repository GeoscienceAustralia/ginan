...
void createTracefiles(ReceiverMap& receiverMap, Network& pppNet, Network& ionNet)
{
    ...
    {
        if (rts && (acsConfig.process_rts == false || acsConfig.pppOpts.rts_lag == 0))
        {
            continue;
        }
        ...
        if (rts)
        {
            ...
            {
                bool newTraceFile = createNewTraceFile(
                    pppNet.id,
                    "Network",
                    not_a_date_time,
                    acsConfig.pppOpts.rts_filename,
                    pppNet.kfState.rts_basename
                );
            ...
            if (acsConfig.output_satellite_trace)
                if (rts == false)
                {
                    newTraceFile |= createNewTraceFile(
                        ...
                        "Sats",
                        logptime,
                        acsConfig.satellite_trace_filename,
                        satNav.traceFilename,
                        true,
        ...
        for (auto& [id, rec] : receiverMap)
        {
            if (acsConfig.output_receiver_trace)
                if (rts == false)
                {
                    newTraceFile |= createNewTraceFile(
                        ...
                        rec.source,
                        logptime,
                        acsConfig.receiver_trace_filename,
                        rec.traceFilename,
                        true,
                        acsConfig.output_config
                    );
            ...
            if (acsConfig.output_json_trace)
                if (rts == false)
                {
                    newTraceFile |= createNewTraceFile(
                        ...
                        rec.source,
                        logptime,
                        acsConfig.receiver_json_filename,
                        rec.jsonTraceFilename
                    );
                }
            ...
            if (acsConfig.output_cost)
            {
                newTraceFile |= createNewTraceFile(
                    ...
                    rec.source,
                    logptime,
                    insertSuffix(acsConfig.cost_filename, suff),
                    pppNet.kfState.metaDataMap[COST_FILENAME_STR + id + metaSuff]
                );
            }
            ...
            if (acsConfig.output_gpx)
            {
                newTraceFile |= createNewTraceFile(
                    ...
                    rec.source,
                    logptime,
                    insertSuffix(acsConfig.gpx_filename, suff),
                    pppNet.kfState.metaDataMap[GPX_FILENAME_STR + id + metaSuff]
                );
            }
        ...
        if (acsConfig.output_network_trace)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.network_trace_filename, suff),
                pppNet.kfState.metaDataMap[TRACE_FILENAME_STR + metaSuff],
                true,
                acsConfig.output_config
            );
        ...
        if (acsConfig.output_ionosphere_trace)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.ionosphere_trace_filename, suff),
                ionNet.kfState.metaDataMap[TRACE_FILENAME_STR + metaSuff],
                true,
                acsConfig.output_config
            );
        ...
        if (acsConfig.output_ionex)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.ionex_filename, suff),
                pppNet.kfState.metaDataMap[IONEX_FILENAME_STR + metaSuff]
            );
        }
        ...
        if (acsConfig.output_trop_sinex)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.trop_sinex_filename, suff),
                pppNet.kfState.metaDataMap[TROP_FILENAME_STR + metaSuff]
            );
        }
        ...
        if (acsConfig.output_bias_sinex)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.bias_sinex_filename, suff),
                pppNet.kfState.metaDataMap[BSX_FILENAME_STR + metaSuff]
            );
            newTraceFile |= createNewTraceFile(
                pppNet.id,
                "Network",
                logptime,
                insertSuffix(acsConfig.bias_sinex_filename, suff),
                ionNet.kfState.metaDataMap[BSX_FILENAME_STR + metaSuff]
            );
        }
        ...
        if (acsConfig.output_erp)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                insertSuffix(acsConfig.erp_filename, suff),
                pppNet.kfState.metaDataMap[ERP_FILENAME_STR + metaSuff]
            );
        }
        ...
        if (acsConfig.output_clocks)
        {
            ...
            for (auto& [filename, dummy] : filenameMap)
            {
                newTraceFile |= createNewTraceFile(
                    ...
                    "Network",
                    logptime,
                    insertSuffix(filename, suff),
                    fileNames[filename + metaSuff]
                );
            }

            pppNet.kfState.metaDataMap[CLK_FILENAME_STR + metaSuff] =
                insertSuffix(singleFilenameMap.begin()->first, suff);
        }
        ...
        if (acsConfig.output_sp3)
        {
            ...
            pppNet.kfState.metaDataMap[SP3_FILENAME_STR + metaSuff] =
                insertSuffix(singleFilenameMap.begin()->first, suff);
        }
        ...
        if (acsConfig.output_orbex)
        {
            ...
            pppNet.kfState.metaDataMap[ORBEX_FILENAME_STR + metaSuff] =
                insertSuffix(singleFilenameMap.begin()->first, suff);
        }
        ...
        if (acsConfig.output_sbas_ems)
        {
            newTraceFile |= createNewTraceFile(
                ...
                "Network",
                logptime,
                acsConfig.ems_filename,
                pppNet.kfState.metaDataMap[EMS_FILENAME_STR]
            );
        }
        ...
        {
            spitFilterToFile(
                pppNet.kfState.metaDataMap,
                E_SerialObject::METADATA,
                pppNet.kfState.rts_basename + FORWARD_SUFFIX,
    ...
    if (acsConfig.output_ntrip_log)
    {
        for (auto& [id, stream_ptr] : ntripBroadcaster.ntripUploadStreams)
        {
            ...
            createNewTraceFile(
                ...
                "NTRIP",
                logptime,
                acsConfig.ntrip_log_filename,
                stream.networkTraceFilename
            );
        }
        ...
        for (auto& [id, streamParser_ptr] : streamParserMultimap)
            try
            {
                ...
                createNewTraceFile(
                    ...
                    logptime,
                    acsConfig.ntrip_log_filename,
                    ntripStream.networkTraceFilename
                );
            }
        ...
        try
        {
            ...
            for (auto nav : {false, true})
            {
                    ...
                    (acsConfig.record_rtcm_obs && isNav == false && nav == false))
                {
                    ...
                    if (nav)
                        filename = acsConfig.rtcm_nav_filename;
                    else
        ...
        try
        {
            ...
            if (acsConfig.record_raw_custom)
            {
                createNewTraceFile(
                    ...
                    streamParser_ptr->stream.sourceString,
                    logptime,
                    acsConfig.raw_custom_filename,
                    customParser.raw_custom_filename
                );
            }
        }
        catch (std::bad_cast& e)
...
void outputPredictedStates(Trace& trace, KFState& kfState)
{
    ...
    for (auto& duo : {forward, reverse})
    {
        ...
        for (GTime time = startTime; sign * (time - stopTime).to_double() <= 0; time += timeDelta)
        {
            ...
            if (orbits.empty() == false && (doOrbits || doAll))
            {
                ...
                integrateOrbits(
                    ...
                    orbits,
                    tgap,
                    acsConfig.propagationOptions.integrator_time_step
                );
                ...
                for (int o = 0; o < orbits.size(); o++)
                {
                    ...
                    for (auto& [key, i] : orbit.subState_ptr->kfIndexMap)
                    {
                        ...
                        if (key.num < 3)
                            propState.x(index) = orbit.pos(i);
                        else
                            propState.x(index) = orbit.vel(i - 3);
...
void configureUploadingStreams()
{
    ...
    for (auto it = ntripBroadcaster.ntripUploadStreams.begin();
         it != ntripBroadcaster.ntripUploadStreams.end();)
    {
        if (acsConfig.netOpts.uploadingStreamData.find(it->first) ==
            acsConfig.netOpts.uploadingStreamData.end())
        {
            auto& [label, outStream_ptr] = *it;
            auto& outStream              = *outStream_ptr;
    ...
    if (acsConfig.process_ppp == false && acsConfig.process_spp == false &&
        acsConfig.slrOpts.process_slr == false && acsConfig.process_preprocessor == false &&
...
)
{
    ...
    // check whether we can write to the main state or need to make a copy (remember it will store
    // in rts too)
    bool hold = false;

    if (inRts == false && acsConfig.ambrOpts.fix_and_hold)
    {
        hold = true;
    }
    ...
    if (acsConfig.process_ppp)
    {
        ...
        if (acsConfig.process_minimum_constraints && acsConfig.minconOpts.once_per_epoch)
        {
            ...
            // erp values have probably changed due to mincon, update the nav vars before all the
            // outputs will have to revert this below because mincon is supposed to be temporary in
            // once_per_epoch mode
        ...
        if (inRts && acsConfig.ambrOpts.fix_and_hold)
        {
        ...
        if (arPossible && acsConfig.ambrOpts.mode && acsConfig.ambrOpts.once_per_epoch)
        {
            ...
            {
                arState_ptr = &kfState;
                BOOST_LOG_TRIVIAL(info) << "Performing AR with fix and hold";
            }
            else
            {
                arState_ptr = &augmentedKF;
            }
            ...
            // erp values have probably changed due to AR, update the nav vars before all the
            // outputs will have to revert this below because AR is supposed to be temporary in
            // once_per_epoch mode
        ...
        for (auto& [id, rec] : receiverMap)
        {
            ...
            {
                outputCost(
                    kfState.metaDataMap[COST_FILENAME_STR + id + META_SUFFIX],
                    augmentedKF,
                    rec
                );
            }
            ...
            {
                writeGPX(
                    kfState.metaDataMap[GPX_FILENAME_STR + id + META_SUFFIX],
                    augmentedKF,
                    rec
                );
            }
            ...
            {
                writePOS(
                    kfState.metaDataMap[POS_FILENAME_STR + id + META_SUFFIX],
                    augmentedKF,
                    rec
                );
            }
    ...
    if (1)
    {
        ...
        {
            outputOrbitConfig(augmentedKF, inRts);
        }
        if (acsConfig.output_trop_sinex)
        {
            outputTropSinex(
                kfState.metaDataMap[TROP_FILENAME_STR + META_SUFFIX],
                time,
                augmentedKF,
                "MIX",
                inRts
        ...
        if (acsConfig.output_bias_sinex)
        {
            writeBiasSinex(
                pppTrace,
                kfState.metaDataMap[BSX_FILENAME_STR + META_SUFFIX],
                time,
                augmentedKF,
                ionNet.kfState,
                receiverMap
        ...
        if (acsConfig.output_clocks)
            ...
            {
                outputClocks(
                    kfState.metaDataMap[CLK_FILENAME_STR + META_SUFFIX],
                    clkOutputTime,
                    augmentedKF,
                    acsConfig.clocks_receiver_sources,
                    acsConfig.clocks_satellite_sources,
                    &receiverMap
                );
                clkOutputTime += max(acsConfig.epoch_interval, acsConfig.clocks_output_interval);
            }
        if (acsConfig.output_orbex)
            while (obxOutputTime <= time)
            {
                outputOrbex(
                    kfState.metaDataMap[ORBEX_FILENAME_STR + META_SUFFIX],
                    obxOutputTime,
                    augmentedKF,
                    acsConfig.orbex_orbit_sources,
                    acsConfig.orbex_clock_sources,
                    acsConfig.orbex_attitude_sources
                );
                obxOutputTime += max(acsConfig.epoch_interval, acsConfig.orbex_output_interval);
            }
        if (acsConfig.output_sp3)
            while (sp3OutputTime <= time)
            {
                outputSp3(
                    kfState.metaDataMap[SP3_FILENAME_STR + META_SUFFIX],
                    sp3OutputTime,
                    augmentedKF,
                    acsConfig.sp3_orbit_sources,
                    acsConfig.sp3_clock_sources,
                    emptyEpoch
                );
                sp3OutputTime += max(acsConfig.epoch_interval, acsConfig.sp3_output_interval);
            }
        if (acsConfig.output_erp)
        {
            writeErpFromNetwork(kfState.metaDataMap[ERP_FILENAME_STR + META_SUFFIX], augmentedKF);
        }
        if (acsConfig.output_ionstec)
        {
        ...
        if (acsConfig.output_ionex)
        {
            ...
            if (acsConfig.process_ionosphere)
            {
                ionexFileWrite(
                    ionTrace,
                    kfState.metaDataMap[IONEX_FILENAME_STR + META_SUFFIX],
                    time,
                    ionNet.kfState
                );
            }
    ...
    if (inRts == false)
    {
        ...
        static double epochsPerRtsInterval =
            acsConfig.pppOpts.rts_interval / acsConfig.epoch_interval;
        static double intervalRtsEpoch = epochsPerRtsInterval;

        if (acsConfig.process_rts && acsConfig.pppOpts.rts_interval && epoch >= intervalRtsEpoch)
        {
