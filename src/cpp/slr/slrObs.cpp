...
/** Read sat ID file
 */
void readSatId(string filepath)  ///< Filepath to sat ID file
{
    ...
    {
        if (line.size() < MIN_LINE_LEN_SATID || line.at(0) == '#')
        {
            continue;
        }
        ...
        SatIdentity newSat;
        string      satName = line.substr(1, 24);
        boost::algorithm::trim(satName);
        ...
        string trackStatus = line.substr(88, 18);
        boost::algorithm::trim(trackStatus);
...
/** Parses a CRD file
 */
vector<CrdSession> readCrdFile(string filepath)  ///< Filepath to CRD file
{
    vector<CrdSession>
        crdSessions;  // One CrdSession per session (pass); May be multiple passes per file

    ifstream fileStream(filepath);
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "Error opening crd file " << filepath << "\n";
    ...
    while (fileStream)
    {
        ...
        if (recordType == "H1")
        {
            ...
            read_h1(str, &h1);
            crdSession.h1     = h1;
            crdSession.readH1 = true;
        }
        else if (recordType == "H2")
        {
            CrdH2 h2;
            read_h2(str, &h2);
            crdSession.h2     = h2;
            crdSession.readH2 = true;
        }
        else if (recordType == "H3")
        {
            CrdH3 h3;
            read_h3(str, &h3);
            crdSession.h3     = h3;
            crdSession.readH3 = true;
        }
        else if (recordType == "H4")
        {
            CrdH4 h4;
            read_h4(str, &h4);
            crdSession.h4     = h4;
            crdSession.readH4 = true;
        }
        else if (recordType == "H5")
        {
            CrdH5 h5;
            read_h5(str, &h5);
            crdSession.h5     = h5;
            crdSession.readH5 = true;
        }
        else if (recordType == "C0")
        {
            CrdC0 c0;
            read_c0(str, &c0);
            crdSession.c0     = c0;
            crdSession.readC0 = true;
        }
        else if (recordType == "C1")
        {
            ...
            read_c1(str, &c1);
            crdSession.c1     = c1;
            crdSession.readC1 = true;
        }
        else if (recordType == "C2")
        {
            CrdC2 c2;
            read_c2(str, &c2);
            crdSession.c2     = c2;
            crdSession.readC2 = true;
        }
        else if (recordType == "C3")
        {
            CrdC3 c3;
            read_c3(str, &c3);
            crdSession.c3     = c3;
            crdSession.readC3 = true;
        }
        else if (recordType == "C4")
        {
            CrdC4 c4;
            read_c4(str, &c4);
            crdSession.c4     = c4;
            crdSession.readC4 = true;
        }
        else if (recordType == "C5")
        {
            CrdC5 c5;
            read_c5(str, &c5);
            crdSession.c5     = c5;
            crdSession.readC5 = true;
        }
        else if (recordType == "C6")
        {
            CrdC6 c6;
            read_c6(str, &c6);
            crdSession.c6     = c6;
            crdSession.readC6 = true;
        }
        else if (recordType == "C7")
        {
            CrdC7 c7;
            read_c7(str, &c7);
            crdSession.c7     = c7;
            crdSession.readC7 = true;
        }
        else if (recordType == "10")
        {
            CrdD10 d10;
            read_10(str, &d10);
            crdSession.d10.push_back(d10);
        }
        else if (recordType == "11")
        {
            CrdD11 d11;
            read_11(str, &d11);
            crdSession.d11.push_back(d11);
        }
        else if (recordType == "12")
        {
            CrdD12 d12;
            read_12(str, &d12);
            crdSession.d12.push_back(d12);
        }
        else if (recordType == "20")
        {
            CrdD20 d20;
            read_20(str, &d20);
            crdSession.d20.push_back(d20);
        }
        else if (recordType == "21")
        {
            CrdD21 d21;
            read_21(str, &d21);
            crdSession.d21.push_back(d21);
        }
        else if (recordType == "30")
        {
            CrdD30 d30;
            read_30(str, &d30);
            crdSession.d30.push_back(d30);
        }
        else if (recordType == "40")
        {
            CrdD40 d40;
            read_40(str, &d40);
            crdSession.d40.push_back(d40);
        }
        else if (recordType == "41")
        {
            CrdD40 d41;
            read_41(str, &d41);
            crdSession.d41.push_back(d41);
        }
        else if (recordType == "42")
        {
            CrdD42 d42;
            read_42(str, &d42);
            crdSession.d42.push_back(d42);
        }
        else if (recordType == "50")
        {
            CrdD50 d50;
            read_50(str, &d50);
            crdSession.d50.push_back(d50);
        }
        else if (recordType == "60")
        {
            CrdD60 d60;
            read_60(str, &d60);
            crdSession.d60.push_back(d60);
        }
        else if (recordType == "00")
        {
            CrdD00 d00;
            read_00(str, &d00);
            crdSession.d00.push_back(d00);
        }
        else if (recordType == "H8")
        {
            crdSessions.push_back(crdSession);
            crdSession = {};
        }  // End of session
        else if (recordType == "H9")
        {
        }  // End of file (ignore)
 ...
 */
int cosparIdToIlrsId(string cosparId)  ///< COSPAR ID to convert
{
...
/** Convert seconds-of-day of an event and the start of the session encompassing the event to a
 * GTime Assumes session duration is <24hrs
 */
 ...
 */
SatSys ilrsIdToSatSys(int ilrsId)  ///< ILRS ID
{
    SatSys Sat;
 ...
 */
void readCrd(string filepath)  ///< CRD file to read
{
...
)
{
    ...
    tracepdeex(
        0,
        filestream,
        "%21s  %-7s %7s %27s  %-11s %8s %4s %18s %13s %12s %9s %12s\n",
        "DateTime",
        "Site_ID",
        "CDP_ID",
        "BigTime",
    ...
    for (auto& [time, slrObs_ptr] : obsMap)
    {
        ...
        tracepdeex(
            0,
            filestream,
            "%21s  %-7s %7d %27.12lf  %-11s %8d %4s %18.12f %13.2f %12.2f %9.1f %12.3f\n",
