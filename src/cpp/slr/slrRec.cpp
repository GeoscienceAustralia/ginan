...
)
{
    ...
    double saturationVaporPressureHpa =
        6.1121 *
        exp((18.678 - temperatureC / 234.5) * (temperatureC / (257.14 + temperatureC)));  // hPa
...
{
    map<char, double>
        biasMap;  // Indexed by "M" models codes - Ref:
                  // https://ilrs.gsfc.nasa.gov/docs/2024/ILRS_Data_Handling_File_2024.02.13.snx

    getSlrRecBias(
        std::to_string(obs.recCdpId),
        obs.Sat,
        obs.time,
        biasMap
    );  // Always do this to allow checking excluded data
    ...
    if (biasMap['X'] || biasMap['N'] || biasMap['Q'] || biasMap['V'])
    {
