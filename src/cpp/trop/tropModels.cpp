...
void defineLocalTropBasis()
{
    ...
    for (auto& [iatm, atmReg] : nav.ssrAtm.atmosRegionsMap)
    {
        ...
        if (atmReg.gridType >= 0 && atmReg.tropGrid)
...
double tropModelCoef(int ind, VectorPos& pos)
{
    ...
    if (recLatDeg > atmReg.maxLatDeg)
        return 0;
    if (recLatDeg < atmReg.minLatDeg)
        return 0;
    ...
    switch (basis.type)
    {
        case E_BasisType::POLYNOMIAL:
        {
            switch (int_to_enum<E_PolyType>(basis.index))
            {
                ...
                case E_PolyType::LON:
                    return londiff;
                case E_PolyType::LAT_LON:
                    return latdiff * londiff;
                case E_PolyType::LAT_SQRD:
                    return latdiff * latdiff;
                case E_PolyType::LON_SQRD:
                    return londiff * londiff;
                default:
                    return 0;
            }
        }
 ...
 */
double gradMapFn(double el)  ///< Elevation (rad)
{
    if (el < 0 || el > 0.9999 * PI / 2)
    {
        return 0;
    }
...
)
{
    ...
    return dTropDx.dryMap * dryZTD + dTropDx.wetMap * wetZTD +
           dTropDx.northMap * tropStates.grads[0] + dTropDx.eastMap * tropStates.grads[1];
}
...
double tropDryZTD(Trace& trace, vector<E_TropModel> models, GTime time, VectorPos& pos)
{
    ...
    for (auto& model : models)
    {
        ...
        {
            case E_TropModel::STANDARD:
                tropSAAS(trace, time, pos, PI / 2, dryZTD, dryMap, wetZTD, wetMap, var);
                break;
            case E_TropModel::SBAS:
                tropSBAS(trace, time, pos, PI / 2, dryZTD, dryMap, wetZTD, wetMap, var);
                break;
            case E_TropModel::GPT2:
                tropGPT2(trace, time, pos, PI / 2, dryZTD, dryMap, wetZTD, wetMap, var);
                break;
            case E_TropModel::VMF3:
                tropVMF3(trace, time, pos, PI / 2, dryZTD, dryMap, wetZTD, wetMap, var);
                break;
            default:
                return 0;
        }
