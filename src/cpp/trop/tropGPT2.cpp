...
struct GptGrid
{
    ...
    array<double, 5> pres;      ///< pressure										a0 A1 B1 A2 B2
                                ///< (pascal)
    array<double, 5> temp;      ///< temperature									a0 A1 B1 A2 B2
                                ///< (kelvin)
    array<double, 5> humid;     ///< humidity										a0 A1 B1 A2 B2
                                ///< (kg/kg)
    array<double, 5>
                     tlaps;     ///< elapse rate									a0 A1 B1 A2 B2 (kelvin/m)
...
/** read GPT grid file
 */
void readgrid(string filepath)  ///< vmf1 coefficients filename
{
    int i = 0;
    ...
    if (!fileStream)
    {
        BOOST_LOG_TRIVIAL(error) << "Error opening gpt grid file" << filepath << "\n";
        return;
    }
...
{
    return +a[0] + a[1] * cosfy + a[2] * sinfy + a[3] * coshy + a[4] * sinhy;
}
 ...
 * gpt2 is used to get pressure, temperature, water vapor pressure and mapping function coefficients
 * and then vmf1 is used to derive dry and wet mapping function.
 */
double tropGPT2(
    Trace&     trace,
    GTime      time,
...
)
{
    ...
    double ew   = 0.5 / 100 * exp(-37.2465 + 0.213166 * tp - 0.000256908 * tp * tp);
    ...
    var          = 0;
    double delay = dryMap * dryZTD + wetMap * wetZTD;
