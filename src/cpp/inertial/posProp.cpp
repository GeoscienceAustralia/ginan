...
)
{
    ...
    for (int i = 0; i < inertialInits.size(); i++)
    {
        ...
        for (auto& [key, index] : inertialInit.subState.kfIndexMap)
        {
            ...
            if (key.type == KF::REC_POS_RATE)
            {
                v(key.num) += offset;
            }
            if (key.type == KF::ORIENTATION)
            {
                q(key.num) += offset;
            }
            if (key.type == KF::GYRO_SCALE)
            {
                gs(key.num) += offset;
            }
            if (key.type == KF::ACCL_SCALE)
            {
                as(key.num) += offset;
            }
            if (key.type == KF::GYRO_BIAS)
            {
                gb(key.num) += offset;
            }
            if (key.type == KF::ACCL_BIAS)
            {
                ab(key.num) += offset;
            }
            ...
            Vector3d deltaR            = (r - r__) - (r_ - r0);
            A.col(index).segment(0, 3) = deltaR / dt / offset;
            Vector3d deltaV            = (v - v__) - (v_ - v0);
            A.col(index).segment(3, 3) = deltaV / dt / offset;
            Vector4d deltaQ            = (q - q__) - (q_ - q0);
            A.col(index).segment(6, 4) = deltaQ / dt / offset;
        }
        ...
        //       derivative.posVelQuatSTM.bottomRightCorner(3, numParams) += dAdParam;
        //       //this is basically assuming params are unchanged and identity matrix?
    }
};
...
{
    if (inertials.empty() || integrationPeriod == 0)
    {
        return;
    }
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        Inertials errors;
        inertialPropagator.odeIntegrator
            .do_step(boost::ref(inertialPropagator), inertials, initTime, dt, errors);
...
KFState getInertialFromState(Trace& trace, string str, const KFState& kfState)
{
    ...
    for (auto& [kfKey, unused] : kfState.kfIndexMap)
    {
        ...
        if (kfKey.type == KF::REC_POS || kfKey.type == KF::REC_POS_RATE ||
...
Inertials prepareInertials(Trace& trace, const KFState& kfState)
{
    ...
    {
        if (kfKey.type != KF::ORIENTATION || kfKey.num != 0)
        {
            continue;
        }
    ...
    for (auto& inertial : inertials)
    {
        ...
        for (auto& [key, index] : subState.kfIndexMap)
        {
            if (key.type == KF::REC_POS)
            {
                inertial.pos(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::REC_POS_RATE)
            {
                inertial.vel(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::ORIENTATION)
            {
                inertial.quat(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::GYRO_SCALE)
            {
                inertial.gyroScale(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::ACCL_SCALE)
            {
                inertial.acclScale(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::GYRO_BIAS)
            {
                inertial.gyroBias(key.num) = inertial.subState.x[index];
            }
            if (key.type == KF::ACCL_BIAS)
            {
                inertial.acclBias(key.num) = inertial.subState.x[index];
            }
        }
        ...
        inertial.posVelQuatSTM = MatrixXd::Identity(
            inertial.subState.kfIndexMap.size(),
            inertial.subState.kfIndexMap.size()
        );
    }
 ...
 */
void applyInertials(Trace& trace, Inertials& inertials, KFState& kfState, GTime time, double tgap)
{
    for (auto& inertial : inertials)
    {
        if (inertial.exclude)
        ...
        MatrixXd transition =
            inertial.posVelQuatSTM -
            MatrixXd::Identity(inertial.posVelQuatSTM.rows(), inertial.posVelQuatSTM.cols());
        transition /= tgap;
        ...
        for (int i = 0; i < 10; i++)
            for (auto& [keyJ, indexJ] : subState.kfIndexMap)
            {
                if (i >= 0 && i < 3)
                {
                    keyI.type = KF::REC_POS;
                    keyI.num  = i - 0;
                }
                if (i >= 3 && i < 6)
                {
                    keyI.type = KF::REC_POS_RATE;
                    keyI.num  = i - 3;
                }
                if (i >= 6 && i < 10)
                {
                    keyI.type = KF::ORIENTATION;
                    keyI.num  = i - 6;
                }
        ...
        // propagated states.
        Vector10d deltaState = inertialState - subState.x.head(10);
