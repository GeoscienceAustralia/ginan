	...
	const	double		timeOffset)
{
	...
	for (int i = 0; i < inertialInits.size(); i++)
	{
		...
		for (auto& [key, index] : inertialInit.subState.kfIndexMap)
		{
			...
			if (key.type == KF::QUAT)			{	q	(key.num) += offset;	}
	...
	const KFState&	kfState)
{
	...
	for (auto& inertial : inertials)
	{
		...
		for (auto& [key, index] : subState.kfIndexMap)
		{
			...
			if (key.type == KF::QUAT)			{	inertial.quat		(key.num) = inertial.subState.x[index];	}
	...
	double			tgap)
{
	...
	for (auto& inertial : inertials)
	{
		...
		for (auto& [keyJ, indexJ] : subState.kfIndexMap)
		{
			...
			if (i >= 6 && i < 10)	{	keyI.type	= KF::QUAT;				keyI.num = i - 6;	}
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (i >= 6 && i < 10)	{	kfKey.type	= KF::QUAT;				kfKey.num	= i - 6;	}
