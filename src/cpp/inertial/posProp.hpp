...
inline InertialState operator*(const double lhs, const InertialState& rhs)
{
    return rhs * lhs;
};
...
inline Inertials operator*(const Inertials& lhs, const double rhs)
{
    Inertials newState = lhs;
    for (int i = 0; i < lhs.size(); i++)
    {
...
inline Inertials operator*(const double rhs, const Inertials& lhs)
{
    Inertials newState = lhs;
    for (int i = 0; i < lhs.size(); i++)
    {
...
struct InertialIntegrator
{
    ...
    runge_kutta_fehlberg78<Inertials, double, Inertials, double, vector_space_algebra>
        odeIntegrator;
