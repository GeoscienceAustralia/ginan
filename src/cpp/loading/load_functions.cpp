...
)
{
    ...
    for (float* lat_ptr = tide_info[0].get_lat_ptr(); lat_ptr != tide_info[0].get_lat_ptr_end();
         lat_ptr++)
        for (float* lon_ptr = tide_info[0].get_lon_ptr(); lon_ptr < tide_info[0].get_lon_ptr_end();
             lon_ptr++)
        {
            double dist, azimuth;
...
void write_BLQ(otl_input* input, int mode)
{
    ...
    for (unsigned int i = 0; i < input->code.size(); i++)
    {
        ...
        out << "  " << input->code[i] << "\n";
        out << "$$ " << input->code[i] << "                    RADI TANG  lon/lat: " << std::setw(9)
            << input->lon[i] << " " << std::setw(9) << input->lat[i] << "\n";

        // write Amplitudes
        for (int i_dir = 0; i_dir < 3; i_dir++)
        {
            for (int it = 0; it < input->wave_names.size(); it++)  // Will need to write nwaves
                out << " " << std::setprecision(7) << std::setw(10)
                    << std::abs(input->out_disp[i][it][i_dir]) << " ";
            out << "\n";
        }
        ...
        {
            for (int it = 0; it < input->wave_names.size(); it++)  // Will need to write nwaves
                out << " " << std::setprecision(4) << std::setw(10)
                    << std::arg(input->out_disp[i][it][i_dir]) * R2D << " ";
            out << "\n";
        }
    }
...
void write_BLQ(otl_input* input)
{
    ...
    for (unsigned int i = 0; i < input->code.size(); i++)
    {
        ...
        // write Amplitudes
        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(5) << std::setw(8)
                << sqrt(SQR(input->dispZ_out[i][it]) + SQR(input->dispZ_in[i][it])) << " ";
        out << "\n";
        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(5) << std::setw(8)
                << sqrt(SQR(input->dispEW_out[i][it]) + SQR(input->dispEW_in[i][it])) << " ";
        out << "\n";
        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(5) << std::setw(8)
                << sqrt(SQR(input->dispNS_out[i][it]) + SQR(input->dispNS_in[i][it])) << " ";
        out << "\n";

        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(1) << std::setw(8)
                << atan2(input->dispZ_out[i][it], input->dispZ_in[i][it]) * R2D << " ";
        out << "\n";
        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(1) << std::setw(8)
                << atan2(input->dispEW_out[i][it], input->dispEW_in[i][it]) * R2D << " ";
        out << "\n";
        for (int it = 0; it < input->tide_file.size(); it++)
            out << " " << std::setprecision(1) << std::setw(8)
                << atan2(input->dispNS_out[i][it], input->dispNS_in[i][it]) * R2D << " ";
        out << "\n";
    }
