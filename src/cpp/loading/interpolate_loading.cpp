 ...
 *
 * With an input data containing the list of the tide file, the Green's function and the coordinate
 * of the site of interest, this code will compute the loading (response to the solid earth to the
 * load generated by the tide) displacement to the tide. It include the vertical, N-S and E-W load
 * in amplitude and phase at the tidal frequencies.
 *
 * @version 0.2
 *
...
void program_options(int argc, char* argv[], otl_input& input)
{
    ...
    if (vm.count("help"))
    {
        ...
        cout << desc << "\n\n";
        cout << "Example: interpolate_loading --type o --grid /<path_to>/oceantide.nc --code 'ALIC "
                "50137M0014' "
                "--location 133.8855 -23.6701 \n\n";
        cout << "Example with input file: interpolate_loading --type o --grid "
                "/<path_to>/oceantide.nc --input "
                "station.csv\n";
    ...
    if (vm.count("type"))
    {
        ...
        if (type == "o")
            ...
            input.type = "Atmospheric";
        else
            throw std::runtime_error(
                "the argument for option '--type' is invalid - only 'o' "
                "(ocean loading) or 'a' (atmospheric loading) "
                "is accepted"
            );
    }
    else
        ...
        while (infile)
        {
            ...
            if (line.size() != 0 and line.at(0) != '#')
            {
                ...
                if (is_ecef)
                {
                    ...
                    {
                        throw std::runtime_error(
                            "xyz coordinates in the csv file should have 4 values 'code, x, y, z'"
                        );
                    }
                }
                else
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        for (int i = 0; i < input.code.size(); i++)
            BOOST_LOG_TRIVIAL(info) << "   * " << input.code[i] << " -- lon, lat -- "
                                    << input.lon[i] << ", " << input.lat[i] << "\n";
        ...
        for (int i_sta = 0; i_sta < input.lat.size(); i_sta++)
            for (int i_wave = 0; i_wave < tideinfo.get_nwave(); i_wave++)
                for (int i_dir = 0; i_dir < 3; i_dir++)
                    input.out_disp[i_sta][i_wave][i_dir] = std::complex<float>(
                        tideinfo.interpolate(
                            i_wave * 6 + 2 * i_dir,
                            input.lon[i_sta],
                            input.lat[i_sta]
                        ),
