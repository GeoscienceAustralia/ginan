...
void tide::fill_ReIm()
{
         ...
         ++amp_ptr, ++phase_ptr, ++in_phase_ptr, ++out_phase_ptr, ++lon_ptr)
    {
        ...
        if (lon_ptr == lon.origin() + lon.num_elements())
        {
            lat_ptr++;
            lon_ptr = lon.origin();
        };
    ...
    //    cout << *std::max_element( Re.origin(), Re.origin() + Re.num_elements()) << " " <<
    //    *std::min_element( Re.origin(), Re.origin() + Re.num_elements())  << "\n"; cout <<
    //    *std::max_element( Im.origin(), Im.origin() + Im.num_elements()) << " " <<
    //    *std::min_element( Im.origin(), Im.origin() + Im.num_elements())  << "\n";
