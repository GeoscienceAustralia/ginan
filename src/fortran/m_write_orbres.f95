MODULE m_write_orbres


! ----------------------------------------------------------------------
! MODULE: m_write_orbres.f03
! ----------------------------------------------------------------------
! Purpose:
...
SUBROUTINE write_orbres (wrtArray, filename)

! ----------------------------------------------------------------------
! Purpose:
!  Write orbit residuals  to an ascii file
! ----------------------------------------------------------------------
! Input arguments:
...
! ----------------------------------------------------------------------
! Array dimensions
      sz1 = SIZE (wrtArray,DIM=1) !satellites
      sz2 = SIZE (wrtArray,DIM=2) !epochs
...
! WRITE THE HEADER TO SPECIFY THE INFO FOR EACH COLUMN
WRITE(UNIT=UNIT_IN,FMT=*) '        MJD                        PRN                   BLOCKTYPE                    &
                                 lambda                    beta_deg                 del_u_deg                  &
                                 yaw_deg                   ANGX_deg                ANGY_deg                   &
                                ANGZ_deg                      dR_m                     dT_m                      &
                                   dN_m                      FR_m^2/s                 FT_m^2/s                 &
                                FN_m^2/s'
! ----------------------------------------------------------------------
      IF (wrt_opt == 1)	THEN
! ----------------------------------------------------------------------
...
! Write data to file | Line by line	  
      DO i_write = 1 , sz1
        if (.not. yml_satellites(i_write)) cycle
        DO i_epoch = 1, sz2
