...
SUBROUTINE writeorbit_multi (orbitsmatrix_crf,orbitsmatrix_trf,orbits_ics_icrf,PRN_array,orbpara_sigma,&
                             filename,erp_filename,EQMfname,VEQfname,POD_version)
...
! IN
      ...
      CHARACTER (*), INTENT(IN) :: filename, erp_filename
...
!       CHARACTER (LEN=1) :: RealT
      ...
      CHARACTER (LEN=70) :: fmt_wrt, fmt_wrt0, fmt_sz2, fmt_erp
      REAL (KIND = prec_q) :: wrtArrayLN 
! ----------------------------------------------------------------------
      INTEGER (KIND = prec_int8) :: Nepochs, Nparam, Nsat, Norbits_ics_icrf, Nsat_real
      INTEGER (KIND = prec_int8) :: i_epoch, i_sat, N_PULSE_Param_glb
! ----------------------------------------------------------------------
      CHARACTER (LEN=100) :: param_id				
      CHARACTER (LEN=500) :: param_value				
      CHARACTER (LEN=512) :: gravity_model_filename, iau_pn_model, DE_fname_data, ocean_tides_model_file
...
! ----------------------------------------------------------------------
    ...
    CHARACTER(LEN=20)  :: str10, str11, STR12,STR13,STR14,STR15,STR16,STR17
...
Nsat_real = 0
do i = 1, Nsat
    if (yml_satellites(i)) Nsat_real = Nsat_real+1
end do
...
! Open ERP file first
      OPEN (UNIT=UNIT_IN,FILE=erp_filename,ACTION="WRITE",POSITION="REWIND", IOSTAT=ios)
      IF (ios /= 0) THEN
         PRINT *, "Error in opening file:", erp_filename
...
write (str5, '(i8)') INT(ERP_day_glb(i,EOP_LOD) * 10000000)
write (str6, '(i7)') INT(ERP_day_glb(i,EOP_X_ERR) * 100000)
...
IF (yml_pod_mode == MODE_IC_INT) WRITE (UNIT=UNIT_IN,FMT='(A,A)',IOSTAT=ios_ith) &
        '#INFO    POD Tool mode:                     ', '4. Orbit & Partials numerical integration from initial conditions file'
IF (yml_pod_mode == MODE_DATA_INT) WRITE (UNIT=UNIT_IN,FMT='(A,A)',IOSTAT=ios_ith) &
        '#INFO    POD Tool mode:                     ', '5. Orbit & Partials numerical integration from initial pod data'
! Initial Conditions
...
gravity_model_filename = trim(yml_gravity_filename)
...
ocean_tides_model_file = trim(yml_ocean_tides_file)
ELSE
...
WRITE (UNIT=UNIT_IN,FMT='(a,i3)'           ,IOSTAT=ios_ith) '#INFO    Number of Satellites:              ',Nsat_real
    ...
    IF (yml_pulses) THEN
       ...
       if (yml_pulse_ref_frame == ICRF) then
          if (BTEST(yml_pulse_parameters, DIR_X - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PR', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PR', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PR', i
             STR99 = trim(STR99)//trim(PNUM)
          end if
          if (BTEST(yml_pulse_parameters, DIR_Y - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PT', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PT', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PT', i
             STR99 = trim(STR99)//trim(PNUM)
          end if
          if (BTEST(yml_pulse_parameters, DIR_Z - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PN', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PN', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PN', i
             STR99 = trim(STR99)//trim(PNUM)
          end if
       else
          if (BTEST(yml_pulse_parameters, DIR_R - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PR', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PR', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PR', i
             STR99 = trim(STR99)//trim(PNUM)
          end if
          if (BTEST(yml_pulse_parameters, DIR_T - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PT', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PT', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PT', i
             STR99 = trim(STR99)//trim(PNUM)
          end if
          if (BTEST(yml_pulse_parameters, DIR_N - one)) then
             if (i<10) write (PNUM, '(1XA2I1)') 'PN', i
             if (i>= 10 .and. i<100) write (PNUM, '(1XA2I2)') 'PN', i
             if (i>=100 .and. i<1000) write (PNUM, '(1XA2I3)') 'PN', i
             STR99 = trim(STR99)//trim(PNUM)
...
! IC 
  ...
  IF (yml_pulses) THEN
   WRITE (UNIT=UNIT_IN,FMT= * ,IOSTAT=ios_ith) orbits_ics_icrf(3:Norbits_ics_icrf-N_PULSE_param_glb,i_sat) &
          &, PULSES_Array_sat_glb(i_sat,:,3:2+yml_pulse_parameter_count) 
  ELSE
...
! IC PULSE RTN
  IF (yml_pulses) THEN
     ...
     IF (i <= 9) THEN
       ...
       WRITE (UNIT=UNIT_IN,FMT= * ,IOSTAT=ios_ith) PULSES_Array_sat_glb(i_sat,i,3:j),&
             &orbpara_sigma(i_sat,NPARAM_glb+6-N_PULSE_param_glb+(i-1)*yml_pulse_parameter_count+1:NPARAM_glb+6-&
