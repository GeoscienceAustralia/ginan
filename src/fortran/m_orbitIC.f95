...
!#ERP_d0  MJD  XP(arcsec) YP(arcsec) UT1-UTC(sec):  58683.0  -9.0511617600253977e-07     3.9276469960236693e-07     3.71764e-08
if (word1_ln == "#ERP_d0") then
   READ (line_ith, *, IOSTAT=ios_data) word_i, word_i, word_i, word_i, word_i, ERP_OFF_ic(1:4)
   erp_offs_read = .true.
end if
...
! ----------------------------------------------------------------------
IF (word1_ln == "#IC_INFO" .or. word1_ln == "#PODIC_INFO") THEN
...
if (erp_offs_read) then
  do i = 1, k
    do j = 2, 4
      ERP_day_IC(i,j) = ERP_day_IC(i,j) + ERP_OFF_ic(j)
    end do
  end do
end if

! Add ERP rate adjustments
if (erp_rates_read) then
  do i = 1, k
! Offset in days between EOP values and time of rate estimate   
    t_off = ERP_day_IC(i,1) - ERP_RATE_ic(1)
    do j = 2, 4
      ERP_day_IC(i,j) = ERP_day_IC(i,j) + ERP_RATE_ic(j) * t_off
    end do
  end do
end if
...
! skip over satellites unestimated by pea
if (len(trim(line_ith)) .le. 72) then
    read (line_ith, * , IOSTAT=ios_data) word_i, PRN_i
    ! No need for warning - printed out above
     !print *, "PRN ", PRN_i, " not estimated by pea, skipping"
    cycle
end if
isat = isat + 1
