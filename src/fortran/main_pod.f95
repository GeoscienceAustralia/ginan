...
!       INTEGER :: ios
      CHARACTER (LEN=512) ::  orbits_partials_fname, erp_fname
!       ,             orbits_fname,   
...
! ----------------------------------------------------------------------
      LOGICAL :: pod_config_exists, is_pod_data
...
pgm_name = 'pod'
call get_yaml(yaml_config, is_pod_data)
FMOD_GRAVFIELD = 0
...
! ----------------------------------------------------------------------
    ...
    EOP_fname_cfg = TRIM(yml_eop_filename)
    ERP_fname_cfg = TRIM(yml_erp_filename)
end if
...
if (LEN(yml_output_dir) > 0) then
    call mkdir(TRIM(yml_output_dir), LEN(TRIM(yml_output_dir)))
end if

! TODO: if pod_data exists overwrite modes?
if (is_pod_data) then
   ! DO SOMETHING
end if
! ----------------------------------------------------------------------
! config checks - modes IC_INT & DATA_INT were afterthoughts, need to add predition arc to determination arc
...
Print *,"POD Tool mode: 4 :: Orbit Integration and Partials from initial conditions file"
Else IF (yml_pod_mode == MODE_DATA_INT) then 
Print *,"POD Tool mode: 5 :: Orbit Integration and Partials from pod data supplied (single sat)"
ELSE
...
ELSE IF (yml_pod_mode == MODE_IC_INT .or. yml_pod_mode == MODE_DATA_INT) THEN
param_id = 'VEQ_integration'
param_value = '1'
Call write_prmfile (EQMfname, fname_id, param_id, param_value)
Call write_prmfile (VEQfname, fname_id, param_id, param_value)
...
write (param_value, *) trim(EOP_fname_cfg)
Call write_prmfile (EQMfname, fname_id, param_id, param_value)
Call write_prmfile (VEQfname, fname_id, param_id, param_value)

param_id = 'ERP_filename'
write (param_value, *) trim(ERP_fname_cfg)
Call write_prmfile (EQMfname, fname_id, param_id, param_value)
Call write_prmfile (VEQfname, fname_id, param_id, param_value)
...
CALL pod_gnss (EQMfname, VEQfname, PRNmatrix, orbpara_sigma, orbits_partials_icrf, orbits_partials_itrf, &
               orbits_ics_icrf,orbit_resR, orbit_resT, orbit_resN, orbdiff2, is_pod_data)
! ----------------------------------------------------------------------
...
mjd = orbits_ics_icrf(1,first_real_sat) 
...
write (erp_fname, FMT='(A,A,A3,I4,I1,A4)') trim (yml_output_dir), "/", "gag", (GPS_week), &
        INT(GPS_day), ".erp"

CALL writeorbit_multi (orbits_partials_icrf, orbits_partials_itrf, orbits_ics_icrf, PRNmatrix, orbpara_sigma, & 
                       orbits_partials_fname, erp_fname, EQMfname, VEQfname, POD_version)
! ----------------------------------------------------------------------
