...
! ----------------------------------------------------------------------
      ...
      INTEGER (KIND = prec_int8) :: i, j, j1, k, l, looptest 
...
If (sz2 .NE. sz4) Then
!print *,"Subroutine statdelta.f95 within Module mdl_statdelta.f95: Input matrices dimension(DIM=2) do not agree"
!print *,"DIM=2", sz2, sz4   
!         STOP "*** - ***"
end if
...
if (AllocateStatus .ne. 0) then
      write(mesg, *) "Not enough memory - failed to allocate ds1_2, dimensions (", &
              Nepochs, ",", Nparam, ")"
      call report('FATAL', pgrm_name, 'statdelta', mesg, 'src/fortran/m_statdelta.f95', 1)
end if
ds1_2 = ds1(:,1:NParam)
...
Do i = 1 , Nepochs
   ...
   Do j = 1 , Nepochs2   
	  ...
	  ! Compute the numerical differences of the state vector
          ! TODO: check here if ds2_2 or ds1_2 array is all zero's at this epoch. Do not count if so
          allzero = .true.
         do l = 3, sz2
             if (ds2_2 (j, l) /= 0.d0) then
                 allzero = .false.
