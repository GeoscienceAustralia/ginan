"""
Manages archival and restoration of GNSS product files.

Provides functions to move product files (SP3, CLK, BIA, etc.) into timestamped
archive subfolders, restore previously archived products to avoid re-downloading,
and archive old output files before a new processing run
"""
...
def archive_products(products_dir: Path = INPUT_PRODUCTS_PATH, reason: str = "manual", startup_archival: bool = False,
    ...
    product_patterns = [
        ...
        "*_ocean.BLQ",  # generated ocean tide loading
        "*_atmos.BLQ",  # generated atmospheric tide loading
    ]
...
def archive_old_outputs(output_dir: Path, visual_dir: Path = None):
    ...
    THIS FUNCTION LOOKS FOR ALL TXT, LOG, JSON, POS, GPX, TRACE, SNX files.
    ...
    # First, collect all files that would be archived (only .POS, .GPX, .TRACE, .SNX)
    files_to_archive = []
    for ext in [".pos", ".POS", ".gpx", ".GPX", ".trace", ".TRACE", ".snx", ".SNX"]:
...
def archive_products_if_rinex_changed(current_rinex: Path, last_rinex: Optional[Path], products_dir: Path) -> Optional[Path]:
    """
...
def archive_products_if_selection_changed(current_selection: Dict[str, Any], last_selection: Optional[Dict[str, Any]],
...
#region Product Restoration From Archive
...
# Static products that happen to share a restorable extension but should not be restored like dynamic products
STATIC_PRODUCT_PREFIXES = {"igs_satellite_metadata"}
...
def restore_from_archive(filename: str, products_dir: Path = INPUT_PRODUCTS_PATH) -> Optional[Path]:
    ...
    if not is_checksum:
        # Skip static products that share a restorable extension (e.g. igs_satellite_metadata.snx)
        if any(filename.startswith(prefix) for prefix in STATIC_PRODUCT_PREFIXES):
            return None
