"""
Main application window for Ginan-UI.

Owns the top-level QMainWindow, initialises and wires together all controllers
(InputController, VisualisationController), manages the Logger system, and handles
the CDDIS credential check dialog shown on startup. Also owns the PEA execution
and download worker threads, and provides the log_message() slot used by Logger
to route messages to the Workflow and Console widgets.
"""

from pathlib import Path
from typing import Optional
from PySide6.QtCore import QCoreApplication, QRegularExpression, QThread, QUrl, Qt, Signal, Slot
from PySide6.QtGui import QTextCursor, QTextDocument
from PySide6.QtWidgets import QMainWindow, QMessageBox
from scripts.GinanUI.app.models.archive_manager import archive_old_outputs, archive_products, archive_products_if_selection_changed
from scripts.GinanUI.app.models.execution import INPUT_PRODUCTS_PATH, Execution
...
from scripts.GinanUI.app.utils.cddis_connection import get_username_from_netrc, test_cddis_connection, write_email
...
from scripts.GinanUI.app.utils.workers import DownloadWorker, PeaExecutionWorker
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.visCtrl.bind_enlarge_button(self.ui.enlargeButton)
        ...
        if not self.offline_mode:
            ...
            Logger.workflow("⚠️ Skipping metadata download - running in offline mode")
    ...
    def _set_processing_state(self, processing: bool):
        ...
        self.ui.snxCheckbox.setEnabled(enabled)

        # "YAML" config tab
        self.ui.yamlOverwriteCheckbox.setEnabled(enabled)
    ...
    #region Process & Download Workflows
    ...
    def _on_process_clicked(self):
        if not self.rnx_file or not self.output_dir:
            Logger.workflow("⚠️ Please select RINEX and output directory first")
            return

        # Check if in offline mode
        if self.offline_mode:
            Logger.workflow("⚠️ Cannot process: Ginan-UI is running in offline mode (no internet connection)")
            self._show_processing_offline_warning()
            return

        # Prevent multiple simultaneous processing
        if self.is_processing:
            Logger.workflow("⚠️ Processing already in progress. Please wait...")
            return
        ...
        if archive_dir:
            Logger.workflow(f"📦 Archived old PPP products → {archive_dir}")

        visual_dir = Path(self.output_dir) / "visual"
        output_archive = archive_old_outputs(Path(self.output_dir), visual_dir)
        if output_archive:
            Logger.workflow(f"📦 Archived old outputs → {output_archive}")
        ...
        Logger.workflow("📡 Starting PPP product downloads...")
        self.download_thread.start()

    @Slot(str, int)
    def _on_download_progress(self, filename: str, percent: int):
        """Update progress display in-place at the bottom of the UI workflow terminal."""
        ...
        cursor = self.ui.workflowTextEdit.textCursor()
        cursor.movePosition(QTextCursor.End)
        flags = QTextDocument.FindFlag.FindBackward
        found_cursor = self.ui.workflowTextEdit.document().find(search_pattern, cursor, flags)

        on_latest_5_lines = self.ui.workflowTextEdit.document().blockCount() - found_cursor.blockNumber() <= 5
        if found_cursor.hasSelection() and on_latest_5_lines:
            ...
            self.ui.workflowTextEdit.setTextCursor(cursor)
            cursor.insertText("\n" + output)
            cursor.movePosition(QTextCursor.End)
            self.ui.workflowTextEdit.setTextCursor(cursor)
    ...
    def _on_atx_downloaded(self, filename: str):
        ...
        Logger.workflow(f"✅ ATX file {filename} installed - ready for RINEX parsing")
    ...
    def _on_download_error(self, msg):
        Logger.workflow(f"⚠️ PPP download error: {msg}")
        self._set_processing_state(False)

    #endregion

    #region CDDIS Credentials
    ...
    def _validate_cddis_credentials_once(self):
        ...
        if not ok and hasattr(self.ui, "cddisCredentialsButton"):
            Logger.workflow("⚠️ No Earthdata credentials. Opening CDDIS Credentials dialog...")
            self.ui.cddisCredentialsButton.click()
            ok, where = gui_validate_netrc()
        if not ok:
            Logger.workflow(f"❌ Credentials invalid: {where}")
            return
        Logger.workflow(f"✅ Earthdata Credentials found: {where}")
        ...
        try:
            ...
            if not ok_conn:
                ...
                return
            Logger.workflow(f"✅ CDDIS connectivity check passed in {why.split(' ')[-2]} seconds")
            ...
            error_msg = str(e)
            Logger.workflow(f"⚠️ No internet connection detected: {error_msg}")
    ...
    def _show_offline_warning(self, title: str, details: str):
        ...
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Icon.Warning)
        msg.setWindowTitle("Ginan-UI - Failed Internet / Server Connection")
        msg.setText(
            "<b>Ginan-UI requires internet access to NASA's CDDIS server function properly. Please ensure your device has a stable internet connection, and your CDDIS credentials are correct</b><br><br>"
    ...
    #region PEA Execution

    def _start_pea_execution(self):
        Logger.workflow("⚙️ Starting PEA execution in background...")
    ...
    def _on_pea_finished(self):
        Logger.workflow("✅ PEA processing completed")
        show_toast(self, "✅ PEA Processing complete!", 3000)
        self._run_visualisation()
        self._set_processing_state(False)

    def _on_pea_error(self, msg: str):
        Logger.workflow(f"⚠️ PEA execution failed: {msg}")
        self._set_processing_state(False)

    #endregion

    #region Worker Thread Management
    ...
    def _run_visualisation(self):
        ...
        if pos_enabled:
            try:
                Logger.workflow("📊 Generating position plots from PEA output...")
                pos_html_files = self.execution.build_pos_plots()
                if pos_html_files:
                    all_html_files.extend(pos_html_files)
                else:
                    Logger.workflow("⚠️ No position plots found")
            except Exception as err:
                Logger.workflow(f"⚠️ Position plot generation failed: {err}")

        # Generate TRACE residual plots
        if trace_enabled:
            try:
                Logger.workflow("📊 Generating trace residual plots from PEA output...")
                trace_html_files = self.execution.build_trace_plots()
                if trace_html_files:
                    all_html_files.extend(trace_html_files)
                else:
                    Logger.workflow("⚠️ No trace plots found")
            except Exception as err:
                Logger.workflow(f"⚠️ Trace plot generation failed: {err}")

        # Update the visualisation panel with all generated plots (or empty list)
        self.visCtrl.set_html_files(all_html_files)

        if not all_html_files:
            Logger.workflow("📊 No visualisations generated - selector disabled")
    ...
    def log_message(self, msg: str, channel = "workflow"):
        """Append a log line to the specified text channel"""
        if channel == "workflow":
            self.ui.workflowTextEdit.append(msg)
    ...
    def on_stopAllClicked(self):
        Logger.workflow("🛑 Stop requested - stopping all running tasks...")
