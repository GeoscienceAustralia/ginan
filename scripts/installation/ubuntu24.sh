...
if [[ "$ubuntu_version" != "24.04" ]]; then
    echo "This script is designed for Ubuntu 24.04. Do you want to continue? (y/n)"
    read -r response
    if [[ ! $response =~ ^[Yy]$ ]]; then
...
curl -fsSL https://pgp.mongodb.com/server-7.0.asc |    $sudo_cmd gpg -o /usr/share/keyrings/mongodb-server-7.0.gpg    --dearmor
echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-7.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/7.0 multiverse" | $sudo_cmd tee /etc/apt/sources.list.d/mongodb-org-7.0.list
$sudo_cmd apt update
